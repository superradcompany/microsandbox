...
class CatalogHistoryTests(unittest.TestCase):
    ...
    def test_cleanup_stops_only_active_sandboxes_then_removes_all_fixture_records(self):
        ...
        with mock.patch.object(HARNESS.subprocess, "run") as run:
            ...
            HARNESS.cleanup(HARNESS.fixture_environment(self.root, self.destination), self.root, io.StringIO())
    ...
    def test_failed_fixture_still_cleans_up_and_remains_a_failure(self):
        ...
        fixture.mkdir()
        args = argparse.Namespace(archive=self.archive, workspace=self.root, output=self.root / "output")
        with mock.patch.object(HARNESS.tempfile, "mkdtemp", return_value=str(fixture)), \
                mock.patch.object(HARNESS.urllib.request, "urlopen", side_effect=[io.BytesIO(self.archive.read_bytes()), io.BytesIO(checksums.encode())]), \
                mock.patch.object(HARNESS.subprocess, "run", side_effect=subprocess.CalledProcessError(100, ["nextest"])) as run, \
                mock.patch.object(HARNESS, "cleanup") as cleanup:
