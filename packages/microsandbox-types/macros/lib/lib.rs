...
/// default, while `clear_*` removes it explicitly. Fields marked `#[config_patch(merge)]` use
/// [`std::iter::Extend`] semantics, while `#[config_patch(merge_with = path)]` delegates to a
/// function taking `(&mut T, T)`. Merge fields also expose `replace_*` and `clear_*` methods. Other
/// fields are atomic: absence inherits and presence replaces the field, including vectors and maps.
