...
impl FileSnapshotState {
    ...
    pub fn head_layer(&self) -> SnapshotManifestResult<&DiskLayer> {
        self.layers
            .last()
    ...
    pub fn state_root(&self) -> SnapshotManifestResult<Option<String>> {
        if self
            .layers
            .iter()
...
fn descriptor_error_value(message: impl Into<String>) -> SnapshotManifestError {
    SnapshotManifestError::ManifestParse(format!("snapshot descriptor: {}", message.into()))
}

fn validate_opaque_id(value: &str, prefix: &str, field: &str) -> SnapshotManifestResult<()> {
...
fn validate_file_state(file: &FileSnapshotState) -> SnapshotManifestResult<()> {
...
fn validate_integrity(integrity: &UpperIntegrity) -> SnapshotManifestResult<()> {
    match integrity {
...
fn validate_digest(value: &str, prefix: &str, field: &str) -> SnapshotManifestResult<()> {
    let Some(encoded) = value.strip_prefix(prefix) else {
...
/// Reject duplicate object keys before decoding canonical metadata.
pub fn reject_duplicate_json_keys(bytes: &[u8]) -> SnapshotManifestResult<()> {
...
fn validate_json_value(value: &serde_json::Value, depth: usize) -> SnapshotManifestResult<()> {
    if depth > 64 {
...
/// Write the bounded canonical JSON subset used by snapshots and checkpoints.
...
mod tests {
    ...
    fn owned_inventory_is_required_but_empty_inventory_keeps_released_bytes() {
        ...
        assert_eq!(manifest.to_canonical_bytes().unwrap(), original);
        let mount = crate::VolumeMount::Owned {
            guest: "/cache".into(),
            storage: crate::OwnedVolumeStorage::Directory { quota_mib: None },
            options: Default::default(),
            stat_virtualization: crate::StatVirtualization::Strict,
            host_permissions: crate::HostPermissions::Private,
        };
        manifest
            .set_owned_volumes(vec![OwnedVolumeCapture {
                mount_id: crate::owned_volume_mount_id(mount.guest()),
