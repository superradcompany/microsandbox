...
fn create_request_still_validates_known_fields_and_variants() {
    for invalid in [
        serde_json::json!({"source": "oci", "reference": "alpine", "network": {"enabled": "yes"}}),
        serde_json::json!({"source": "oci", "reference": "alpine", "network": {"strict": "yes"}}),
        serde_json::json!({"source": "oci", "reference": "alpine", "network": {"max_connections": -1}}),
        serde_json::json!({"source": "oci", "reference": "alpine", "runtime": {"workdir": 42}}),
        serde_json::json!({"source": "oci", "reference": "alpine", "resources": {"vcpus": 256}}),
        serde_json::json!({"source": "oci"}),
        serde_json::json!({"source": "future_source", "reference": "alpine"}),
        serde_json::json!({"source": "oci", "reference": "alpine", "pull_policy": "future_policy"}),
        serde_json::json!({"source": "oci", "reference": "alpine", "rlimits": [{"resource": "nofile", "soft": 64}]}),
...
fn cloud_snapshot_serializes_approved_shape_and_round_trips() {
    ...
    let snapshot = CloudSnapshot::Disk {
        snapshot: CloudSnapshotDetails {
            ...
            sandbox_id: Some("00000000-0000-0000-0000-000000000002".into()),
            digest: digest.clone(),
            size_bytes: 4_294_967_296,
            manifest,
    ...
    assert_eq!(
        ...
        serde_json::json!({
            ...
            "sandbox_id": "00000000-0000-0000-0000-000000000002",
            "digest": digest,
...
fn create_snapshot_request_serializes_approved_shape() {
    let request = CloudCreateSnapshotRequest::Disk {
        snapshot: CloudSnapshotSpec {
            sandbox_id: "00000000-0000-0000-0000-000000000002".into(),
            name: "post-setup".into(),
...
#[test]
fn create_snapshot_request_requires_sandbox_and_name_and_defaults_the_rest() {
    ...
    assert_eq!(request.sandbox_id, "00000000-0000-0000-0000-000000000002");
    ...
    let missing_sandbox = serde_json::from_value::<CloudCreateSnapshotRequest>(serde_json::json!({
        "kind": "disk",
        "name": "post-setup",
    }));
    assert!(missing_sandbox.is_err());
