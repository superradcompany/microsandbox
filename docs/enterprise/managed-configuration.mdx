...
On macOS and Linux, the loader warns if the file or its immediate parent directory is not root-owned or is group- or world-writable. These checks do not reject the policy or inspect ACLs and other ancestor directories. Windows permissions are not checked. Protect the complete path, and keep secrets in [credential stores or references](/configuration#registries).
