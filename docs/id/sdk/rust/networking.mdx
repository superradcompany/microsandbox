...
The default policy denies egress except for an implicit `allow public`, and allows ingress with no rules. See the [defaults rationale](/id/networking/overview#defaults) for the asymmetry.
...
Allow the `Loopback` group (`127.0.0.0/8`, `::1`). The **guest's own** loopback, not the host. To reach a service on the host's localhost, use [`allow_host()`](#allow_host) instead. See the [loopback-vs-host watch-out](/id/networking/overview#loopback-vs-host-a-common-trap).
