...
```promql
# Are exports flowing? Non-zero means yes.
...
rate(microsandbox_collector_exports_failure_total[5m])
  /
clamp_min(rate(microsandbox_collector_exports_success_total[5m]) +
          rate(microsandbox_collector_exports_failure_total[5m]), 1)

# Staleness alert: no successful export in 5 minutes.
time() - microsandbox_collector_last_success_timestamp_seconds > 300
```

The fully-qualified series are emitted by the same scope as the
sandbox metrics (`otel_scope_name="microsandbox-metrics-collector"`,
`otel_scope_version=<crate version>`).

### Failed exports back off exponentially

When an export call fails (timeout, transport error, backend returning
a non-2xx) the per-exporter worker doubles the delay before the next
scheduled flush attempt: `flush_interval`, then `2× flush_interval`,
`4×`, up to a `32× flush_interval` cap. At the default 10s flush
interval that's a worst case of ~5 minutes between retries during a
sustained outage, instead of hammering the backend every 10s.

Explicit `RunningCollector::flush()` calls (and an embedding
application's equivalent) bypass the backoff gate, so a caller that
knows the upstream has recovered can force-retry immediately. On the
first successful export the worker logs `metrics exporter recovered`
at INFO and the multiplier resets to 1×.

Data still accumulates in the buffer (subject to `--max-buffered`) while
backoff is in effect, so a backend that returns within the buffer
window loses no datapoints.

### Cumulative counters reset across sandbox restarts
...
2026-05-30T02:44:31Z sandbox=devbox id=33 cpu=0.000107 \
    mem=13.56MiB/512.00MiB disk_r=89.33MiB disk_w=644.69MiB \
    net_rx=48.05MiB net_tx=268.54KiB uptime=148515.9s
```
</Accordion>
...
| `--http-listen` | none | Bind a tiny health-check HTTP server to `ADDR` (e.g. `127.0.0.1:9095`). Exposes `/healthz` (always `200 ok` once running) and `/readyz` (`200 ok` once the collector attached to shm and started its worker; `503 not ready` during startup/shutdown). For systemd / Kubernetes / Nomad probes. Omit to leave the server off. |
