...
value, and counter `add()` semantics would require tracking
...
OTLP pipeline as the per-sandbox series. You can confirm the sidecar is
actually flowing using the same Prometheus, Grafana, or Datadog queries
the rest of your telemetry runs through:
...
**Label attributes** are the user-defined labels set at sandbox
...
or drop individual keys with `--exclude-label-key <key>` (repeatable).
...
DB is missing (for example `msb-metrics` started before `msb` initialized
`$MSB_HOME`) or a label query fails, that tick ships without labels and
the collector retries on the next tick, so enrichment switches on
automatically once the catalog is available. No restart required.
...
- [Metrics backends](/examples/metrics-backends/grafana-cloud):
  end-to-end examples per provider.
