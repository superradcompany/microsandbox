...
```bash
msb create agent-1 --image alpine --label user.id=alice --label tenant=acme
```

`msb-metrics` reads those labels from the catalog and attaches them to
every datapoint. In Grafana Cloud or Prometheus you then filter by them
(dots become underscores in PromQL):
