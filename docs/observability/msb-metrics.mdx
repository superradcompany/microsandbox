...
| `memory.usage` | `By` | Guest memory in use, measured against the live memory size when the runtime reports it. |
| `memory.limit` | `By` | Effective guest memory limit: boot memory plus plugged memory. Follows live memory resizes. |
| `memory.host_resident` | `By` | Host-resident memory backing the guest, when the VMM can report it. |
...
During a memory resize, `memory.limit` tracks plugged memory and converges to the
accepted target as the guest plugs or releases memory. Right after a resize,
`memory.usage` can briefly be off or missing until the guest's memory statistics
catch up. Sandboxes started by an older runtime keep reporting their boot
allocation until they are restarted.
