...
`msb-metrics` defaults to OTLP over gRPC on port `4317`, which suits
most local collectors and sidecars. To switch to HTTP/Protobuf, set
`--protocol=http`:
...
HTTP/Protobuf's default port is `4318`. Some backends (Grafana Cloud's
OTLP gateway is the common case) require an explicit `/v1/metrics` path
on the endpoint URL.
...
At ~1000 sandboxes per host:

**Heap.** Dominated by the per-exporter buffer. Plan for
`--max-buffered × active sandboxes × ~350B`:

| `--max-buffered` | Worst-case heap (1000 sandboxes) |
|---|---|
| `60` (default) | ~21 MB per exporter |
| `20` | ~7 MB per exporter |

**Shm registry.** Fixed at ~512 KiB regardless of sandbox count.

**Hot path.** `msb-metrics` is pure-shm; no sqlite read on the hot path.
