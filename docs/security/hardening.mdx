...
See [Networking overview](/networking/overview#low-level-custom-policies) for the full rule syntax and SDK equivalents.
...
- Use **read-only** mounts wherever write isn't required. microsandbox enforces read-only host-side, so the guest can't remount around it.
...
- For credentialed egress you want to inspect, enable [TLS inspection](/networking/tls) and pin [DNS](/networking/dns).
...
| Credential-handling egress | Deny-by-default + TLS inspection | Non-root + restricted | Minimal and read-only, with narrow secret allow lists |
