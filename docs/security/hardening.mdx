...
- For credentialed egress you want to inspect, enable [TLS MITM](/networking/tls) and pin [DNS](/networking/dns).
...
| Credential-handling egress | Deny-by-default + TLS MITM | Non-root + restricted | Minimal and read-only, with narrow secret allow lists |
