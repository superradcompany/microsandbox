...
- **Path patterns are location-based, not a confidentiality boundary.** The bind-mount `deny` list hides matching paths at lookup time. On a *writable* mount a guest can rename an ancestor directory to sidestep a path pattern (for example `mv sub x` defeats `sub/.env`). Name-only patterns such as `.env` match the basename anywhere and are robust against renames, but for untrusted guests that must not read particular host files, prefer a read-only mount (or don't mount the directory at all) over any pattern. Pattern matching follows the host filesystem's case-sensitivity, so on Windows or default macOS APFS a `.env` pattern also hides `.ENV`, while on case-sensitive filesystems matching is exact. `msb inspect` echoes the configured deny patterns back in its mount listing as `[deny=...]`, so you can audit what a mount enforces. On macOS, path-pattern matching reconstructs the mount-relative path via `/.vol`/`F_GETPATH`; a transient failure there fails closed and hides the whole subtree, which errs toward over-hiding rather than exposing a hidden path. Directory-only and path patterns also add per-operation work (a type lookup or a path reconstruction); name-only patterns carry the least overhead, which matters most for `readdir`- and lookup-heavy workloads on large directories. See [Volumes](/sandboxes/volumes) for the performance characteristics of each pattern kind.
