...
A bind rootfs is different from an OCI root: it is a host directory that microsandbox modifies in place when applying pre-boot patches. Patch destinations are resolved through a directory capability pinned to the selected root. Existing absolute symlinks use guest semantics and restart at the guest root, relative parent traversal clamps there, and no-replace file creation is atomic.
