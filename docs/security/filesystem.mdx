...
A bind rootfs is different from an OCI root: it is a host directory that microsandbox modifies in place when applying pre-boot patches. Patch destinations must be absolute, canonical guest paths without explicit `..` components and are resolved through a directory capability pinned to the selected root. Existing absolute symlinks use guest semantics and restart at the guest root, parent traversal inside symlink targets clamps there, and no-replace creation and mode application avoid pathname races.
