...
This is the network chapter of the security model: the threats the stack is built to stop and where each defense lives. For day-to-day configuration like policies, ports, DNS knobs, and TLS inspection, see the [Networking guide](/networking/overview).
...
So microsandbox drops `curl http://192.168.1.10` (a machine on your LAN) or `curl http://host.microsandbox.internal:5432` (a service on your host) at the gateway before it reaches the destination.
...
```python Python
from microsandbox import Action, Destination, DestGroup, NetworkPolicy, Rule

policy = NetworkPolicy(
    default_egress=Action.ALLOW,
    default_ingress=Action.ALLOW,
    rules=(Rule.deny(destination=Destination.group(DestGroup.METADATA)),),
)
```

```go Go
network := &m.NetworkConfig{
    DefaultEgress: m.PolicyActionAllow,
    DefaultIngress: m.PolicyActionAllow,
    Rules: []m.PolicyRule{
        {
            Action: m.PolicyActionDeny,
...
DNS-based defenses like block lists, rebind protection, and domain pinning only act on queries the interceptor sees. The gateway intercepts UDP/53 and TCP/53 directly, and DoT (TCP/853) when TLS inspection is enabled. microsandbox either refuses other transports at the port layer (DoQ, mDNS, LLMNR, NetBIOS-NS) or cannot distinguish them from ordinary traffic (DoH on TCP/443). Tunneled DNS (VPN, SOCKS5 hostname mode, HTTP `CONNECT`) rides inside the outer encrypted flow and never reaches the forwarder.
...
- [TLS inspection](/networking/tls): plaintext visibility for per-host policy and secret injection
