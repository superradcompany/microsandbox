...
MEMBER_TABLE_RE = re.compile(
    r"^\|\s*(?:Property|Field|Method|Factory|Function|Member|Class\s+method)\b",
    re.IGNORECASE | re.MULTILINE,
)
METHOD_TABLE_RE = re.compile(
    r"^\|\s*(?:(?:Property|Field|Member)\s*/\s*Method|Method(?:\s*/\s*(?:Property|Protocol))?|Factory|Function|Class\s+method)\b",
    re.IGNORECASE | re.MULTILINE,
)
GENERIC_ROOTS = {
    "attach",
...
def check_member_structure(section_name: str, body: str) -> list[str]:
    """Require first-class, property-first member documentation."""

    errors: list[str] = []
    if section_name != "Types" and MEMBER_TABLE_RE.search(body):
        errors.append(
            f'"{section_name}" contains a member summary table; use member headings instead'
        )

    members = re.findall(
        r'^####\s+.*?<span className="msb-hn">([^<]+)</span>.*$',
        body,
        re.MULTILINE,
    )
    seen: set[str] = set()
    ...
    for member in members:
        ...
        if normalized in seen:
            errors.append(f'"{section_name}" repeats member heading "{member}"')
        seen.add(normalized)
        ...
        if member.endswith("()"):
            ...
            errors.append(
                f'"{section_name}" places property "{member}" after method headings'
            )
...
def check_page(path: Path) -> list[str]:
    ...
    for index, (name, offset) in enumerate(roots):
        ...
        errors.extend(check_member_structure(name, text[offset:end]))
