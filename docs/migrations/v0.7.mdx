...
If v0.7.0–v0.7.2 reports `unsupported persisted sandbox configuration`, upgrade to v0.7.3 and retry first.
...
Use the current CLI before reinstalling older SDKs:
...
Reinstalling an older SDK alone does not reverse migrations. A database downgrade also does not make every newer snapshot or feature usable by an older release. See [downgrade options](/cli/management#msb-self).
