---
title: "DNS"
description: "Risoluzione DNS e applicazione dei criteri"
...
The knobs below cover the day-to-day controls. For the rebinding and TOCTOU protections that also ride on the interceptor, see the [security model](/it/networking/security-model).
...
- DNS over UDP (UDP/53)
- DNS over TCP (TCP/53)
- DNS over TLS (DoT, TCP/853): requires [TLS interception](/it/networking/tls)
...
- [Security model](/it/networking/security-model): rebinding protection and DNS-to-IP binding (TOCTOU defense)
- [TLS interception](/it/networking/tls): domains that get intercepted also go through TLS inspection
