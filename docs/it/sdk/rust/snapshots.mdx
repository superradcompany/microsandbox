...
Disk-only snapshots of a stopped sandbox. See [Snapshots](/it/sandboxes/snapshots) for concepts and walkthroughs; this page is the Rust SDK reference.
...
Boot a fresh sandbox from a snapshot artifact. Mutually exclusive with [`image()`](/it/sdk/rust/sandbox#image) and [`image_with()`](/it/sdk/rust/sandbox#image-with) — the snapshot already pins the image.
