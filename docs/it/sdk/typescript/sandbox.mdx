...
Get a filesystem handle for reading and writing files inside the running sandbox. See [Filesystem](/it/sdk/typescript/filesystem) for the full API.

**Returns**

| Type | Description |
|------|-------------|
| [`SandboxFs`](/it/sdk/typescript/filesystem) | Filesystem handle |
...
| Type | Description |
|------|-------------|
| [`ExitStatus`](/it/sdk/typescript/execution#exitstatus) | Exit code and success flag |
...
| `init(cmd, args?)` | Hand off PID 1 to `cmd` (absolute path or `"auto"`) after agentd setup. See [Custom init system](/it/sandboxes/customize#custom-init-system) |
...
| `volume(guestPath, m => ...)` | Mount a volume — see [Volumes](/it/sdk/typescript/volumes) |
| `patch(p => ...)` | Modify the rootfs before boot — see [Snapshots](/it/sdk/typescript/snapshots) |
...
| `network(n => ...)` | Configure DNS / TLS / policy / secrets — see [Networking](/it/sdk/typescript/networking) |
| `secret(s => ...)` | Add a secret via [`SecretBuilder`](/it/sdk/typescript/secrets#secretbuilder) |
