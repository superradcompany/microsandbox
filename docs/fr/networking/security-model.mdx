...
description: "Comment microsandbox sécurise la couche réseau"
icon: "shield-halved"
---
...
- **Host trust store integrity.** When [`trust_host_cas`](/fr/sdk/rust/networking#trust_host_cas) is opted into, an attacker who can plant a CA on the host has that trust inside every sandbox. Off by default for exactly this reason.

## See also

- [DNS](/fr/networking/dns): the controls that ride on the DNS interceptor
- [TLS interception](/fr/networking/tls): plaintext visibility for per-host policy and secret injection
