...
msb run -t --name goose-demo --replace \
  ...
  debian:bookworm-slim -- sh -lc '
    ...
    ln -s "$HOME/.local/bin/goose" /usr/local/bin/goose &&
    exec goose session
  '
```

On first launch Goose asks whether to share anonymous usage data, then walks through provider configuration. The symlink puts `goose` on the sandbox's normal `PATH`, including later `msb exec` sessions. `GOOSE_PATH_ROOT` keeps its configuration, data, and session state in the `goose-data` volume.
