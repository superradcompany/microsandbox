...
Windows 10 or later is supported in preview. Local sandboxes require Windows Hypervisor Platform, and the PowerShell installer is the current Windows install path.
...
- Downloads the matching `microsandbox-windows-aarch64.zip` or `microsandbox-windows-x86_64.zip` release bundle.
- Verifies the bundle against `checksums.sha256`.
- Installs into `%USERPROFILE%\.microsandbox` unless `MSB_HOME` is set.
- Adds the install `bin` directory to the current user's PATH when needed.
- Runs `msb doctor`.

If the doctor check reports that Windows Hypervisor Platform is missing, the installer offers to run `msb doctor --fix`, which opens an elevated PowerShell prompt.
...
| `MSB_INSTALL_VERSION` | Release tag to install, such as `v0.6.1` |
