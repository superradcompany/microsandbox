...
vol, err := microsandbox.CreateVolume(ctx, "my-data",
    ...
    microsandbox.WithVolumeLabels(map[string]string{"env": "prod"}),
)
```
...
```go
if _, err := vfs.Read("../etc/passwd"); errors.Is(err, microsandbox.ErrPathEscape) {
...
sb, err := microsandbox.CreateSandbox(ctx, "worker",
    ...
    microsandbox.WithMounts(map[string]microsandbox.MountConfig{
        "/host":    microsandbox.Mount.Bind("/var/data", microsandbox.MountOptions{}),
        "/data":    microsandbox.Mount.Named("my-data", microsandbox.MountOptions{}),
        "/scratch": microsandbox.Mount.Tmpfs(microsandbox.TmpfsOptions{SizeMiB: 128}),
        "/img":     microsandbox.Mount.Disk("./data.qcow2", microsandbox.DiskOptions{
            Format:   "qcow2",
            Fstype:   "ext4",
            Readonly: true,
        }),
...
Each `Mount.X(...)` factory returns a [`MountConfig`](#mountconfig). The `kind` discriminator is set internally; callers should always use the factory rather than constructing the struct manually.
...
Discriminated mount configuration produced by the `Mount` factory. Inspect the kind via `Kind()`.
