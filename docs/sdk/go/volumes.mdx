...
Get the Cloud account's always-present default volume. It has no user-assigned name, cannot be removed, and supports direct filesystem operations through `FS()`. The local backend returns `ErrUnsupported`; it never substitutes a directory from the caller's machine.

```go
volume, err := m.GetDefaultVolume(ctx)
err = volume.FS().WriteString("customers/acme.json", `{"active":true}`)
contents, err := volume.FS().ReadString("customers/acme.json")
```
