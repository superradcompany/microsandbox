...
| `Deny` | `[]string` | Host-side deny-list of gitignore-style patterns (e.g. `.env`, `sub/secret`). Matching entries are hidden from the guest and writes to them are forbidden. Bind mounts only |
