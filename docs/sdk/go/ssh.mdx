---
title: SSH
"og:title": Go SDK SSH reference
description: Open sandbox SSH sessions and transfer files over SFTP with the Go SDK.
keywords: ["Go SDK", "Go ssh", "microsandbox ssh"]
---

See [SSH](/sandboxes/ssh) for client sessions, server endpoints, and file transfer workflows.

[Managed inactivity timeouts](/configuration#ssh) take precedence over all timeout options below, including disabling the timeout. This applies to local and cloud sessions using the sandbox backend's retained configuration.
...
Bridge the local terminal to an interactive SSH shell. Requests a PTY sized to the current terminal, puts the terminal into raw mode, forwards keystrokes, and relays window-resize events. Returns when the shell exits or you type the detach key sequence.
...
|---|---|---|
| <span id="o-status"></span>`Status` | `int` | Exit status code |
| <span id="o-stdout"></span>`Stdout` | `[]byte` | Captured stdout bytes |
| <span id="o-stderr"></span>`Stderr` | `[]byte` | Captured stderr bytes |
