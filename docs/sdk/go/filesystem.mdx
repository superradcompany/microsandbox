---
title: Filesystem
"og:title": Go SDK Filesystem reference
description: Read, write, and stream sandbox files with the Go SDK.
keywords: ["Go SDK", "Go filesystem", "microsandbox filesystem"]
---
...
Read the entire contents of a file as raw bytes. The default FFI buffer is 1 MiB. For files larger than ~750 KiB (after base64 inflation), the runtime returns `BufferTooSmall` on the single-shot path. This method then transparently falls back to [`ReadStream`](#fs-readstream), so you get a uniform bytes-returning interface up to runtime memory limits.
...
Copy a file from the host machine into the sandbox. For transferring many files, consider a [bind-mounted volume](/sdk/go/volumes#mount-bind) instead.
...
Open a streaming reader for a file. Use this for files too large to fit in memory; data is delivered in chunks. You must `Close` the returned [`*FsReadStream`](#fsreadstream). [`Read`](#fs-read) falls back to this automatically when a file exceeds the single-shot buffer.
...
Open a streaming writer for a file. Use this for files too large to fit in memory. You must call `Close(ctx)` on the returned [`*FsWriteStream`](#fswritestream) to finalize the write.
...
<div className="msb-params">
  <div className="msb-param">
    ...
    <div className="msb-param-desc">Open write stream; <code>Close(ctx)</code> it to finalize.</div>
  </div>
</div>
...
#### <span className="msb-recv">readStream.</span><span className="msb-hn">Recv()</span>

```go
Recv(ctx context.Context)
```

Receive the next chunk; returns `(nil, nil)` at EOF

<p className="msb-label">Returns</p>

`([]byte, error)`

#### <span className="msb-recv">readStream.</span><span className="msb-hn">WriteTo()</span>

```go
WriteTo(w io.Writer)
```

Drain the stream into `w` using `context.Background()`; implements `io.WriterTo`

<p className="msb-label">Returns</p>

`(int64, error)`

#### <span className="msb-recv">readStream.</span><span className="msb-hn">CopyTo()</span>

```go
CopyTo(ctx context.Context, w io.Writer)
```

Drain into `w` honoring `ctx` for per-chunk cancellation

<p className="msb-label">Returns</p>

`(int64, error)`

#### <span className="msb-recv">readStream.</span><span className="msb-hn">Close()</span>

```go
Close()
```

Release the read stream handle

<p className="msb-label">Returns</p>

`error`

On a write error or partial write, `WriteTo` and `CopyTo` return the partial byte count and leave the stream open so you can recover; close it explicitly in either case.
...
An open streaming write to a guest file. Must be closed with `Close(ctx)` to finalize the write.


#### <span className="msb-recv">writeStream.</span><span className="msb-hn">Write()</span>

```go
Write(p []byte)
```

Send a chunk; implements `io.Writer`, uses `context.Background()` internally

<p className="msb-label">Returns</p>

`(int, error)`

#### <span className="msb-recv">writeStream.</span><span className="msb-hn">WriteCtx()</span>

```go
WriteCtx(ctx context.Context, data []byte)
```

Send a chunk with explicit context control

<p className="msb-label">Returns</p>

`error`

#### <span className="msb-recv">writeStream.</span><span className="msb-hn">Close()</span>

```go
Close(ctx context.Context)
```

Send the EOF marker and wait for the guest to confirm; must be called to complete the write
