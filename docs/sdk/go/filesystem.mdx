...
Open a streaming writer for a file. Use this for files too large to fit in memory. You must call `Close(ctx)` on the returned [`*FsWriteStream`](#fswritestream) to finalise the write.
...
Drain into `w` honouring `ctx` for per-chunk cancellation
