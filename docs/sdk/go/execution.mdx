...
Run commands inside a running sandbox, collect their output, stream events live, or drive an interactive pseudo-terminal programmatically. See [Commands](/sandboxes/commands) for usage examples.
...
<div className="msb-glance">
  ...
  <p className="msb-gl"><span className="msb-dot instance"></span>Methods · *ExecHandle<span className="msb-ct">9</span></p>
  ...
  <a className="msb-row" href="#h-resize"><span className="msb-rn">h.Resize()</span><span className="msb-rg">resize the pseudo-terminal</span></a>
  ...
  <a className="msb-row" href="#sink-close"><span className="msb-rn">sink.Close()</span><span className="msb-rg">close the stdin sink</span></a>

  <p className="msb-gl"><span className="msb-dot builder"></span>Options · ExecOption<span className="msb-ct">6</span></p>
  <div className="msb-chiprow">
    ...
    <a className="msb-chip" href="#withexectty">WithExecTTY()</a>
...
<div className="msb-params">
  ...
  <div className="msb-param">
    ...
    <div className="msb-param-desc">Per-command cwd, timeout, TTY allocation, user, and env.</div>
  </div>
</div>
...
<div className="msb-params">
  ...
  <div className="msb-param">
    <div className="msb-param-key"><code>opts</code><a className="msb-type" href="#execoption">...ExecOption</a></div>
    <div className="msb-param-desc">Per-command options; add <a className="msb-type" href="#withexecstdinpipe">WithExecStdinPipe()</a> to enable stdin and <a className="msb-type" href="#withexectty">WithExecTTY(true)</a> to allocate a pseudo-terminal.</div>
  </div>
</div>
...
A live streaming exec session returned by [`ExecStream`](#sb-execstream) and [`ShellStream`](#sb-shellstream). [`Signal`](#h-signal), [`Kill`](#h-kill), and [`Resize`](#h-resize) may be called concurrently with [`Recv`](#h-recv), allowing a control goroutine to act while another goroutine waits for terminal output. Other method combinations, including concurrent `Recv` calls, are not supported.
...
Resize the pseudo-terminal for this exec session. Use with a session started with [`WithExecTTY(true)`](#withexectty), typically when relaying a terminal-size change from a remote client.

`Resize` may be called concurrently while another goroutine is blocked in [`Recv`](#h-recv).
...
<div className="msb-params">
  <div className="msb-param">
    <div className="msb-param-key"><code>ctx</code><span className="msb-type">context.Context</span></div>
    <div className="msb-param-desc">Cancels the resize call.</div>
  </div>
  <div className="msb-param">
    <div className="msb-param-key"><code>rows</code><span className="msb-type">uint16</span></div>
    <div className="msb-param-desc">New terminal height in character cells.</div>
  </div>
  <div className="msb-param">
    <div className="msb-param-key"><code>cols</code><span className="msb-type">uint16</span></div>
    <div className="msb-param-desc">New terminal width in character cells.</div>
  </div>
</div>
...
A live streaming exec session. Must be closed with [`Close`](#h-close) when done to release Rust-side resources. `Signal`, `Kill`, and `Resize` may run concurrently with `Recv`; other method combinations, including multiple concurrent `Recv` calls, are not supported.
...
| [`Resize(ctx, rows, cols)`](#h-resize) | `error` | Resize the pseudo-terminal |
...
| [`Close()`](#sink-close) | `error` | Close the sink; sends EOF in non-TTY pipe mode |
...
When enabled, the guest process runs with its stdin, stdout, and stderr connected to the PTY. Streaming sessions expose the combined terminal output through [`ExecEventStdout`](#execeventkind); they do not emit a separately attributable stderr stream. Collected output similarly places the combined stream in [`Stdout`](#out-stdout) and leaves [`Stderr`](#out-stderr) empty. Add [`WithExecStdinPipe()`](#withexecstdinpipe) to write to the terminal programmatically, and use [`Resize`](#h-resize) to update its dimensions after the session starts.
...
<Accordion title="Programmable terminal example">
