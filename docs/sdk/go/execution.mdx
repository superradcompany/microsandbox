...
keywords: ["Go SDK", "Go execution", "microsandbox execution"]
---

Run commands inside a running sandbox. Non-zero exit codes are returned in [`ExecOutput`](#execoutput), not as Go errors. See [Commands](/sandboxes/commands) for usage examples.

## Sandbox

The exec entry points live on [`*Sandbox`](/sdk/go/sandbox#sandbox).
...
Collected output and exit status. Non-zero exit codes are results, not Go errors.
...
#### <span className="msb-hn">WithExecCapture()</span>
...
```go
out, err := sb.Shell(ctx, "./migrate.sh", m.WithExecCapture(true))
if err != nil {
    return err
}
_ = out

// Later, even after the sandbox stops:
entries, err := sb.Logs(ctx, m.LogOptions{})
```

</Accordion>

Record this command's output to the sandbox's `exec.log`, where [`Logs`](/sdk/go/sandbox#logs) reads it. Default: off for `Exec`, `ExecStream`, `Shell` and `ShellStream`. An ad-hoc command's output, a shell's above all, can carry anything typed or printed, so it is written to the host's disk only when asked. `ExecDefault` and `ExecDefaultStream` run the sandbox's workload and record it without asking; pass `WithExecCapture(false)` there to opt out. No effect on a sandbox created with [`WithDisableExecLog()`](/sdk/go/sandbox#withdisableexeclog).
...
<div className="msb-params">
  <div className="msb-param">
    <div className="msb-param-key"><code>enabled</code><span className="msb-type">bool</span></div>
    <div className="msb-param-desc"><code>true</code> to record this command's output; <code>false</code> to opt a default-workload call out.</div>
  </div>
</div>
