...
Gracefully shut down the sandbox and wait until stopped state is observed. Local waits up to ten seconds by default, then force-kills the sandbox if it is still running. Cloud waits up to six minutes because shutdown may include creating a durable disk checkpoint. If that deadline expires, this returns `ErrSandboxStopTimedOut` without cancelling the accepted server-side stop, which may still complete afterward. Pass [`WithStopTimeout`](#withstoptimeout) to change the observation deadline.
...
<div className="msb-params">
  <div className="msb-param">
    ...
    <div className="msb-param-desc">Stopped-state observation deadline, e.g. <code>WithStopTimeout(30 * time.Second)</code>.</div>
  </div>
</div>
...
Set how long [`Stop`](#sb-stop) waits for stopped state. Local force-kills when the deadline expires. Cloud returns `ErrSandboxStopTimedOut` instead, while the accepted server-side stop may continue. The default is ten seconds on Local and six minutes on Cloud. This is a `StopOption`, not a `SandboxOption`. Pass it to [`Stop`](#sb-stop).
...
Graceful shutdown with the same Local and Cloud timeout behavior as [`Sandbox.Stop`](#sb-stop); accepts [`StopOption`](#withstoptimeout).
