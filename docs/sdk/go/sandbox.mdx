...
Apply is all-or-nothing: if any change conflicts, is unsupported, or needs a restart you did not allow, it returns an error and nothing changes.

`CPUs` and `MemoryMiB` take effect immediately on a running sandbox as long as the new values stay within [`WithMaxCPUs`](#withmaxcpus) / [`WithMaxMemory`](#withmaxmemory); raising those limits requires a restart. Env and workdir changes affect future execs only; running processes keep their current environment (the plan notes this as a warning). The CLI equivalent is [`msb modify`](/cli/sandbox-commands#msb-modify).

The same method is available on [`SandboxHandle`](#sandboxhandle). It never starts a stopped sandbox; changes made while it is stopped take effect on the next boot. Secret changes are Rust-SDK and CLI only for now.
