...
Read persisted output from the sandbox's `exec.log`. It holds the sandbox's workload output (`ExecDefault`, `AttachDefault`) and any command that asked with [`WithExecCapture(true)`](/sdk/go/execution#withexeccapture); a sandbox created with [`WithDisableExecLog()`](#withdisableexeclog) records nothing. Backed by an on-disk file, so it works for running and stopped sandboxes alike without guest-agent protocol traffic. The default sources are stdout and stderr; add `LogSourceOutput` for PTY-merged output or `LogSourceSystem` for runtime and kernel diagnostics. The same method exists on [`SandboxHandle`](#sandboxhandle) for callers that don't want to start the sandbox first.
...
#### <span className="msb-recv"></span><span className="msb-hn">WithDisableExecLog()</span>

<Tooltip tip="Applies to local sandboxes; microsandbox cloud does not accept this setting."><span className="msb-badge-local">Local-only <Icon icon="circle-info" size={11} /></span></Tooltip>

```go
func WithDisableExecLog() SandboxOption
```

Do not record exec output to the sandbox's `exec.log` at all, not even the workload's. For workloads whose output must not reach the host's disk; [`Logs`](#logs) then returns no user-program output.
...
#### <span className="msb-hn">WithAttachCapture()</span>

```go
func WithAttachCapture(enabled bool) AttachOption
```

Record the attached session's output to the sandbox's `exec.log`. Default: off for `AttachWith`, since an interactive transcript can carry anything typed or printed. `AttachDefault` attaches to the sandbox's workload and records it unless `WithAttachCapture(false)` opts out.
...
<div className="msb-params">
  <div className="msb-param">
    ...
    <div className="msb-param-desc"><code>true</code> to record the session; <code>false</code> to opt <code>AttachDefault</code> out.</div>
  </div>
</div>
