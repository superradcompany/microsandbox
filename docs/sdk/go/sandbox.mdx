...
Wait until every resource is `"applied"`, `"guest-refused"`, or `"failed"`. There is no built-in deadline; cancel `ctx` to stop waiting.
...
Same as [`WaitUntilResized`](#sb-waituntilresized) within `timeout`. A zero or negative timeout checks once. Expiry returns `ErrResizeTimeout`; the host still enforces the new limit. These methods need a matching native SDK; an older native library returns an unsupported-operation error.
...
Wait until every live resize is terminal. The timed variant returns `ErrResizeTimeout` on expiry.
