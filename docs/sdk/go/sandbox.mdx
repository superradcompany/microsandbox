...
keywords: ["Go SDK", "Go sandbox", "microsandbox sandbox"]
---
...
handle.ConnectOrStart(ctx, opts...) (*Sandbox, error)
handle.WaitForStatus(ctx, status) (*SandboxHandle, error)
handle.Restart(ctx, opts...) (*Sandbox, error)
handle.Destroy(ctx, opts...) error
sandbox.ID() string
handle.ID() string
```
...
Full restores use strict external-filesystem admission by default. Pass `WithExternalMountPolicy(ExternalMountRelaxed)` to accept supported mismatches, then inspect `sandbox.RestoreWarnings(ctx)`. Each `ExternalMountWarning` has `GuestPath`, `Reason`, and `StaleInodes []uint64`. `ExternalMountStrict` explicitly selects the default. Existing mount options select the destination paths. See [Snapshot external mounts](/sandboxes/snapshots#external-mounts).

### Creation and restore progress

`CreateSandboxWithProgress` and `RestoreSandboxWithProgress` return a bounded event channel and a single-result channel. Ignoring events does not delay the operation. Always receive the result and close the handle when finished; restored sandboxes are detached, so use `Stop` or `Destroy` to stop them. Cancel `ctx` to cancel the operation.

```go
events, result := m.RestoreSandboxWithProgress(ctx, "saved", "child", m.WithForked())
for event := range events {
    fmt.Println(event.Kind, event.Progress.Phase, event.Progress.CompletedBytes)
}
created := <-result
if created.Err != nil { return created.Err }
defer created.Sandbox.Close()
```

`Kind` is `"pull"` or `"startup"`. Startup phases distinguish backing preparation, waits, reuse, syncing, and activation. `TotalBytes == nil` means indeterminate work. Preparation has no fixed deadline and does not consume the activation timeout; an optional context deadline still bounds the entire operation.
...
<div className="msb-params">
  <div className="msb-param">
    <div className="msb-param-key"><a className="msb-type" href="#sandbox">*Sandbox</a></div>
...
#### <span className="msb-recv">m.</span><span className="msb-hn">ConnectOrCreateSandbox()</span>
...
Restart a previously stopped sandbox. The VM reboots using the persisted configuration and returns a live [`*Sandbox`](#sandbox).
...
Return the opaque stable identity of this persisted sandbox. It remains unchanged across stop and restart and changes when the reusable name is removed and recreated. Use it for equality, logging, and correlation; do not parse it.
...
settled, err := sb.WaitUntilResizedWithTimeout(ctx, time.Minute)
var timeout *m.ResizeTimeoutError
if errors.As(err, &timeout) {
    settled = timeout.Status // last observed status
} else if err != nil {
    return err
}
...
// Grow the owned OCI root disk while it is running
_, err = sb.Modify(ctx, m.ModifyOptions{
...
Plan or apply a configuration change. The returned plan labels each change `"live"`, `"next start"`, `"requires restart"`, or `"unsupported"`.

`CPUs` and `MemoryMiB` resize live within the [`WithMaxCPUs`](#withmaxcpus) / [`WithMaxMemory`](#withmaxmemory) ceilings; raising a ceiling requires a restart. `RootDiskSizeMiB` grows owned managed and flat ext4 roots live, including checkpoint-backed roots, tmpfs root disks can change in either direction on the next boot, and user-supplied disk images are rejected. Env and workdir changes affect future execs only. Stopped roots grow before the new size is saved. Explicit restart and next-start policies remain available.
...
A live CPU or memory resize can take a moment to settle. The new limits are enforced immediately. The returned plan's `ResizeStatus` is a single reading taken when the change was accepted; use [`WaitUntilResized`](#sb-waituntilresized) to wait for the guest. See [`SandboxModificationPlan`](#sandboxmodificationplan).
...
Request graceful shutdown and wait for the same sandbox run to finish, including local runtime ownership release. There is no built-in deadline and no automatic force-kill. Cancelling `ctx` ends the wait without force-killing. Use [`StopWithTimeout`](#sb-stopwithtimeout) or [`WithStopTimeout`](#withstoptimeout) for a bounded wait. These semantics require the matching updated native SDK and runtime; an older native library returns an unsupported-operation error.
...
<div className="msb-params">
  <div className="msb-param">
    ...
    <div className="msb-param-desc">Optional wait deadline, e.g. <code>WithStopTimeout(30 * time.Second)</code>; expiry never force-kills.</div>
  </div>
</div>

#### <span className="msb-recv">sb.</span><span className="msb-hn">StopWithTimeout()</span>

```go
func (s *Sandbox) StopWithTimeout(ctx context.Context, timeout time.Duration) error
```

Bound graceful shutdown with an explicit deadline. The deadline includes local transition ownership, request dispatch, and termination observation. Expiry returns `ErrStopTimeout` without force-killing; zero or a negative duration expires before sending a request. Use [`Kill`](#sb-kill) separately when forced termination is intended.

```go
err := sb.StopWithTimeout(ctx, 30*time.Second)
```
...
Request graceful shutdown and return once the request is sent, without waiting for the sandbox to reach stopped state or scheduling a force-kill. Pair with [`WaitUntilStopped`](#sb-waituntilstopped) to await termination. A paused sandbox is not force-killed merely because its agent cannot process shutdown.
...
#### <span className="msb-recv">sb.</span><span className="msb-hn">ResizeStatus()</span>
...
Read the current live CPU and memory resize status without changing anything. Returns an empty slice when the sandbox is not running.

#### <span className="msb-recv">sb.</span><span className="msb-hn">WaitUntilResized()</span>
...
Wait until every resource is `"applied"`, `"guest-refused"`, or `"failed"`. Returns an empty slice when the sandbox is not running. There is no built-in deadline and a guest that never converges keeps this polling; cancel `ctx` or use [`WaitUntilResizedWithTimeout`](#sb-waituntilresizedwithtimeout) to bound it.

#### <span className="msb-recv">sb.</span><span className="msb-hn">WaitUntilResizedWithTimeout()</span>
...
Same as [`WaitUntilResized`](#sb-waituntilresized) within `timeout`. A zero or negative timeout checks once. Expiry returns a nil slice and a `*ResizeTimeoutError` of kind `ErrResizeTimeout`; use `errors.As` to read the last observed status from its `Status` field. The status is empty if no read completed before the deadline. The host still enforces the new limit. These methods need a matching native SDK; an older native library returns an unsupported-operation error.

#### <span className="msb-recv">sb.</span><span className="msb-hn">WaitForStatus()</span>

```go
func (s *Sandbox) WaitForStatus(ctx context.Context, status SandboxStatus) (*SandboxHandle, error)
```

Wait until this exact sandbox reaches `status`, returning a refreshed metadata handle. The context supplies cancellation or a deadline. Returns `ErrSandboxReplaced` rather than following a reused name.
...
Restart this exact sandbox. Defaults to graceful shutdown, a ten-second timeout, and attached local start. A created, stopped, or crashed sandbox starts directly; a starting sandbox is observed until it settles. Use `WithRestartForce`, `WithRestartTimeout`, and `WithRestartDetached` for explicit controls.
...
Stop and remove this exact sandbox. Defaults to graceful shutdown with a ten-second timeout. Use `WithDestroyForce` or `WithDestroyTimeout` for explicit controls. Returns `ErrSandboxReplaced` instead of deleting a same-name replacement.
...
Release the Rust-side handle. Safe to call multiple times; the second call returns an error with `Kind == ErrInvalidHandle`. A local attached handle stops its VM on `Close`; local detached and cloud handles release client resources only. Stop or remove a cloud sandbox explicitly.
...
Report whether this handle owns the VM process. Local attached handles return `true`; local detached handles and all cloud handles return `false`, because the cloud worker owns the sandbox process. The error return covers stale handles and FFI failures; use [`OwnsLifecycleOrFalse`](#sb-ownslifecycleorfalse) for callers that don't need the error.
...
Convenience wrapper around [`OwnsLifecycle`](#sb-ownslifecycle) that swallows the error and returns `false` for cloud, detached, or invalid handles. Suitable for log lines and best-effort branching.
...
Set the root filesystem source: an OCI image name, local directory path, or disk image path (e.g. `"python:3.12"`, `"docker.io/library/alpine"`). Required for creation; use `RestoreSandbox` for snapshots. Use [`WithImageDisk`](#withimagedisk) when a disk-image root needs an explicit filesystem type.
...
Create the sandbox in detached mode. The VM continues running after the Go process exits; reattach via [`GetSandbox`](#m-getsandbox). `Close` releases this non-owning handle without stopping the VM.
...
<Tooltip tip="Plain-HTTP registry pulls are not available on microsandbox cloud."><span className="msb-badge-local">Local-only <Icon icon="circle-info" size={11} /></span></Tooltip>

```go
func WithRegistryInsecure() SandboxOption
```

Use plain HTTP for the registry instead of HTTPS, for local or self-hosted registries served without TLS such as `localhost:5050/my-app:latest`.

#### <span className="msb-recv"></span><span className="msb-hn">WithRegistryCACerts()</span>

<Tooltip tip="Custom registry CA certificates are not available on microsandbox cloud."><span className="msb-badge-local">Local-only <Icon icon="circle-info" size={11} /></span></Tooltip>

```go
func WithRegistryCACerts(pem []byte) SandboxOption
```

Trust additional PEM-encoded CA certificates when pulling, for registries served with a private certificate authority. Called repeatedly, the bundles accumulate.
...
Like [`WithRegistryCACerts`](#withregistrycacerts) but reads the PEM bundle from a file. The file is read when [`CreateSandbox`](#m-createsandbox) runs, which fails if it is unreadable. Called repeatedly, the bundles accumulate.
...
#### <span className="msb-recv"></span><span className="msb-hn">WithProxy()</span>

```go
func WithProxy(proxy *OutboundProxy) SandboxOption
```

Configure the single proxy used for outbound sandbox connections. See [Proxies](/sdk/go/proxies) for constructors and examples.

<p className="msb-label">Parameters</p>

<div className="msb-params">
  <div className="msb-param">
    <div className="msb-param-key"><code>proxy</code><a className="msb-type" href="/sdk/go/proxies#outboundproxy">*OutboundProxy</a></div>
...
<div className="msb-params">
  <div className="msb-param">
    <div className="msb-param-key"><code>secrets</code><a className="msb-type" href="/sdk/go/secrets#secretentrystruct">...SecretEntry</a></div>
...
Set a deadline for [`Stop`](#sb-stop), which otherwise waits without a built-in timeout. Expiry returns `ErrStopTimeout` without force-killing. Zero or a negative duration expires before sending a shutdown request. This is a `StopOption`, not a `SandboxOption`; pass it to [`Stop`](#sb-stop), or use [`StopWithTimeout`](#sb-stopwithtimeout).
...
<div className="msb-params">
  <div className="msb-param">
    <div className="msb-param-key"><code>timeout</code><span className="msb-type">time.Duration</span></div>
    <div className="msb-param-desc">Stopped-state observation deadline.</div>
  </div>
</div>

#### <span className="msb-recv"></span><span className="msb-hn">WithConnectOrStartDetached()</span>

```go
func WithConnectOrStartDetached() ConnectOrStartOption
```

When `ConnectOrStart` must start a local sandbox, start it in detached/background mode. This option does not change ownership when the sandbox is already running.

#### <span className="msb-recv"></span><span className="msb-hn">WithRestartForce()</span>

```go
func WithRestartForce() RestartOption
```

Force-kill instead of requesting graceful shutdown during `Restart`.

#### <span className="msb-recv"></span><span className="msb-hn">WithRestartTimeout()</span>
...
Start the restarted local sandbox in detached/background mode.

#### <span className="msb-recv"></span><span className="msb-hn">WithDestroyForce()</span>

```go
func WithDestroyForce() DestroyOption
```

Force-kill instead of requesting graceful shutdown during `Destroy`.

#### <span className="msb-recv"></span><span className="msb-hn">WithDestroyTimeout()</span>
...
Set how long [`Kill`](#sb-kill) waits for stopped-state observation. The default is five seconds. This is a `KillOption`; pass it to [`Kill`](#sb-kill).
...
Use `ResolveRuntime(RuntimeConfig{})` to require a pre-provisioned runtime without downloading. See [Runtime setup](/sdk/setup#customize-installation).
...
Return the opaque stable identity captured by this metadata handle. Receiver lifecycle calls remain bound to this value and return `ErrSandboxReplaced` when the reusable name now points to another sandbox.
...
<Tooltip tip="Parsed SandboxConfig is local-only; use the raw cloud specification from JSON inspection on microsandbox cloud."><span className="msb-badge-local">Local-only <Icon icon="circle-info" size={11} /></span></Tooltip>
...
#### <span className="msb-recv">h.</span><span className="msb-hn">ConnectOrStart()</span>

```go
func (h *SandboxHandle) ConnectOrStart(ctx context.Context, opts ...ConnectOrStartOption) (*Sandbox, error)
```

Connect when this exact sandbox is running, wait through `SandboxStatusStarting`, or start it when it is created, stopped, or crashed. Draining and paused states are rejected. `WithConnectOrStartDetached` affects only a required local start; connecting to an already-running sandbox does not change ownership.
...
Boot a stopped sandbox. The returned local handle is attached; a cloud handle does not own the service-managed VM.
...
#### <span className="msb-recv">h.</span><span className="msb-hn">WaitForStatus()</span>

```go
func (h *SandboxHandle) WaitForStatus(ctx context.Context, status SandboxStatus) (*SandboxHandle, error)
```

Wait until this exact sandbox reaches `status`, returning a refreshed handle. The context supplies cancellation or a deadline. Returns `ErrSandboxReplaced` rather than rebinding to a replacement.
...
Restart this exact sandbox with the same state and option semantics as [`Sandbox.Restart`](#sb-restart).
...
Stop and remove this exact sandbox. A stale handle returns `ErrSandboxReplaced` instead of destroying a same-name replacement.
...
Request graceful shutdown and wait for this exact sandbox run and local runtime ownership release, without a built-in deadline or force-kill. Accepts [`StopOption`](#withstoptimeout); context cancellation ends only the wait.

<p className="msb-label">Returns</p>

`error`

#### <span className="msb-recv">h.</span><span className="msb-hn">StopWithTimeout()</span>

```go
StopWithTimeout(ctx, timeout)
```

Bound graceful shutdown for this exact sandbox run. Expiry returns `ErrStopTimeout` without force-killing; zero or a negative duration expires before sending a request.
...
Send a graceful shutdown request without waiting for completion or scheduling forced termination.
...
#### <span className="msb-recv">h.</span><span className="msb-hn">ResizeStatus()</span>
...
Current live CPU and memory resize status of this exact sandbox. Empty when not running. Returns `ErrSandboxReplaced` if the name now refers to another sandbox.

<p className="msb-label">Returns</p>

`([]ResourceResizeStatus, error)`

#### <span className="msb-recv">h.</span><span className="msb-hn">WaitUntilResized()</span>

<Tooltip tip="Live resize is not available on microsandbox cloud."><span className="msb-badge-local">Local-only <Icon icon="circle-info" size={11} /></span></Tooltip>

```go
WaitUntilResized(ctx)
WaitUntilResizedWithTimeout(ctx, timeout)
```

Wait until every live resize on this exact sandbox is terminal. Returns an empty slice when the sandbox is not running, and `ErrSandboxReplaced` if the name now refers to another sandbox. The untimed variant keeps polling while a guest never converges; the timed variant returns a `*ResizeTimeoutError` on expiry, with the last observed status in its `Status` field.
...
<Tooltip tip="Cloud disk snapshots require a stopped or crashed source. Local snapshots also support running or paused sources."><span className="msb-badge-limited">Limited on cloud <Icon icon="circle-info" size={11} /></span></Tooltip>

```go
Snapshot(ctx, name)
```

Capture a disk snapshot. Locally, the name selects a member within the sandbox's snapshot group. See [Snapshots](/sdk/go/snapshots).
...
| State | `string` | `"applied"` when requested, actual, and enforced match; `"converging"` while the guest is still onlining or offlining CPUs or plugging or releasing memory; `"guest-refused"` when the guest would not cooperate (the host enforces the new limit anyway); `"accepted"` or `"failed"` otherwise |

A shrink stays `"converging"` until the guest releases the capacity. `"applied"`, `"guest-refused"`, and `"failed"` are terminal.
...
| Secrets | [`[]SecretEntry`](/sdk/go/secrets#secretentrystruct) | Secret injection entries |
...
| Constant | Value | Description |
|----------|-------|-------------|
| `SandboxStatusCreated` | `"created"` | Persisted resource exists but has not started |
| `SandboxStatusStarting` | `"starting"` | Runtime is booting or waiting for capacity |
...
| Field | Type | Description |
|-------|------|-------------|
| Kind | [`PatchKind`](#patchkind) | Patch flavor |
...
The setup-only `InstallOptions` type is documented under [Runtime setup](/sdk/setup#customize-installation).
