...
Override the user-workload entrypoint baked into the image: the command the agent execs per request. This is the user workload, **not** the guest PID 1. To override the guest PID 1, use [`WithInit`](#withinit) instead.
...
| RootDiskSizeMiB | `uint64` | Desired root disk size in MiB. Managed and flat OCI disks are grow-only; applies on restart or next start |
