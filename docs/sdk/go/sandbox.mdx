...
Call [`Init`](#init) once at program startup before any other SDK function. The FFI library ships embedded in the SDK and extracts to `~/.microsandbox/` on first run; msb + libkrunfw download from the matching GitHub release if not already present. Idempotent.
...
Configure via setup options:

- [`WithInstallDir(dir)`](#withinstalldir) — extract to a path other than `~/.microsandbox/`
- [`WithSkipDownload()`](#withskipdownload) — refuse to fetch msb + libkrunfw (require them to be pre-installed)
...
```go
func Init(ctx context.Context, opts ...SetupOption) error
```

Prepare the SDK for use: extract the embedded FFI library, ensure msb + libkrunfw are present, dlopen the FFI library. Idempotent — subsequent calls are no-ops. Options apply only to the first call.

**Parameters**

| Name | Type              | Description                                                                         |
| ---- | ----------------- | ----------------------------------------------------------------------------------- |
| ctx  | `context.Context` | Cancels the msb + libkrunfw download (no effect once the library is loaded)         |
| opts | `...SetupOption`  | See [`WithInstallDir`](#withinstalldir) and [`WithSkipDownload`](#withskipdownload) |
...
Override the on-disk install location (default: `~/.microsandbox/`).
...
Prevent `Init` from fetching the msb + libkrunfw bundle from GitHub. Use when the runtime is already on disk at the install path (e.g. air-gapped CI). The embedded FFI library is unaffected — it ships with the SDK.
...
Return the version reported by the loaded library. Returns `ErrLibraryNotLoaded` if `Init` was not called.
