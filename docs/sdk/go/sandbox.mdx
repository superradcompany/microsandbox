...
#### <span className="msb-recv"></span><span className="msb-hn">WithSnapshotDiskOnly()</span>
<div className="msb-tags"><span className="msb-tag is-builder">option</span></div>
...
Cold-boot only the disk state carried by a full snapshot. Use with [`WithFromSnapshot()`](#withfromsnapshot). Without it, a full snapshot resumes its captured execution and does not run the image CMD.
