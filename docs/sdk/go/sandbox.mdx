---
title: Sandbox
"og:title": Go SDK Sandbox reference
description: Create and manage microVM sandboxes with the Go SDK. Configure resources, connect to running sandboxes, and control their lifecycle.
keywords: ["Go SDK", "Go sandbox", "microsandbox sandbox"]
---

Use the microsandbox Go SDK to create a microVM sandbox for untrusted code, run commands, stream logs and metrics, and shut it down. Each sandbox has its own guest kernel. See [Overview](/sandboxes/overview) for configuration examples and [Lifecycle](/sandboxes/lifecycle) for state management.
...
Runtime installation and verification helpers are documented under [Runtime setup](/sdk/setup#resolve-check-install-and-ensure). The Go-only version helpers are under [Inspect Go versions](/sdk/setup#inspect-go-versions).
...
Same as [`Attach`](#sb-attach), but takes [`AttachOption`](#attachoption) values. Use them to run the session as a different guest user, in a different working directory, with extra environment variables, or with custom detach keys.
...
#### <span className="msb-recv">sb.</span><span className="msb-hn">RestoreWarnings()</span>
...
|---|---|---|
| `GuestPath` | `string` | Guest-visible mount path. |
| `Reason` | `string` | Why the resource could not be reconstructed. |
| `StaleInodes` | `[]uint64` | Permanently invalid captured node IDs; empty when the whole export is unavailable. |
...
| Field | Type | Description |
|---|---|---|
| <span id="entry-source"></span>`Source` | [`LogSource`](#logsource) | Origin of the captured data |
| <span id="entry-sessionid"></span>`SessionID` | `*uint64` | Relay-monotonic session id; nil for system entries |
| <span id="entry-timestamp"></span>`Timestamp` | `time.Time` | Wall-clock capture time on the host |
| <span id="entry-data"></span>`Data` | `[]byte` | The captured bytes |
| <span id="entry-cursor"></span>`Cursor` | `string` | Opaque resume token; pass to [`LogStreamOptions.FromCursor`](#logstreamoptions) |
