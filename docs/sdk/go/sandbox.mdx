...
The Go SDK uses **functional options** rather than a fluent builder — pass `WithImage(...)`, `WithMemory(...)`, etc. as variadic arguments to [`CreateSandbox`](#createsandbox). Every operation takes a `context.Context` for cancellation and deadline propagation. The package is imported as:
...
Call [`EnsureInstalled`](#ensureinstalled) once at program startup before any other SDK function. It downloads the runtime (msb launcher, libkrunfw, FFI shared library) into `~/.microsandbox/` on first run and is a no-op on subsequent invocations.
...
Set `MICROSANDBOX_LIB_PATH` to a locally-built `libmicrosandbox_go_ffi` to bypass the download during development; set `MICROSANDBOX_SKIP_DOWNLOAD=1` to fail fast instead of fetching from GitHub.
...
Ensure the microsandbox runtime is installed and loaded into the current process. Idempotent and safe for concurrent calls.

**Parameters**

| Name | Type | Description |
|------|------|-------------|
| ctx | `context.Context` | Cancels the download (no effect after the library is loaded) |
...
Report whether the runtime artefacts are present on disk at the SDK's pinned version. Does **not** dlopen the library.
...
Return the version reported by the loaded library. Returns `ErrLibraryNotLoaded` if `EnsureInstalled` was not called.
...
sb, err := microsandbox.CreateSandbox(ctx, "my-sandbox",
    ...
    microsandbox.WithEnv(map[string]string{"PYTHONDONTWRITEBYTECODE": "1"}),
)
if err != nil {
    return err
}
...
Look up a sandbox by name and return a metadata handle without connecting to it. Returns `ErrSandboxNotFound` if no such sandbox exists. The returned [`*SandboxHandle`](#sandboxhandle) exposes `Connect`, `Start`, `Stop`, `Kill`, `Remove`, `Metrics`.
...
Gracefully shut down the sandbox (SIGTERM by default; override with [`WithStopSignal`](#withstopsignal)). Does **not** wait for the VM process to exit — use [`StopAndWait`](#stopandwait) for that.
...
fmt.Printf("CPU: %.1f%% Mem: %d bytes Uptime: %s\n",
    m.CPUPercent, m.MemoryBytes, m.Uptime)
```
...
Factory namespace for [`InitConfig`](#initconfig) values used with [`WithInit`](#withinit) to hand off PID 1 inside the guest after agentd setup. See [Custom init system](/sandboxes/customize#custom-init-system) for image picks and shutdown semantics.
...
```go
microsandbox.WithInit(microsandbox.Init.Cmd(
    "/lib/systemd/systemd",
    microsandbox.InitOptions{
...
OCI image, local path, or disk image (e.g. `"python:3.12"`, `"docker.io/library/alpine"`). Required unless [`WithSnapshot`](#withsnapshot) is used.
...
Like [`WithReplace`](#withreplace) but with a caller-specified grace period between SIGTERM and SIGKILL. Implies `WithReplace`. A zero duration skips SIGTERM and SIGKILLs immediately.
...
Override the user-workload entrypoint baked into the image. Note: this is the user workload, not PID 1 — for that, use [`WithInit`](#withinit).
...
Hand off PID 1 to a guest init binary. Construct `cfg` via the [`Init`](#init) factory.
...
Override the signal sent on graceful stop. Defaults to `SIGTERM`. Pass standard names like `"SIGTERM"` or `"SIGINT"`.
...
Attach key-value labels to the sandbox config. Merges on top of any image-level labels (user values win on conflict).
...
Append rootfs patches applied before the VM boots. Patches are only compatible with OverlayFS rootfs (not disk images). Construct via the [`Patch`](#patch) factory.
...
| StopSignal | `string` | Signal sent on graceful stop |
...
A single rootfs patch produced by the [`Patch`](#patch) factory.
...
Tuning struct for the `Patch` factory.
