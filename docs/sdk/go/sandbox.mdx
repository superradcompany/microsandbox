...
Override the host-side `libkrunfw` shared library used to boot this sandbox. Use this for custom kernel/firmware builds. The path is a host path, not a guest path, and must point to the platform shared library (`.so` on Linux, `.dylib` on macOS).
...
| LibkrunfwPath    | `string`                                             | Host-side custom `libkrunfw` path                                    |
