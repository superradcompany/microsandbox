...
The Go SDK exposes local OCI image-cache operations through the package-level `Image` factory. These APIs inspect and prune images that have already been pulled by sandbox creation.
...
```go
images, err := microsandbox.Image.List(ctx)
if err != nil {
    return err
}
