...
| TCPListenBacklog | `*uint32` | Accept-queue depth for published TCP port listeners, 1 to 2147483647. `nil` keeps the default, 1,024; the host kernel caps it at its own `somaxconn` (4,096 by default on Linux, 128 on macOS) |
...
For dedicated snapshot restore, use `WithRestoreMaxTCPConnections` and `WithRestoreMaxUDPConnections`, or the corresponding fields in `RestoreConfig`. `WithRestoreMaxConnections` and `RestoreConfig.MaxConnections` remain deprecated TCP aliases with the same duplicate-alias restriction. These limits are destination resource overrides; `WithRestoreNetworkPolicy` accepts only policy rules and default actions. `WithRestoreTCPListenBacklog` (or `RestoreConfig.TCPListenBacklog`) sets the accept-queue depth for the listeners a restored child publishes.
