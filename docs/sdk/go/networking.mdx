...
| EgressRateLimiter | [`*RateLimiterConfig`](#ratelimiterconfig) | Outbound (egress) rate limiter. `nil` means unlimited. Takes effect on the next sandbox start |
| IngressRateLimiter | [`*RateLimiterConfig`](#ratelimiterconfig) | Inbound (ingress) rate limiter. `nil` means unlimited. Takes effect on the next sandbox start |
...
sb, err := m.CreateSandbox(ctx, "throttled",
    m.WithImage("python"),
    m.WithNetwork(&m.NetworkConfig{
        EgressRateLimiter: &m.RateLimiterConfig{
...
Rate limiter for one traffic direction. Caps bandwidth (bytes) and packet rate (frames) independently; a `nil` bucket leaves that dimension unlimited. Cloud sandboxes reject rate limiters as unsupported for now.
