...
The Go SDK's network surface is a single [`NetworkConfig`](#networkconfig) struct passed via [`WithNetwork`](/sdk/go/sandbox#withnetwork). Common shapes come from the [`NetworkPolicy`](#networkpolicy) factory; custom rule sets are built by populating `NetworkConfig.Rules` directly.
...
The factory namespace returning common preset [`*NetworkConfig`](#networkconfig) values. Invoke through the package-level `NetworkPolicy` value.
...
```go
Rules: []microsandbox.PolicyRule{
    {Action: microsandbox.PolicyActionAllow, Destination: "10.0.0.0/8"},  // matches everything in 10.x
    {Action: microsandbox.PolicyActionDeny,  Destination: "10.0.0.5"},    // never reached
},
```
...
|-------|------|-------------|
| Policy | [`NetworkPolicyPreset`](#networkpolicypreset) | Preset name. Mutually exclusive with `Rules` for canned configurations; set by the [`NetworkPolicy`](#networkpolicy) factory |
...
```go
sb, err := microsandbox.CreateSandbox(ctx, "inspect",
    microsandbox.WithImage("python:3.12"),
    microsandbox.WithNetwork(&microsandbox.NetworkConfig{
