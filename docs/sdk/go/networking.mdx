...
| MaxTCPConnections | `*uint` | TCP connection cap; zero means unlimited |
...
| OnSecretViolation | [`ViolationAction`](/sdk/go/secrets#violationactionstring-enum) | Sandbox-wide action when a secret is sent to a disallowed host. Per-secret overrides via `SecretEntry.OnViolation` |
