...
| TxRateLimiter | [`*RateLimiterConfig`](#ratelimiterconfig) | Guest upload (egress) rate limiter. `nil` means unlimited. Takes effect on the next sandbox start |
| RxRateLimiter | [`*RateLimiterConfig`](#ratelimiterconfig) | Guest download (ingress) rate limiter. `nil` means unlimited. Takes effect on the next sandbox start |
...
sb, err := m.CreateSandbox(ctx, "throttled",
    m.WithImage("python"),
    m.WithNetwork(&m.NetworkConfig{
        TxRateLimiter: &m.RateLimiterConfig{
