...
For dedicated snapshot restore, use `WithRestoreMaxTCPConnections` and `WithRestoreMaxUDPConnections`, or the corresponding fields in `RestoreConfig`. `WithRestoreMaxConnections` and `RestoreConfig.MaxConnections` remain deprecated TCP aliases with the same duplicate-alias restriction. These limits are destination resource overrides; `WithRestoreNetworkPolicy` accepts only policy rules and default actions.
