...
keywords: ["Go SDK", "Go networking", "microsandbox networking"]
---

Configure a sandbox's network stack: a first-match-wins egress/ingress policy, published ports, DNS interception, TLS interception, and secret-violation handling. See [Networking](/networking/overview) for the conceptual overview and [TLS MITM](/networking/tls) for proxy details.

The Go SDK exposes networking as a [`NetworkConfig`](#networkconfig) struct passed to [`WithNetwork`](#m-withnetwork). Outbound proxies use the sibling [`WithProxy`](/sdk/go/proxies#m-withproxy) option. Common policy shapes come from the [`NetworkPolicy`](#networkpolicy) factory; custom firewalls are built by populating `NetworkConfig.Rules` directly.
...
<div className="msb-params">
  <div className="msb-param">
    ...
    <div className="msb-param-desc">Option to pass to <a className="msb-type" href="/sdk/go/sandbox#m-createsandbox">CreateSandbox</a>.</div>
  </div>
</div>
...
<p className="msb-member-group" id="destination-groups">Destination groups</p>
...
| DisableStrict | `bool` | Opt out of strict hostname policy enforcement. By default, hostname allows require inspectable request authority: plain HTTP exposes `Host`; HTTPS requires TLS interception that is not bypassed |
...
| TCPAcceptQueueSize | `*uint32` | Accept-queue depth for published TCP port listeners, 1 to 2147483647. `nil` keeps the default, 1,024; the host kernel caps it at its own `somaxconn` (4,096 by default on Linux, 128 on macOS) |
...
| MaxTCPConnections | `*uint` | TCP connection cap; defaults to unlimited for single-tenant and 1,024 for multi-tenant; zero means unlimited |
| MaxConnections | `*uint` | Deprecated TCP alias; cannot be combined with MaxTCPConnections |
| MaxUDPConnections | `*uint` | UDP relay session cap; defaults to unlimited for single-tenant and 1,024 for multi-tenant; zero means unlimited. At capacity, the least recently active session is evicted. |
| RateLimiter | [`*NetworkRateLimiterConfig`](#networkratelimiterconfig) | Rate limits grouped by direction. `nil` means unlimited |
| SecretViolationAction | [`ViolationAction`](/sdk/go/secrets#violationactionstring-enum) | Sandbox-wide action when a secret is sent to a disallowed host. Per-secret overrides via `SecretEntry.ViolationAction` |
...
Connection limits use pointers to distinguish an omitted value from explicit zero. `MaxConnections` and `MaxTCPConnections` cannot both be specified, even with the same value. Either TCP spelling can be combined with `MaxUDPConnections`.

For dedicated snapshot restore, use `WithRestoreMaxTCPConnections` and `WithRestoreMaxUDPConnections`, or the corresponding fields in `RestoreConfig`. `WithRestoreMaxConnections` and `RestoreConfig.MaxConnections` remain deprecated TCP aliases with the same duplicate-alias restriction. These limits are destination resource overrides; `WithRestoreNetworkPolicy` accepts only policy rules and default actions. `WithRestoreTCPAcceptQueueSize` (or `RestoreConfig.TCPAcceptQueueSize`) sets the accept-queue depth for the listeners a restored child publishes.

Historical runtime launch contracts reject explicit UDP limits, including zero, with an upgrade-required error. See [deployment profiles](/networking/overview#deployment-profiles) for historical TCP limits and defaults.
...
Composable profile names accepted by [`NetworkPolicy.FromProfiles()`](#networkpolicy-fromprofiles).
