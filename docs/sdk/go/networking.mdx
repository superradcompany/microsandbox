...
| Strict | `bool` | Require hostname-based policy allows to use an inspectable request authority. Plain HTTP exposes `Host`; HTTPS requires TLS interception that is not bypassed |
