...
Create a disk snapshot from a stopped or crashed sandbox, or set `Resumable` to checkpoint a running sandbox. [`SnapshotCreateOptions.Name`](#snapshotcreateoptionsstruct) and [`SnapshotCreateOptions.FromSandbox`](#snapshotcreateoptionsstruct) are both required.
...
Snapshot scope: `SnapshotScopeDisk` (`"disk"`) or `SnapshotScopeResumable` (`"resumable"`).
...
Scope of what a snapshot captures: disk-only or disk plus resumable VM state.
...
| Checkpoint | `*SnapshotCheckpointVerifyStatus` | Verified checkpoint root for resumable state; otherwise `nil` |
