...
Use `WithRestoreConfig(RestoreConfig{...})` for explicit `Volumes`, `CapturedVolumes`, `Ports`, `Vsock`, `User`, and `LogLevel`. Image, replacement, and startup-command options are not accepted. Host resources remain unmapped unless explicitly supplied or `WithDangerouslyInheritResources()` is selected. Full restore rejects missing external filesystems and additional disks by default; `WithAllowMissingResources()` opts into unavailable devices with warnings. `WithExternalMountPolicy` validates supplied filesystem objects, independently of missing-resource permission. Root and owned storage remain required.
...
Clone `source` (path, name, or digest) into a new snapshot named `newName`. Always writes a new artifact rather than mutating `source` in place: `source` and anything referencing it by digest are left untouched. With [`SnapshotCloneOptions.Sparsify`](#snapshotcloneoptionsstruct), also reclaims host disk space for blocks the guest filesystem has already freed. With [`SnapshotCloneOptions.RootDiskSizeMib`](#snapshotcloneoptionsstruct), grows the clone's root disk in place (grow-only). If `source` recorded content integrity, the new artifact's integrity is recomputed fresh.
...
<div className="msb-params">
  ...
  <div className="msb-param">
    <div className="msb-param-key"><code>opts</code><a className="msb-type" href="#snapshotcloneoptionsstruct">SnapshotCloneOptions</a></div>
    <div className="msb-param-desc">Destination directory, group, labels, force, sparsification, and root disk size.</div>
  </div>
</div>
...
```go
slim, err := m.Snapshot.Clone(ctx, "bloated", "slim", m.SnapshotCloneOptions{
    Sparsify:        true,
...
### SnapshotCloneOptions<span className="msb-tag is-type" style={{marginLeft: "8px"}}>struct</span>

<p className="msb-backref">Accepted by <a href="#snapshot-clone">Snapshot.Clone()</a></p>

Configures [`Snapshot.Clone`](#snapshot-clone). All fields are optional; a zero value clones into the default snapshots directory without sparsifying or resizing.

| Field | Type | Description |
|-------|------|-------------|
| DestDir | `string` | Parent directory for the new artifact; empty = the default snapshots directory |
| Group | `string` | Snapshot group to create the clone in; empty creates a group named after the new member |
| Labels | `map[string]string` | Labels for the new snapshot. Not inherited from the source |
...
| RootDiskSizeMib | `uint32` | Grow the clone's ext4 filesystem to this size in MiB, offline, before recording the artifact. Zero leaves the size unchanged. Grow-only: a target at or below the source's current size errors |
