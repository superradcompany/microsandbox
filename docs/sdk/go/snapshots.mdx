...
Use `WithRestoreConfig(RestoreConfig{...})` for explicit `Volumes`, `CapturedVolumes`, `Ports`, `Vsock`, `User`, and `LogLevel`. Fresh-boot options are not accepted. Host resources remain unmapped unless explicitly supplied or `WithDangerouslyInheritResources()` is selected; `WithExternalMountPolicy` validates supplied filesystem mappings without granting access.
