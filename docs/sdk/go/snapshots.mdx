...
| Compaction | `SnapshotCompaction` | Flat-snapshot compaction. The zero value inherits `snapshot_defaults.compaction` (built-in `off`); explicit values are `SnapshotCompactionOff`, `SnapshotCompactionAuto`, or `SnapshotCompactionOn`. |
| Resumable | `bool` | Request a resumable snapshot; returns an unsupported-feature error today |

The created artifact's `Compaction()` method returns `*SnapshotCompactionInfo`, including the requested policy, resolved status, journal replay flag, validated free bytes, physically deallocated bytes, and range count. It returns `nil` for artifacts created by older producers.
