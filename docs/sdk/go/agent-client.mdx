---
title: Agent client
"og:title": Go SDK Agent client reference
description: Send requests to the sandbox guest agent with the Go SDK.
keywords: ["Go SDK", "Go agent client", "microsandbox agent client"]
---
...
Resolve a sandbox's `agentd` relay socket path **without connecting**. Returns the same path [`ConnectAgentSandbox()`](#m-connectagentsandbox) would dial internally. It prefers the hashed path and falls back to the legacy name-derived path. Use it to talk to `agentd` over a raw byte transport instead of this frame client, for example a transparent relay that splices bytes between a WebSocket and the socket. The sandbox need not be running; the path is derived from the name and the configured home directory.
