...
`create()` boots the sandbox without executing its image workload. `exec_default()` resolves the effective OCI ENTRYPOINT and CMD, then runs it through the guest agent. Use `exec_default_with(...)` for per-call options, `exec_default_stream()` / `exec_default_stream_with(...)` for streaming, and `attach_default()` / `attach_default_with(...)` for a PTY. These methods return `MicrosandboxError::NoDefaultCommand` when no executable default exists; they never fall back to a shell.
...
let out = sb.exec_default_with(|e| e.env("LOG_LEVEL", "debug")).await?;
```
## Sandbox
