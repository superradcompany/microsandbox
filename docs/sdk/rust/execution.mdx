...
keywords: ["Rust SDK", "Rust execution", "microsandbox execution"]
---
...
Run a command through the sandbox's configured shell (default: `/bin/sh`, set via [`SandboxBuilder::shell()`](/sdk/rust/sandbox#sandbox-shell)). The script is run as `<shell> -c "<script>"`, so shell syntax like pipes, redirects, and `&&` chains works.
...
Streaming execution with per-execution overrides. Enable [`stdin_pipe()`](#exec_options-stdin_pipe) to write to the process's stdin via [`ExecHandle::take_stdin()`](#exechandle), and [`tty(true)`](#exec_options-tty) to allocate a pseudo-terminal for interactive programs like shells, REPLs, or editors.
...
Attach to the sandbox's default shell (configured via [`SandboxBuilder::shell()`](/sdk/rust/sandbox#sandbox-shell), default `/bin/sh`) with an interactive PTY session.
...
#### <span className="msb-recv">exec_options.</span><span className="msb-hn">arg()</span>
...
#### <span className="msb-recv">exec_options.</span><span className="msb-hn">args()</span>
...
#### <span className="msb-recv">exec_options.</span><span className="msb-hn">cwd()</span>
...
#### <span className="msb-recv">exec_options.</span><span className="msb-hn">user()</span>
...
#### <span className="msb-recv">exec_options.</span><span className="msb-hn">env()</span>
...
#### <span className="msb-recv">exec_options.</span><span className="msb-hn">envs()</span>
...
#### <span className="msb-recv">exec_options.</span><span className="msb-hn">timeout()</span>
...
#### <span className="msb-recv">exec_options.</span><span className="msb-hn">capture()</span>

```rust
fn capture(self, enabled: bool) -> Self
```

Record this command's output to the sandbox's `exec.log`, where `logs()` reads it. Default: `false`. An ad-hoc exec's output, a shell's above all, can carry anything typed or printed, so it is written to the host's disk only when asked. The sandbox's workload (`exec_default`, `attach_default`) is recorded without asking; pass `.capture(false)` there to opt out. The attach options have the same method. No effect on a sandbox built with `disable_exec_log()`. An explicit `.capture(false)` fails on a sandbox whose runtime predates this behaviour, since that runtime records every session; leaving `capture` unset does not.
...
<div className="msb-params">
  <div className="msb-param">
    <div className="msb-param-key"><code>enabled</code><span className="msb-type">bool</span></div>
    <div className="msb-param-desc"><code>true</code> to record this session's output.</div>
  </div>
</div>

#### <span className="msb-recv">exec_options.</span><span className="msb-hn">stdin_null()</span>
...
#### <span className="msb-recv">exec_options.</span><span className="msb-hn">stdin_pipe()</span>
...
#### <span className="msb-recv">exec_options.</span><span className="msb-hn">stdin_bytes()</span>
...
#### <span className="msb-recv">exec_options.</span><span className="msb-hn">rlimit()</span>
...
#### <span className="msb-recv">exec_options.</span><span className="msb-hn">rlimit_range()</span>

```rust
fn rlimit_range(self, resource: RlimitResource, soft: u64, hard: u64) -> Self
```

Set a resource limit with different soft and hard values. [`build()`](#exec_options-build) errors if `soft > hard`.
...
#### <span className="msb-recv">attach_options.</span><span className="msb-hn">arg()</span>
...
#### <span className="msb-recv">attach_options.</span><span className="msb-hn">args()</span>
...
#### <span className="msb-recv">attach_options.</span><span className="msb-hn">cwd()</span>
...
#### <span className="msb-recv">attach_options.</span><span className="msb-hn">user()</span>
...
#### <span className="msb-recv">attach_options.</span><span className="msb-hn">env()</span>
...
#### <span className="msb-recv">attach_options.</span><span className="msb-hn">envs()</span>
...
#### <span className="msb-recv">attach_options.</span><span className="msb-hn">rlimit()</span>
...
#### <span className="msb-recv">attach_options.</span><span className="msb-hn">rlimit_range()</span>

```rust
fn rlimit_range(self, resource: RlimitResource, soft: u64, hard: u64) -> Self
```

Set a resource limit with different soft and hard values for the attached process. [`build()`](#attach_options-build) errors if `soft > hard`.
...
#### <span className="msb-recv">h.</span><span className="msb-hn">control()</span>
...
#### <span className="msb-recv">h.</span><span className="msb-hn">take_stdin()</span>
...
#### <span className="msb-recv">control.</span><span className="msb-hn">id()</span>
...
#### <span className="msb-recv">control.</span><span className="msb-hn">signal()</span>
...
#### <span className="msb-recv">control.</span><span className="msb-hn">kill()</span>
...
#### <span className="msb-recv">control.</span><span className="msb-hn">resize()</span>
...
A writer for sending data to a running process's stdin. Obtained via [`ExecHandle::take_stdin()`](#exechandle) after enabling [`stdin_pipe()`](#exec_options-stdin_pipe).
...
#### <span className="msb-recv">out.</span><span className="msb-hn">status()</span>
...
#### <span className="msb-recv">out.</span><span className="msb-hn">stdout_bytes()</span>
...
#### <span className="msb-recv">out.</span><span className="msb-hn">stderr_bytes()</span>
...
<p className="msb-backref">Configured via <a href="#rlimit">rlimit()</a> · <a href="#exec_options-rlimit_range">rlimit_range()</a></p>

A POSIX resource limit. Built indirectly by [`rlimit()`](#rlimit) and [`rlimit_range()`](#exec_options-rlimit_range) on the option builders; you rarely construct it by hand.
...
<p className="msb-backref">Used by <a href="#rlimit">rlimit()</a> · <a href="#exec_options-rlimit_range">rlimit_range()</a> · <a href="#attach_options-rlimit">AttachOptionsBuilder.rlimit()</a></p>
