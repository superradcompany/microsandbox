...
Delete a named volume and its contents from disk. Locally, `Volume::remove` deletes the database record first, then the directory, so an orphaned directory is easier to detect than an orphaned record. Fails with `VolumeNotFound` if the volume does not exist.
