...
#### <span className="msb-recv">.</span><span className="msb-hn">compaction()</span>
<div className="msb-tags"><span className="msb-tag is-builder">builder</span></div>

```rust
fn compaction(self, compaction: SnapshotCompaction) -> Self
```

Select flat-snapshot free-space compaction. `Off` is the built-in default and avoids the ext4 scan, `Auto` is best effort, and `On` requires a flat root and successful host deallocation. `snapshot_defaults.compaction` can change the local default; calling this method is an explicit per-snapshot override. The created [`Snapshot`](#instance-methods) exposes the resolved [`SnapshotCompactionInfo`](#snapshotcompactioninfo) through `snap.compaction()`.
...
Records the requested `SnapshotCompaction` policy, resolved status (`skipped`, `compacted`, `unsupported`, or `not_applicable`), whether the ext4 journal was replayed, validated free bytes, physically deallocated bytes, and submitted range count. `Snapshot::compaction()` returns `None` only for artifacts created by older producers.
