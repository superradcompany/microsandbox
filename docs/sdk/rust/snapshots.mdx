...
Unpack a snapshot archive (`.tar.zst` or `.tar`, detected from magic bytes) into the snapshots directory or `dest`. Any bundled image-cache entries land in the global cache, and everything found is registered in the index. Recorded integrity is verified as artifacts land. Returns a handle for the head snapshot.
