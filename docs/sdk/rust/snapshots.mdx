...
Request a resumable snapshot (disk plus VM state). The cloud backend translates
this to the checkpoint kind, but [`create()`](#snapshot_builder-create) currently fails with
`Unsupported`; checkpoint capture has not landed yet.
