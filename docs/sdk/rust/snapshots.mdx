...
Create disk snapshots of stopped sandboxes and resumable checkpoints of running sandboxes. See [Snapshots](/sandboxes/snapshots) for usage and lifecycle concepts.
...
Create an installed snapshot artifact atomically, then best-effort update the rebuildable local index. Disk mode captures a stopped or crashed sandbox; resumable mode captures a running sandbox's checkpoint closure. Most callers use the [builder](#snapshotbuilder)'s [`create()`](#create) instead of constructing a [`SnapshotConfig`](#snapshotconfig) by hand.
...
Capture a disk or resumable snapshot directly into an archive. The operation creates no installed snapshot directory and no index row.
...
Snapshot payload scope: [`SnapshotScope::Disk`](#snapshotscope) for a disk-only snapshot or `Resumable` for disk plus VM execution state.
...
| resumable | `bool` | Capture a running sandbox's resumable checkpoint instead of a stopped-sandbox disk snapshot |
...
| checkpoint | `Option<CheckpointVerifyStatus>` | Verified checkpoint root for resumable state |
