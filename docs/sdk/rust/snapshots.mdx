...
<Note>
  v1 is **disk-only and stopped-only**. Live snapshots and qcow2 backing chains are tracked as future work.
</Note>
...
```rust
fn from_snapshot(self, src: impl AsRef<str>) -> Self
```
...
|------|------|-------------|
| src | `impl AsRef<str>` | Bare name (resolved under `~/.microsandbox/snapshots/`) or filesystem path to an artifact directory |
...
|-------|------|-------------|
| `with_parents` | `bool` | Walk the parent chain and include each ancestor (no-op in v1) |
...
h.digest();           // &str — sha256:...
h.name();             // Option<&str>
h.parent_digest();    // Option<&str> — None in v1
h.image_ref();        // &str
...
v1 always emits `Raw`. The `Qcow2` variant is reserved for backing-chain checkpoints.
