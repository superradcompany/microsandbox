...
Clone `source` (path, name, or digest) into a new snapshot named `new_name`. Always writes a new artifact rather than mutating `source` in place: `source` and anything referencing it by digest are left untouched. With [`CloneOpts::compact`](#cloneopts), also reclaims host disk space for blocks the guest filesystem has already freed. With [`CloneOpts::root_disk_size_mib`](#cloneopts), grows the clone's root disk in place (grow-only). If `source` recorded content integrity, the new artifact's integrity is recomputed fresh — there's no stale-digest hazard, since it's a brand-new digest either way.
...
<div className="msb-params">
  ...
  <div className="msb-param">
    ...
    <div className="msb-param-desc">Destination directory, labels, force, compaction, and root disk size.</div>
  </div>
</div>
...
#### <span className="msb-recv">snapshot_builder.</span><span className="msb-hn">compact()</span>
<div className="msb-tags"><span className="msb-tag is-builder">builder</span></div>
...
Deallocate host storage for blocks the guest ext4 filesystem has already freed, before recording the artifact. Never changes guest-visible content — it only reclaims host disk space the guest no longer considers in use — and a compaction failure never fails snapshot creation.
...
| compact | `bool` | Deallocate host storage for blocks the guest ext4 filesystem has already freed, before recording the artifact |
...
Options for [`Snapshot::clone_snapshot()`](#snapshotclone_snapshot). Implements `Default`; `CloneOpts::default()` clones into the default snapshots directory without compacting or resizing.
...
| compact | `bool` | Deallocate host storage for blocks the guest ext4 filesystem has already freed, while cloning |
