...
Create a snapshot artifact from a stopped sandbox. Writes the `snapshot.json` descriptor, optional `metadata.json`, and the captured raw layer under `layers/` into the artifact directory atomically, then best-effort upserts a row into the local index. Index failures are logged but do not fail the call; the artifact is the source of truth. Most callers use the [builder](#snapshotbuilder)'s [`create()`](#create) instead of constructing a [`SnapshotConfig`](#snapshotconfig) by hand.
...
Capture a stopped sandbox directly into an archive. The operation creates no installed snapshot directory and no index row.
