...
Create disk snapshots of stopped sandboxes and full checkpoints of running sandboxes. See [Snapshots](/sandboxes/snapshots) for usage and lifecycle concepts.
...
Snapshot::save("checkpoint-b", Path::new("changes.tar.zst"), SaveOpts {
    since: Some("checkpoint-a".into()),
    ..Default::default()
}).await?;
Snapshot::load_with_base(Path::new("changes.tar.zst"), None, "checkpoint-a").await?;
```

`layers` counts the oldest physical layers including the base, excluding the writable head. Omit it to compact all sealed layers. `last_layers: Some(n)` is an alternative to `since`; they cannot be combined with each other or `with_parents`. For direct restore, use `Sandbox::builder("child").from_snapshot("changes.tar.zst").snapshot_base("checkpoint-a")`. See [disk-chain maintenance](/sandboxes/snapshots#export-changes-and-compact-a-disk-chain) for dependency and retention rules.
...
Create an installed snapshot artifact atomically, then best-effort update the rebuildable local index. Disk mode captures a stopped or crashed sandbox; full mode captures a running sandbox's checkpoint closure. Most callers use the [builder](#snapshotbuilder)'s [`create()`](#create) instead of constructing a [`SnapshotConfig`](#snapshotconfig) by hand.
...
`SandboxBuilder` setter. Create a sandbox from a snapshot artifact. Disk snapshots cold-boot; full snapshots resume captured execution. The snapshot already pins the image reference and digest, so this is mutually exclusive with [`image()`](/sdk/rust/sandbox#image) and [`image_with()`](/sdk/rust/sandbox#image_with). The artifact is structurally opened at [`create()`](/sdk/rust/sandbox#create) time; persistent payload integrity is checked only through explicit [`Snapshot::verify()`](#snap-verify).
...
<div className="msb-params">
  <div className="msb-param">
    ...
    <div className="msb-param-desc">Bare name resolved under the default snapshots directory, or a path to an artifact directory or archive file.</div>
  </div>
</div>

#### <span className="msb-recv">sandbox.</span><span className="msb-hn">disk_only()</span>

```rust
fn disk_only(self) -> Self
```

Cold-boot only the disk state carried by a full snapshot. Chain after [`from_snapshot()`](#from_snapshot). Without it, a full snapshot resumes its captured execution and does not run the image CMD.
...
| full | `bool` | Capture a running sandbox's full checkpoint instead of a stopped-sandbox disk snapshot |
