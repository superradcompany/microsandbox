...
Select flat-snapshot free-space compaction. `Off` is the default and avoids the ext4 scan, `Auto` is best effort, and `On` requires a flat root and successful host deallocation. The created [`Snapshot`](#instance-methods) exposes the resolved [`SnapshotCompactionInfo`](#snapshotcompactioninfo) through `snap.compaction()`.
