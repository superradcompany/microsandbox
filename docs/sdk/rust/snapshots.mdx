...
<Note>
  ...
  crashed. `list_dir`, `reindex`, `save`/`save_to`, `copy_to`, `load`, and `verify` remain
...
Request a future resumable snapshot (disk plus VM state).
[`create()`](#snapshot_builder-create)
