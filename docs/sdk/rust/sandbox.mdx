...
keywords: ["Rust SDK", "Rust sandbox", "microsandbox sandbox"]
---
...
### External mount restore

Full restore requires captured external filesystems and additional disks by default. Supply bindings or explicitly request validated source-local inheritance; `.allow_missing_resources()` instead accepts unavailable resources with warnings. `ExternalMountRestorePolicy::Strict` validates supplied objects; `.external_mount_policy(ExternalMountRestorePolicy::Relaxed)` accepts supported object mismatches without waiving required bindings. Direct branches retain their existing omission behavior. `sandbox.restore_warnings().await?` reports unavailable filesystems and accepted mismatches through `guest_path`, `reason`, and `stale_inodes`. See [Snapshot external mounts](/sandboxes/snapshots#external-mounts).

### Creation and restore progress

Use `create_with_progress()` for fresh creation and `restore_with_progress()` for snapshots. Progress covers image preparation, snapshot RAM backing preparation, and activation. Await the task for the result; ignoring or dropping progress does not cancel the operation.

```rust
let (mut progress, task) = Sandbox::restore("saved")
    .name("child")
    .forked()
...
Local startup events distinguish waiting for backing, preparing it, reusing it, syncing, and activating. Byte totals cover required data slices, not reserved RAM. Lock waits and syncing have no percentage. Preparation has no fixed deadline; the activation timeout begins afterward. Abort the creation task to cancel, or wrap the operation in your own overall deadline. `create_detached_with_progress()` provides the same events for detached creation. Existing pull-only APIs remain available.
...
// Wait for the guest to converge.
let settled = sb.wait_until_resized_with_timeout(Duration::from_secs(60)).await?;
```

</Accordion>

Plan or apply a configuration change. Set what to change (CPUs, memory, env vars, labels, workdir, secrets), then call [`dry_run()`](#modification-dry_run) to preview or [`apply()`](#modification-apply) to commit. Both return a [`SandboxModificationPlan`](#sandboxmodificationplan) labeling each change `live`, `next start`, `requires restart`, or `unsupported`. Apply is all-or-nothing.

CPU and memory resize live within the [`max_cpus`](#modification-max_cpus) / [`max_memory`](#modification-max_memory) ceilings; raising a ceiling requires a restart. Env and workdir changes affect future execs only. On a stopped sandbox, changes are saved for the next boot.
...
<Tooltip tip="On Cloud, the timeout only limits how long the SDK waits; the accepted stop may continue server-side."><span className="msb-badge-limited">Different on cloud <Icon icon="circle-info" size={11} /></span></Tooltip>
...
Request graceful shutdown and wait indefinitely for the targeted run to become terminal and release runtime ownership. Success allows immediate removal without racing teardown. Cancelling the wait does not kill the sandbox or undo a delivered request. Paused or unreachable sandboxes return an error; resume explicitly or choose `kill()` when force termination is intended.
...
Apply one total budget to graceful dispatch and completion. Expiry returns `MicrosandboxError::StopTimeout` without killing; `Duration::ZERO` expires before dispatch. A shutdown request already sent may complete later. Only explicit `kill()` requests force termination.
...
Stop the sandbox and wait for runtime completion. An owned child-process handle supplies its actual exit status. Detached or connected handles retain the existing synthetic success status; use [`stop()`](#sb-stop) or [`stop_with_timeout()`](#sb-stop_with_timeout) when you only need completion.
...
<div className="msb-params">
  <div className="msb-param">
    ...
    <div className="msb-param-desc">Observed terminal sandbox state.</div>
  </div>
</div>

#### <span className="msb-recv">sb.</span><span className="msb-hn">resize_status()</span>

<Tooltip tip="Live resize is not available on microsandbox cloud."><span className="msb-badge-local">Local-only <Icon icon="circle-info" size={11} /></span></Tooltip>

```rust
async fn resize_status(&self) -> MicrosandboxResult<Vec<ResourceResizeStatus>>
```

Read the current live CPU and memory resize status without changing anything. Returns an empty list when the sandbox is not running.

<p className="msb-label">Returns</p>

<div className="msb-params">
  <div className="msb-param">
    <div className="msb-param-key"><a className="msb-type" href="#resourceresizestatus">Vec&lt;ResourceResizeStatus&gt;</a></div>
    <div className="msb-param-desc">One entry per resizable resource.</div>
  </div>
</div>

#### <span className="msb-recv">sb.</span><span className="msb-hn">wait_until_resized()</span>

<Tooltip tip="Live resize is not available on microsandbox cloud."><span className="msb-badge-local">Local-only <Icon icon="circle-info" size={11} /></span></Tooltip>

```rust
async fn wait_until_resized(&self) -> MicrosandboxResult<Vec<ResourceResizeStatus>>
```

Wait until every resource reaches `Applied`, `GuestRefused`, or `Failed`. Returns an empty list when the sandbox is not running. There is no deadline; a guest that never converges keeps this polling, so use [`wait_until_resized_with_timeout()`](#sb-wait_until_resized_with_timeout) to bound it.
...
<div className="msb-params">
  <div className="msb-param">
    <div className="msb-param-key"><a className="msb-type" href="#resourceresizestatus">Vec&lt;ResourceResizeStatus&gt;</a></div>
    <div className="msb-param-desc">The settled status.</div>
  </div>
</div>

#### <span className="msb-recv">sb.</span><span className="msb-hn">wait_until_resized_with_timeout()</span>

<Tooltip tip="Live resize is not available on microsandbox cloud."><span className="msb-badge-local">Local-only <Icon icon="circle-info" size={11} /></span></Tooltip>

```rust
async fn wait_until_resized_with_timeout(&self, timeout: Duration) -> MicrosandboxResult<Vec<ResourceResizeStatus>>
```

Same as [`wait_until_resized()`](#sb-wait_until_resized) within `timeout`. `Duration::ZERO` checks once. Expiry returns `MicrosandboxError::ResizeTimeout` with the last observed status, which is empty if no read completed before the deadline. The host still enforces the new limit.

#### <span className="msb-recv">sb.</span><span className="msb-hn">wait_for_status()</span>
...
#### <span className="msb-recv">sb.</span><span className="msb-hn">restart_with()</span>
...
Gracefully stop and remove this exact sandbox. Unlike [`remove_persisted()`](#sb-remove_persisted), `destroy` accepts a running sandbox, waits for it to shut down, and then deletes its persisted state.
...
Materialize the [`SandboxConfig`](#sandboxconfig) without pulling an image or booting the sandbox. Validates fresh-boot configuration. Use `Sandbox::restore()` for snapshots. For booting, use [`create`](#sandbox-create) instead; call [`detached(true)`](#sandbox-detached) first for background mode. Local `create()` keeps patches until image metadata is available. Cloud `create()` layers settings before sending the request; the cloud worker resolves image metadata.
...
Apply sparse sandbox settings and create-time options after built-in and global defaults. Builder methods called after `overlay` take precedence over the patch; managed overrides are applied last.

The builder uses the active backend's cached configuration when `build()` or `create()` runs. Local creation stacks image defaults, global defaults, CLI/SDK patches, then managed overrides in one resolver before validation. Omitted fields inherit lower layers; explicit nullable clears survive image defaults. Cloud creation also applies user and managed settings, then rejects unsupported options before sending the request.

`build()` returns a concrete `SandboxConfig` without pulling image metadata. On local creation, `workdir: None` inherits global defaults, then image defaults if global workdir is omitted. A concrete config cannot distinguish omission from an explicit clear; use direct builder `create()` to preserve sparse clears through image resolution. Managed overrides apply last in both paths.

`SandboxConfigPatch` patches the SDK configuration. Its `spec` field is a `SandboxSpecPatch` for shared settings such as image, resources, and networking. Create-time options, including registry credentials, replacement settings, and the requested slug, live alongside `spec`.

<Accordion title="Example">

```rust
use microsandbox::{
    EnvVar, Sandbox, SandboxConfigPatch, SandboxResourcesPatch, SandboxSpecPatch,
};
...
Patch fields preserve the visibility of their source configuration fields, so ordinary fields and nested patches can be updated directly:

```rust
let mut patch = SandboxConfigPatch::new();
patch.spec.resources.cpus = Some(2);
patch.spec.runtime.cmd = Some(vec!["python".into(), "app.py".into()]);
patch.spec.runtime.workdir = Some(None); // Explicitly clear the working directory.
patch.registry_auth = Some(None); // Explicitly clear per-create credentials.
```

For nullable fields, `None` omits the update, `Some(None)` clears the value, and `Some(Some(value))` sets it.

Mergeable collections keep their merge/replacement state private and expose `get_<field>()` for borrowing their pending values. Ordinary fields have no generated getters. The collections’ `get_<field>_mut()` accessors edit pending values without cloning, initializing an absent collection empty in merge mode and preserving any existing replacement mode:

```rust
patch.spec.get_env_mut().push(EnvVar::new("REGION", "us-east"));
patch.spec.get_labels_mut().insert("team".into(), "platform".into());
```

These accessors edit pending values without deduplicating them. Use `env_mut(...)` to merge entries by name immediately, or `replace_env_mut(...)` to replace the collection. Fluent setters, replacements, clears, and `overlay` remain available, along with their `_mut` variants returning `&mut Self`.

Use fluent setters to construct patches and `if let` for optional inputs. Once all overlays are composed, `into_config()` applies the patch to the type's defaults:

```rust
use microsandbox::{SandboxConfigPatch, SandboxResourcesPatch, SandboxSpecPatch};
...
let config = SandboxConfigPatch::new()
    .spec(
        SandboxSpecPatch::new()
            .name("example".into())
            .resources(resources),
...
`into_config()` does not load configuration files, consult a backend, or validate the result. Keep patches for `SandboxBuilder::overlay()` when creating a sandbox so backend and managed settings can still be layered. Use `apply_to(&mut config)` when updating an existing value.

Both patches contain typed inputs. YAML loading, relative-path resolution, `${ENV}` expansion, and snapshot lookup remain CLI or builder responsibilities. Runtime metadata stays internal, and create-time credentials and flags retain their existing serialization exclusions.

<Accordion title="Migrating from the earlier patch types">

The shared patch previously named `SandboxConfigPatch` is now `SandboxSpecPatch`. To use it with the builder, wrap it with `SandboxConfigPatch::new().spec(spec_patch)`. Update direct field access such as `patch.resources` to `patch.spec.resources` when using the SDK patch. Existing configuration files and serialized sandbox specifications keep their shape.

The generated patch model also replaces the handwritten patch hierarchy from microsandbox 0.6.15 and earlier:
...
Closure-based patch modifiers have been removed. Update Rust call sites to use direct field access or mutable collection accessors instead.

Filesystem, script, image, and init values use `SandboxSpecPatch`, nested under `SandboxConfigPatch.spec` in the SDK. `LocalConfig` remains available as a deprecated alias of `GlobalConfig`.
...
#### <span className="msb-recv">sandbox.</span><span className="msb-hn">max_cpus()</span>
...
#### <span className="msb-recv">sandbox.</span><span className="msb-hn">create_detached()</span>
...
Boot the sandbox in detached mode. This is a compatibility helper for `.detached(true).create()`. Prefer [`detached(true)`](#sandbox-detached) with [`create()`](#sandbox-create) for new code so attached and detached creation use the same flow.
...
#### <span className="msb-recv">sandbox.</span><span className="msb-hn">disable_network()</span>
...
#### <span className="msb-recv">sandbox.</span><span className="msb-hn">idle_timeout()</span>
...
#### <span className="msb-recv">sandbox.</span><span className="msb-hn">init_with()</span>
...
Like [`init`](#sandbox-init), but with a closure-builder for argv and env vars. Mirrors `exec_with` in shape. The builder exposes `arg`, `args`, `env`, and `envs`. Calling `init` or `init_with` more than once overwrites, unlike `env`, which appends. The init is one-shot pre-boot.
...
#### <span className="msb-recv">sandbox.</span><span className="msb-hn">image_with()</span>
...
#### <span className="msb-recv">sandbox.</span><span className="msb-hn">log_level()</span>
...
#### <span className="msb-recv">sandbox.</span><span className="msb-hn">max_duration()</span>

```rust
fn max_duration(self, secs: u64) -> Self
```

Set the maximum sandbox lifetime in seconds. When exceeded, the host drains and stops the sandbox. Enforced on the host side; the guest cannot override it.
...
#### <span className="msb-recv">sandbox.</span><span className="msb-hn">max_memory()</span>
...
#### <span className="msb-recv">sandbox.</span><span className="msb-hn">proxy()</span>
...
Configure the single proxy used for outbound sandbox connections. See [Proxies](/sdk/rust/proxies) for the protocol builders and examples.
...
#### <span className="msb-recv">sandbox.</span><span className="msb-hn">port_bind()</span>
...
#### <span className="msb-recv">sandbox.</span><span className="msb-hn">port_udp()</span>
...
#### <span className="msb-recv">sandbox.</span><span className="msb-hn">port_udp_bind()</span>
...
#### <span className="msb-recv">sandbox.</span><span className="msb-hn">pull_policy()</span>
...
Shorthand for adding a header-injected secret. Equivalent to `.secret(|s| s.env(env_var).value(value).allow(allowed_host))`.
...
Apply the changes. Live changes are made to the running sandbox first, and the new config is saved only after they succeed, so a failed apply leaves the old config in place. Changes for a stopped sandbox, or requested with [`next_start()`](#modification-next_start), are saved and take effect on the next start. With [`restart()`](#modification-restart), `apply()` stops and starts the sandbox so that restart-required changes take effect.

A live CPU or memory resize can take a moment to settle. The returned plan's `resize_status` is a single reading taken when the change was accepted; see [`ResourceResizeStatus`](#resourceresizestatus). Use [`wait_until_resized()`](#sb-wait_until_resized) to wait for the guest.
...
#### <span className="msb-recv">modification.</span><span className="msb-hn">max_cpus()</span>
...
#### <span className="msb-recv">modification.</span><span className="msb-hn">dry_run()</span>
...
#### <span className="msb-recv">modification.</span><span className="msb-hn">remove_env()</span>

```rust
fn remove_env(self, key: impl Into<String>) -> Self
```

Remove an environment variable. Same future-execs-only semantics as [`env()`](#modification-env).
...
#### <span className="msb-recv">modification.</span><span className="msb-hn">remove_label()</span>
...
#### <span className="msb-recv">modification.</span><span className="msb-hn">memory_mib()</span>

```rust
fn memory_mib(self, memory_mib: u32) -> Self
```

Set the desired effective guest memory in MiB. Same as [`memory()`](#modification-memory) with an explicit unit.
...
#### <span className="msb-recv">modification.</span><span className="msb-hn">max_memory()</span>
...
#### <span className="msb-recv">modification.</span><span className="msb-hn">max_memory_mib()</span>
...
Set the desired boot-time maximum hotpluggable memory in MiB. Same as [`max_memory()`](#modification-max_memory) with an explicit unit.
...
#### <span className="msb-recv">modification.</span><span className="msb-hn">next_start()</span>
...
Plan under restart-backed apply semantics. When the patch contains restart-required changes, [`apply()`](#modification-apply) stops the sandbox, persists the config, and starts it again so the changes become active now.
...
Declare the desired state of one secret via a [`SecretPatchBuilder`](#secretpatchbuilder) closure. The spec mirrors the create-time [`SecretBuilder`](/sdk/rust/secrets#secretbuilder) vocabulary. The planner diffs it against the existing config to infer the change: a secret that does not exist yet is `added`, material on an existing secret `rotated`, and host or placeholder differences update those aspects. Declaring the same secret again replaces the earlier spec; removal is always explicit through [`remove_secret()`](#modification-remove_secret).
...
#### <span className="msb-recv">modification.</span><span className="msb-hn">remove_secret()</span>
...
<p className="msb-backref">Used by <a href="#sandbox-registry">registry()</a></p>

Builder passed to [`registry()`](#sandbox-registry) for per-sandbox registry connection settings.
...
#### <span className="msb-recv">registry.</span><span className="msb-hn">ca_certs()</span>
...
Fluent configuration passed to [`Sandbox::list_with()`](#sandbox-list_with). Keep the labels and limit unchanged when continuing with a cursor.
...
<p className="msb-backref">Returned by <a href="#sandbox-get">Sandbox::get()</a> · <a href="#sandbox-list">Sandbox::list()</a></p>
...
#### <span className="msb-recv">h.</span><span className="msb-hn">config_json()</span>
...
#### <span className="msb-recv">h.</span><span className="msb-hn">connect_with_timeout()</span>
...
#### <span className="msb-recv">h.</span><span className="msb-hn">connect_or_start()</span>
...
Behaves like [`connect_or_start()`](#h-connect_or_start), but starts a local stopped sandbox in detached/background mode. Connecting to an already-running sandbox never changes its ownership.
...
#### <span className="msb-recv">h.</span><span className="msb-hn">kill_with_timeout()</span>
...
#### <span className="msb-recv">h.</span><span className="msb-hn">request_drain()</span>
...
#### <span className="msb-recv">h.</span><span className="msb-hn">request_kill()</span>
...
#### <span className="msb-recv">h.</span><span className="msb-hn">request_stop()</span>
...
#### <span className="msb-recv">h.</span><span className="msb-hn">start_detached()</span>
...
#### <span className="msb-recv">h.</span><span className="msb-hn">wait_for_status()</span>
...
#### <span className="msb-recv">h.</span><span className="msb-hn">restart_with()</span>

```rust
async fn restart_with(&self, options: RestartOptions) -> MicrosandboxResult<Sandbox>
```

Restart with explicit force, shutdown timeout, and detached-start controls. See [`sb.restart_with()`](#sb-restart_with) for state behavior and defaults.
...
#### <span className="msb-recv">h.</span><span className="msb-hn">destroy_with()</span>

```rust
async fn destroy_with(&self, options: DestroyOptions) -> MicrosandboxResult<()>
```

Destroy with explicit force and shutdown-timeout controls. See [`sb.destroy_with()`](#sb-destroy_with) for defaults.
...
Wait indefinitely for graceful completion and release of the same runtime's ownership. No implicit kill; cancellation stops only the wait.

<p className="msb-label">Returns</p>

`Result<()>`

#### <span className="msb-recv">h.</span><span className="msb-hn">stop_with_timeout()</span>
...
Same completion contract as `stop()` under one total budget. Expiry returns `MicrosandboxError::StopTimeout` without killing; `Duration::ZERO` expires before dispatch.
...
#### <span className="msb-recv">h.</span><span className="msb-hn">updated_at()</span>
...
#### <span className="msb-recv">h.</span><span className="msb-hn">wait_until_stopped()</span>
...
#### <span className="msb-recv">h.</span><span className="msb-hn">resize_status()</span>
...
Current live CPU and memory resize status. Empty when not running.

<p className="msb-label">Returns</p>

`Result<Vec<`[`ResourceResizeStatus`](#resourceresizestatus)`>>`

#### <span className="msb-recv">h.</span><span className="msb-hn">wait_until_resized()</span>

<Tooltip tip="Live resize is not available on microsandbox cloud."><span className="msb-badge-local">Local-only <Icon icon="circle-info" size={11} /></span></Tooltip>

```rust
wait_until_resized()
wait_until_resized_with_timeout(timeout)
```

Wait until every live resize is terminal. Returns an empty list when the sandbox is not running. The untimed variant keeps polling while a guest never converges; the timed variant returns `MicrosandboxError::ResizeTimeout` on expiry.
...
#### <span className="msb-recv">secret.</span><span className="msb-hn" id="secretpatchbuilder-allow">allow()</span>

```rust
fn allow(self, host: impl Into<String>) -> Self
```
...
#### <span className="msb-recv">secret.</span><span className="msb-hn" id="secretpatchbuilder-substitution">substitution()</span>

```rust
fn substitution(self, value: SecretSubstitution) -> Self
```

Replace the enabled request locations for this secret. Omit this method to leave existing settings unchanged. At least one location must remain enabled. See [SecretSubstitution](/sdk/rust/secrets#secretsubstitution) for defaults and protocol limits.

<span id="secretpatchbuilder-allow_passthrough_for" />

#### <span className="msb-recv">secret.</span><span className="msb-hn" id="secretpatchbuilder-allow_placeholder_for">allow_placeholder_for()</span>

```rust
fn allow_placeholder_for(self, host: impl Into<String>) -> Self
```

Add a destination allowed to receive the unchanged placeholder where substitution does not apply. `allow_passthrough_for()` remains a deprecated alias. This does not grant access to the real secret.

#### <span className="msb-recv">secret.</span><span className="msb-hn" id="secretpatchbuilder-violation_action">violation_action()</span>

```rust
fn violation_action(self, value: SecretViolationAction) -> Self
```

Set the per-secret blocking action. See [SecretViolationAction](/sdk/rust/secrets#secretviolationaction).

#### <span className="msb-recv">secret.</span><span className="msb-hn" id="secretpatchbuilder-require_tls_identity">require_tls_identity()</span>

```rust
fn require_tls_identity(self, value: bool) -> Self
```

Set whether substitution requires a verified TLS identity. Omit this method to retain the existing setting.
...
<p className="msb-backref">Used by <a href="#sandbox-log_level">log_level()</a></p>
...
<p className="msb-backref">Used by <a href="#sandbox-pull_policy">pull_policy()</a></p>
...
<p className="msb-backref">Returned by <a href="#sb-config">config()</a> · <a href="#sandbox-build">build()</a></p>
...
<p className="msb-backref">Returned by <a href="#modification-dry_run">dry_run()</a> · <a href="#modification-apply">apply()</a></p>
...
| `RequiresRestart` (`"requires restart"`) | Needs a restart before it can take effect; `apply()` refuses it unless the [`restart()`](#modification-restart) policy is selected |
...
| `Applied` | Desired, actual, and enforced state match. A shrink is applied only after the guest releases the CPUs or memory |
| `GuestRefused` | The guest would not cooperate; the host enforces the new limit anyway |
| `Failed` | The resize failed |

`Applied`, `GuestRefused`, and `Failed` are terminal; [`wait_until_resized()`](#sb-wait_until_resized) returns once every resource reaches one of them.
