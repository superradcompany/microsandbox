...
```rust
fn libkrunfw_path(self, path: impl Into<PathBuf>) -> Self
```

Override the host-side `libkrunfw` shared library used to boot this sandbox. Use this for custom kernel/firmware development; by default microsandbox resolves `libkrunfw` from the global config, next to the `msb` binary, or under `~/.microsandbox/lib/`.

The path is a host path, not a guest path. It must point to the platform shared library (`.so` on Linux, `.dylib` on macOS) and is validated when [`build`](#build) runs.

**Parameters**

| Name | Type | Description |
|------|------|-------------|
| path | `impl Into<PathBuf>` | Host path to the custom `libkrunfw` shared library |
...
| libkrunfw_path | `Option<PathBuf>` | Per-sandbox custom `libkrunfw` path. Host-side and not persisted |
