...
```rust
fn init(self, program: impl Into<PathBuf>) -> Self
```

Hand off PID 1 inside the guest to `program` after agentd finishes its boot-time setup. The agent forks; the parent execs the init and becomes PID 1, the agent continues as a child process. See [Init handoff](/sandboxes/customization#init-handoff) for image picks, shutdown semantics, and tradeoffs.

`program` is either an absolute path inside the guest rootfs or the literal `"auto"` (probes `/sbin/init`, `/lib/systemd/systemd`, `/usr/lib/systemd/systemd`). For init binaries that take argv or extra env (rare), use [`init_with`](#init_with).

**Parameters**

| Name | Type | Description |
|------|------|-------------|
| program | `impl Into<PathBuf>` | Absolute path inside the guest, or `"auto"` |
...
| Name | Type | Description |
|------|------|-------------|
| program | `impl Into<PathBuf>` | Absolute path to the init binary inside the guest |
