...
`program` must be an absolute path inside the guest rootfs. Pass an empty array for `args` if the init takes no extra argv.
...
| args | `impl IntoIterator<Item = impl Into<String>>` | Supplemental argv (`argv[0]` is implicitly `program`) |
