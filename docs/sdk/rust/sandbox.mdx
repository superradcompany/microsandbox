...
<div className="msb-glance">
  ...
  <a className="msb-row" href="#sb-touch"><span className="msb-rn">sb.touch()</span><span className="msb-rg">refresh idle timer</span></a>
  ...
  <div className="msb-chiprow">
    ...
    <a className="msb-chip" href="#max_memory">.max_memory()</a>
    <a className="msb-chip" href="#cpus">.cpus()</a>
    <a className="msb-chip" href="#max_cpus">.max_cpus()</a>
...
Check whether the running sandbox's guest agent is reachable. This sends `core.ping`, returns the SDK-measured round-trip latency, and does not refresh the sandbox idle timer. Experimental: requires `MSB_EXPERIMENTAL_MODIFY=1` in the environment.
...
Explicitly refresh the running sandbox's idle timer. This sends `core.touch`; use it when keeping an idle sandbox alive is intentional. Experimental: requires `MSB_EXPERIMENTAL_MODIFY=1` in the environment.
...
Set the boot-time maximum possible virtual CPU capacity. This reserves the envelope a sandbox can use after restart-backed changes and future live CPU activation; it does not increase the effective vCPU count by itself. Experimental: requires `MSB_EXPERIMENTAL_MODIFY=1` in the environment.
...
Set the boot-time maximum hotpluggable guest memory. This reserves the envelope a sandbox can use after restart-backed changes and future live memory activation; it does not increase the effective memory by itself. Experimental: requires `MSB_EXPERIMENTAL_MODIFY=1` in the environment.
