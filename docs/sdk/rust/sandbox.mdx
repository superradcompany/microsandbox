...
Wait until every resource reaches `Applied`, `GuestRefused`, or `Failed`. There is no deadline; a guest that never converges keeps this waiting.
...
Same as [`wait_until_resized()`](#sb-wait_until_resized) within `timeout`. `Duration::ZERO` checks once. Expiry returns `MicrosandboxError::ResizeTimeout` with the last observed status. The host still enforces the new limit.
...
Wait until every live resize is terminal. The timed variant returns `MicrosandboxError::ResizeTimeout` on expiry.
...
#### <span className="msb-recv">secret.</span><span className="msb-hn" id="secretpatchbuilder-allow_passthrough_for">allow_passthrough_for()</span>

```rust
fn allow_passthrough_for(self, host: impl Into<String>) -> Self
```

Add a destination allowed to receive the unchanged placeholder. This does not grant access to the real secret.
