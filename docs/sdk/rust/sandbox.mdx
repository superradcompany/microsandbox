...
Restore and branch leave host filesystems unmapped unless you supply bindings or explicitly request resource inheritance. `ExternalMountRestorePolicy::Strict` validates the authorized mappings by default; `.external_mount_policy(ExternalMountRestorePolicy::Relaxed)` accepts supported mismatches with warnings. Neither mode grants host access, and intentional omissions do not fail strict validation. `sandbox.restore_warnings().await?` reports unmapped filesystems and accepted mismatches through `guest_path`, `reason`, and `stale_inodes`. See [Snapshot external mounts](/sandboxes/snapshots#external-mounts).
