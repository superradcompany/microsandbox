...
Gracefully shut down the sandbox and wait until stopped state is observed. Local waits up to ten seconds, then force-kills the sandbox if it is still running. Cloud waits up to six minutes because shutdown may include creating a durable disk checkpoint. If that deadline expires, this returns `MicrosandboxError::SandboxStopTimedOut` without cancelling the accepted server-side stop, which may still complete afterward.
...
Gracefully shut down the sandbox with an explicit observation timeout. Local force-kills after the timeout. Cloud returns `MicrosandboxError::SandboxStopTimedOut` instead, while the accepted server-side stop may continue. `Duration::ZERO` requests immediate force termination and is therefore local-only.
...
booting, use [`create`](#sandbox-create) instead; call
[`detached(true)`](#sandbox-detached) first for background mode. `create()` calls
`build` internally.
...
Gracefully shut down and wait until stopped. Local waits up to ten seconds, then force-kills. Cloud waits up to six minutes and returns `SandboxStopTimedOut` on expiry without cancelling the accepted server-side stop.
...
Same as `stop()` with an explicit timeout. Local force-kills on expiry; Cloud returns `SandboxStopTimedOut` while the accepted stop may continue. `Duration::ZERO` requests immediate force termination and is local-only.
