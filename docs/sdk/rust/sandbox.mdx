---
title: Sandbox
"og:title": Rust SDK Sandbox reference
description: Create and manage microVM sandboxes with the Rust SDK. Configure resources, connect to running sandboxes, and control their lifecycle.
keywords: ["Rust SDK", "Rust sandbox", "microsandbox sandbox"]
---

Use the microsandbox Rust SDK to create a microVM sandbox for untrusted code, run commands, stream logs and metrics, and shut it down. Each sandbox has its own guest kernel. See [Overview](/sandboxes/overview) for configuration examples and [Lifecycle](/sandboxes/lifecycle) for state management.
...
#### <span className="msb-recv">sb.</span><span className="msb-hn">restore_warnings()</span>

```rust
async fn restore_warnings(&self) -> MicrosandboxResult<Vec<ExternalMountWarning>>
```

Return warnings for unavailable external filesystems and accepted restore mismatches. See [External mounts](/sandboxes/snapshots#external-mounts) for restore options.

Each `ExternalMountWarning` contains:

| Field | Type | Description |
|---|---|---|
| `guest_path` | `String` | Guest-visible mount path. |
| `reason` | `String` | Why the resource could not be reconstructed. |
...
Declare the desired state of one secret via a [`SecretPatchBuilder`](#secretpatchbuilder) closure. The spec mirrors the create-time [`SecretBuilder`](/sdk/rust/secrets#secretbuilder) vocabulary. The planner diffs it against the existing config to infer the change. A secret that does not exist yet is `added`, and material on an existing secret is `rotated`. Host or placeholder differences update those aspects. Declaring the same secret again replaces the earlier spec; removal is always explicit through [`remove_secret()`](#modification-remove_secret).
