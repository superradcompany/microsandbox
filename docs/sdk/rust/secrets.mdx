...
Control whether the placeholder is replaced in the HTTP request body. When enabled on HTTP/1 requests with a known `Content-Length` and no body content encoding, the TLS proxy buffers the request body, substitutes the placeholder, and updates the `Content-Length` header to match the new body size. Encoded or chunked bodies are preserved rather than rewritten blindly.
...
For intercepted HTTP/1 traffic, each request's `Host` header must match the TLS SNI, ignoring case and an optional port. A request that uses `SNI=front.example` but `Host: target.example` is blocked before it reaches upstream. This check is applied per request on keep-alive connections.
