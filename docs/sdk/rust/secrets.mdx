...
## Constructing configuration structs directly

`SecretsConfig` now includes `passthrough_hosts` to preserve global defaults from v0.6 configurations. This is a Rust source-compatibility change: existing literals that list every field must add the field or use `..Default::default()`.

```rust
use microsandbox_types::{SecretViolationAction, SecretsConfig};
...
Exhaustive destructuring must likewise include the new field or `..`. The shared `CloudSecretsConfig` struct has the same addition. Existing builder calls and literals already using `..Default::default()` need no changes.
