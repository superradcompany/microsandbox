...
#### <span className="msb-recv">secret.</span><span className="msb-hn">substitute_in_header_fields()</span>

```rust
fn substitute_in_header_fields(fields: impl IntoIterator<Item = impl Into<String>>) -> Self
```

Enable header substitution but restrict it to the given header field names (matched case-insensitively), e.g. only `Authorization`. A placeholder found in any other header field is blocked unless the destination matches the secret's passthrough list. Passing an empty list restores the default of substituting in every header field. Names must be valid HTTP field names.

Restricting this list matters for confidentiality, not just correctness: with every header substitutable, an untrusted guest can place the placeholder in a header the upstream host reflects back in its response and read the real secret from that response. Allow only the intended credential header.

<p className="msb-label">Parameters</p>

<div className="msb-params">
  <div className="msb-param">
    <div className="msb-param-key"><code>fields</code><span className="msb-type">IntoIterator&lt;Item = impl Into&lt;String&gt;&gt;</span></div>
    <div className="msb-param-desc">Header field names allowed to receive the substitution. Empty restores all headers.</div>
  </div>
</div>
...
| `header_fields` | `Vec<String>` | `[]` (all) | When non-empty, restrict header substitution to these field names. |
