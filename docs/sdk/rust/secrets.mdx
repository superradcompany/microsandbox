...
|---|---|---|
| <span id="entry-env_var"></span>`env_var` | `String` | Environment variable holding the placeholder. Non-empty, no `=` or NUL. |
| <span id="entry-value"></span>`value` | `Zeroizing<String>` | The real secret value; never enters the guest. Inline values are persisted; source-backed durable entries carry an empty value until host-side resolution. Debug output redacts the value. |
| <span id="entry-source"></span>`source` | `Option<SecretSource>` | Host-side reference resolved at spawn time. Prefer the builder's [`source()`](#secret-source) method rather than populating a resolved entry manually. |
| <span id="entry-placeholder"></span>`placeholder` | `String` | What the guest sees. Non-empty, at most 1024 bytes, no NUL/CR/LF. |
| <span id="entry-allowed_hosts"></span>`allowed_hosts` | `Vec<HostPattern>` | Hosts allowed to receive the real value. |
| <span id="entry-substitution"></span>`substitution` | [`SecretSubstitution`](#secretsubstitution) | Where in the request the value may be substituted. |
| <span id="entry-violation_action"></span>`violation_action` | `Option<SecretViolationAction>` | Per-secret violation override. `None` falls back to the sandbox-wide action. |
| <span id="entry-passthrough_hosts"></span>`passthrough_hosts` | `Vec<HostPattern>` | Destinations allowed to receive the unchanged placeholder. Empty by default. |
| <span id="entry-require_tls_identity"></span>`require_tls_identity` | `bool` | Require verified TLS identity before substituting. Default: `true`. |
