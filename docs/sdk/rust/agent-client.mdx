---
title: Agent client
"og:title": Rust SDK Agent client reference
description: Connect to the sandbox guest agent from Rust. Send requests and handle response streams through the low-level relay client.
keywords: ["Rust SDK", "Rust agent client", "microsandbox agent client"]
---
...
FFI-friendly bytes-in/bytes-out wrapper around [`AgentClient`](#agentclient).
