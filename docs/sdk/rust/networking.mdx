...
Configure a sandbox's network stack: a first-match-wins egress/ingress policy, published ports, DNS interception, TLS interception, rate limits, and secret-violation handling. See [Networking](/networking/overview) for the conceptual overview and [TLS Interception](/networking/tls) for proxy details.
...
#### <span className="msb-recv">.</span><span className="msb-hn">egress_rate_limiter()</span>

```rust
fn egress_rate_limiter(self, f: impl FnOnce(RateLimiterBuilder) -> RateLimiterBuilder) -> Self
```
...
.network(|n| n.egress_rate_limiter(|r| r
...
Limit outbound (egress) traffic. See [`RateLimiterBuilder`](#ratelimiterbuilder) for the bucket setters. Unset means unlimited. The limit applies on the next sandbox start; cloud sandboxes reject rate limiters as unsupported for now.

#### <span className="msb-recv">.</span><span className="msb-hn">ingress_rate_limiter()</span>

```rust
fn ingress_rate_limiter(self, f: impl FnOnce(RateLimiterBuilder) -> RateLimiterBuilder) -> Self
```

Limit inbound (ingress) traffic. Same shape as [`egress_rate_limiter()`](#egress_rate_limiter).
...
Builder for one direction's rate limiter, used in [`NetworkBuilder::egress_rate_limiter(|r| r...)`](#egress_rate_limiter) and [`ingress_rate_limiter(|r| r...)`](#ingress_rate_limiter). A limiter caps bandwidth (bytes) and packet rate (frames) independently; leaving a bucket unset leaves that dimension unlimited. Buckets start full plus their one-time burst and refill continuously. Every setter returns `Self`.
...
<p className="msb-backref">Built by <a href="#ratelimiterbuilder">RateLimiterBuilder</a> · used by <a href="#egress_rate_limiter">egress_rate_limiter()</a> · <a href="#ingress_rate_limiter">ingress_rate_limiter()</a></p>

Rate limiter for one traffic direction. A missing bucket leaves that dimension unlimited. Lives in `microsandbox_types` and is stored on the network config as `egress_rate_limiter` / `ingress_rate_limiter`.
