...
#### <span className="msb-recv">network.</span><span className="msb-hn">http_deny_message()</span>

```rust
fn http_deny_message(self, message: impl Into<String>) -> Self
```

Body returned to HTTP clients (and HTTPS clients on intercepted ports) when a domain rule denies the connection. The gateway answers `403 Forbidden` with this plain-text body instead of resetting the connection. `{host}` is replaced with the blocked hostname. Omit to use the default, which tells an agent the host is not on the allow list. See [What a denied HTTP request sees](/networking/overview#what-a-denied-http-request-sees).
...
<p className="msb-backref">Built by <a href="#violationactionbuilder">ViolationActionBuilder</a> · used by <a href="#network-on_secret_violation">on_secret_violation()</a></p>
