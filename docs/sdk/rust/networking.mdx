...
#### <span className="msb-recv">.</span><span className="msb-hn">tx_rate_limiter()</span>

```rust
fn tx_rate_limiter(self, f: impl FnOnce(RateLimiterBuilder) -> RateLimiterBuilder) -> Self
```
...
.network(|n| n.tx_rate_limiter(|r| r
...
Limit guest upload (egress) traffic. See [`RateLimiterBuilder`](#ratelimiterbuilder) for the bucket setters. Unset means unlimited. The limit applies on the next sandbox start; cloud sandboxes reject rate limiters as unsupported for now.

#### <span className="msb-recv">.</span><span className="msb-hn">rx_rate_limiter()</span>

```rust
fn rx_rate_limiter(self, f: impl FnOnce(RateLimiterBuilder) -> RateLimiterBuilder) -> Self
```

Limit guest download (ingress) traffic. Same shape as [`tx_rate_limiter()`](#tx_rate_limiter).
...
<p className="msb-backref">Built by <a href="#ratelimiterbuilder">RateLimiterBuilder</a> · used by <a href="#tx_rate_limiter">tx_rate_limiter()</a> · <a href="#rx_rate_limiter">rx_rate_limiter()</a></p>

Rate limiter for one traffic direction. A missing bucket leaves that dimension unlimited. Lives in `microsandbox_types` and is stored on the network config as `tx_rate_limiter` / `rx_rate_limiter`.
