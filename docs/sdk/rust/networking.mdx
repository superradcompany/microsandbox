---
title: Networking
"og:title": Rust SDK Networking reference
description: Configure sandbox network policies with the Rust SDK.
...
Create a [`NetworkPolicyBuilder`](#networkpolicybuilder) to configure rules and per-direction defaults. The first matching rule wins in each direction. [`build()`](#policy-build) validates string inputs (`.ip`, `.cidr`, `.domain`, `.domain_suffix`) and returns a [`BuildError`](#builderror) for invalid input.
...
Set the sandbox-wide blocking action. Passthrough hosts are configured per secret with `allow_passthrough_for()`.
...
Validation runs at `NetworkBuilder::build()`. Each of these surfaces as [`BuildError`](#builderror): a limiter with neither bucket, a zero bucket size or refill interval, a burst without its bucket, or a refill interval that does not fit in u64 milliseconds.
...
|---|---|---|
| <span id="ruledirection"></span>`direction` | [`Direction`](#direction) | Which evaluator considers this rule |
| <span id="ruledestination"></span>`destination` | [`Destination`](#destination) | Target filter |
| <span id="ruleprotocols"></span>`protocols` | [`Vec<Protocol>`](#protocol) | Set semantics; empty = any protocol |
| <span id="ruleports"></span>`ports` | [`Vec<PortRange>`](#portrange) | Set semantics; empty = any port |
| <span id="ruleaction"></span>`action` | [`Action`](#action) | What to do on match |
...
| Field | Type | Description |
|---|---|---|
| <span id="portrangestart"></span>`start` | `u16` | Start port (inclusive) |
| <span id="portrangeend"></span>`end` | `u16` | End port (inclusive) |
