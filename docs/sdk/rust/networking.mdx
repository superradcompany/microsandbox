...
description: Configure sandbox network policies, DNS, TLS interception, and published ports with the Rust SDK.
...
Create a [`NetworkPolicyBuilder`](#networkpolicybuilder) to configure rules and per-direction defaults. The first matching rule wins in each direction. String inputs (`.ip`, `.cidr`, `.domain`, `.domain_suffix`) are validated by [`build()`](#policy-build), which returns a [`BuildError`](#builderror) for invalid input.
