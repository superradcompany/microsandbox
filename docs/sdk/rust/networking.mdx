...
Network configuration is attached to a sandbox via `SandboxBuilder::network(|n| n...)`. Inside the closure you get a [`NetworkBuilder`](#networkbuilder) which exposes DNS, TLS, secret, and policy configuration.

```rust
use microsandbox::{NetworkPolicy, Sandbox};

let sb = Sandbox::builder("filtered")
    .image("alpine")
    .network(|n| n
...
For port publishing, use `SandboxBuilder::port(host, guest)` / `port_udp(host, guest)` (or the equivalent methods on `NetworkBuilder`).

## NetworkPolicy

A network access policy: two per-direction defaults plus an ordered list of rules evaluated first-match-wins per direction. Construct with [`NetworkPolicy::builder()`](#networkpolicybuilder).
...
`NetworkPolicy::default()` is `default_egress = Deny + implicit allow @ public`, `default_ingress = Allow + no rules`. This preserves the historical `public_only` egress behavior plus today's unfiltered published-port behavior. See the [defaults rationale](/networking/overview#defaults) for why the defaults are asymmetric.
...
NetworkPolicy::builder()
    ...
    .egress(|e| e
        ...
        .deny().ip("10.0.0.5"))          // never reached: shadowed by the line above
    .build()?;
```
...
At `.build()` time the builder walks the rule list and emits a `tracing::warn!` for any rule whose match set is fully covered by an earlier rule in a compatible direction. Coverage is checked for `Ip`, `Cidr`, and `Group` destinations. Domain shadowing depends on runtime DNS state and is intentionally out of scope.

The build still succeeds: a shadowed rule is a hint, not a hard error. Enable `tracing` to see the warnings:
...
Inside a `.rule()` / `.egress()` / `.ingress()` / `.any()` closure, state setters (`.tcp()`, `.port(N)`, etc.) accumulate eagerly. Each rule-adder commits a rule using the state at that point. State is **not reset** between rule-adders.
...
To reset state between rules, use separate `.rule()` / `.egress()` / `.ingress()` closures, or interleave fresh state setters between adders.
...
Match the `Public` group (complement of named categories: every IP not in any other group).

---

#### allow_private() / deny_private()
...
Match the `Private` group (RFC1918 + ULA + CGN).

---

#### allow_loopback() / deny_loopback()
...
Match the `Loopback` group (`127.0.0.0/8`, `::1`). The **guest's own** loopback, not the host. To reach a service on the host's localhost, use [`allow_host()`](#allow_host--deny_host) instead. See the [loopback-vs-host watch-out](/networking/overview#loopback-vs-host-a-common-trap).

---

#### allow_link_local() / deny_link_local()
...
Match the `LinkLocal` group (`169.254.0.0/16`, `fe80::/10`). Excludes the metadata IP `169.254.169.254`.

---

#### allow_meta() / deny_meta()
...
Match the `Metadata` group (`169.254.169.254`). **Dangerous on cloud hosts** (exposes IAM credentials).

---

#### allow_multicast() / deny_multicast()
...
Match the `Multicast` group (`224.0.0.0/4`, `ff00::/8`).
...
Match the `Host` group: per-sandbox gateway IPs that back `host.microsandbox.internal`. This is the right shortcut for "let the sandbox reach my host's localhost", not [`allow_loopback()`](#allow_loopback--deny_loopback).
...
Add one `Destination::Domain` rule per name.

---

#### allow_domain_suffixes() / deny_domain_suffixes()
...
Add one `Destination::DomainSuffix` rule per suffix.
...
Add three rules atomically: `Loopback + LinkLocal + Host`. Each uses the closure's current state. `Metadata` is intentionally not included; opt in via [`allow_meta()`](#allow_meta--deny_meta) separately.
