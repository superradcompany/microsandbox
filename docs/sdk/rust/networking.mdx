...
#### allow_secret_passthrough_host()

```rust
fn allow_secret_passthrough_host(self, host: impl Into<String>) -> Self
```

Allow any configured secret placeholder to pass through unchanged to an exact host. This does **not** allow the host to receive the real secret value; substitution still only happens for hosts configured with `SecretBuilder::allow_host()`.

---

#### allow_secret_passthrough_host_pattern()

```rust
fn allow_secret_passthrough_host_pattern(self, pattern: impl Into<String>) -> Self
```

Allow any configured secret placeholder to pass through unchanged to hosts matching a wildcard pattern (for example, `"*.anthropic.com"`). This does **not** allow those hosts to receive real secret values.
