...
keywords: ["Rust SDK", "Rust networking", "microsandbox networking"]
---
...
<p className="msb-backref">Built by <a href="#networkpolicybuilder">NetworkPolicy::builder()</a> · used by <a href="#network-policy">policy()</a></p>
...
Start the fluent [`NetworkPolicyBuilder`](#networkpolicybuilder). The primary construction path. String inputs (`.ip`, `.cidr`, `.domain`, `.domain_suffix`) are stored raw and parsed at [`build()`](#policy-build), so the chain stays clean and the first parse or validation failure surfaces as [`BuildError`](#builderror).
...
No network access: deny everything in both directions, no rules. This is the policy set by [`SandboxBuilder::disable_network()`](/sdk/rust/sandbox#sandbox-disable_network).
...
Open a multi-rule batch closure. Direction must be set inside via [`.egress()`](#rb-egress), [`.ingress()`](#rb-ingress), or [`.any()`](#rb-any) before any rule-adder, otherwise [`build()`](#policy-build) returns [`BuildError::DirectionNotSet`](#builderror).
...
Add [`Icmpv6`](#protocol) to the protocols set. Egress-only; same rule as [`icmpv4()`](#rule-icmpv4).
...
Commit an allow rule for the [`Public`](#destinationgroup) group: every IP not in another named category. A matching [`deny_public()`](#rule-deny_public) exists for each `allow_*` group adder below.
...
Allow the [`Loopback`](#destinationgroup) group (`127.0.0.0/8`, `::1`): the **guest's own** loopback, not the host. To reach a service on the host's localhost use [`allow_host()`](#rule-allow_host) instead. See the [loopback-vs-host watch-out](/networking/overview#reaching-the-host).
...
Allow the [`Host`](#destinationgroup) group: per-sandbox gateway IPs that back `host.microsandbox.internal`. This is the right shortcut for "let the sandbox reach my host's localhost", not [`allow_loopback()`](#rule-allow_loopback).
...
Commit **three** allow rules atomically: [`Loopback`](#destinationgroup) + `LinkLocal` + `Host`. Each uses the closure's current state. `Metadata` is intentionally not included; opt in via [`allow_meta()`](#rule-allow_meta) separately.
...
Add one allow-[`Domain`](#destination) rule per name, inheriting the closure's current direction / protocol / port state. Lazy-parse: invalid names surface as [`BuildError::InvalidDomain`](#builderror) from [`build()`](#policy-build).
...
Commit with [`Destination::Cidr`](#destination) of the IP as `/32` (v4) or `/128` (v6). The string is parsed at [`build()`](#policy-build); invalid values surface as [`BuildError::InvalidIp`](#builderror).
...
Builder for the sandbox's network stack, used in [`SandboxBuilder::network(|n| n...)`](/sdk/rust/sandbox#sandbox-network). Every setter returns `Self`, so calls chain. Errors accumulated by nested builders cascade up: the outermost `SandboxBuilder::build()` surfaces them as [`MicrosandboxError::NetworkBuilder(BuildError)`](#builderror).
...
Publish a TCP port from the sandbox to the host. The default host bind address is `127.0.0.1`. Equivalent to [`SandboxBuilder::port()`](/sdk/rust/sandbox#sandbox-port).
...
#### <span className="msb-recv">network.</span><span className="msb-hn">strict()</span>

```rust
fn strict(self, enabled: bool) -> Self
```
...
#### <span className="msb-recv">network.</span><span className="msb-hn">tcp_accept_queue_size()</span>

```rust
fn tcp_accept_queue_size(self, size: u32) -> Self
```

Set how many not-yet-accepted connections each published TCP port's host listener queues. Default: `1024`. Accepts `1..=i32::MAX`; other values record [`BuildError::InvalidTcpAcceptQueueSize`](#builderror). Connections arriving while the queue is full never reach the sandbox, so raise this when a burst of parallel connections — such as a reverse proxy fanning out one page load — exceeds it. The host kernel caps the effective depth at `net.core.somaxconn` on Linux (4,096 by default) or `kern.ipc.somaxconn` on macOS (128 by default). Runtimes that predate the setting are refused with an upgrade-required error rather than silently ignoring it.

`RestoreBuilder`, `BranchBuilder`, and `BranchManyBuilder` expose the same `tcp_accept_queue_size()` for the listeners a child publishes.

#### <span className="msb-recv">network.</span><span className="msb-hn">max_udp_connections()</span>

```rust
fn max_udp_connections(self, max: usize) -> Self
```
...
#### <span className="msb-recv">network.</span><span className="msb-hn">max_tcp_connections()</span>
...
```rust
fn max_tcp_connections(self, max: usize) -> Self
```

Limit the maximum number of concurrent TCP connections from the sandbox. When omitted, single-tenant mode is unlimited and multi-tenant mode defaults to 1,024. A positive value overrides the default; passing zero explicitly selects unlimited in either mode. Hosting services are responsible for restricting tenant overrides. See [deployment profiles](/networking/overview#deployment-profiles) for historical TCP limits and defaults.

For dedicated snapshot restore, `RestoreBuilder::max_tcp_connections` and `RestoreBuilder::max_udp_connections` set the destination limits. `RestoreBuilder::max_connections` remains a deprecated TCP alias; the last TCP setter call wins.
...
<div className="msb-params">
  <div className="msb-param">
    ...
    <div className="msb-param-desc">Maximum concurrent TCP connections.</div>
  </div>
</div>
...
Override the guest interface settings wholesale: MAC, MTU, IPv4/IPv6 addresses, and the derivation pools. A low-level escape hatch. For the common case of changing only the address pools, prefer [`ipv4_pool()`](#network-ipv4_pool) and [`ipv6_pool()`](#network-ipv6_pool), which validate the prefix. Unset fields fall back to values derived deterministically from the sandbox slot. See [`InterfaceOverrides`](#interfaceoverrides).
...
Enable or disable networking. Default: `true`. To fully turn networking off, prefer [`SandboxBuilder::disable_network()`](/sdk/rust/sandbox#sandbox-disable_network), which also sets the policy to [`NetworkPolicy::none()`](#networkpolicy-none).

#### <span className="msb-recv">network.</span><span className="msb-hn">secret_violation_action()</span>

```rust
fn secret_violation_action(self, action: SecretViolationAction) -> Self
```
...
Add a secret via a closure builder. Mirrors [`SandboxBuilder::secret()`](/sdk/rust/sandbox#sandbox-secret). See [`SecretBuilder`](/sdk/rust/secrets#secretbuilder) for the full API. A companion `secret_env(env_var, value, placeholder, allowed_host)` shorthand and `secret_entry(SecretEntry)` are also available on `NetworkBuilder`.
...
Builder for DNS interception, used in [`NetworkBuilder::dns(|d| d...)`](#network-dns). Owns rebind protection, nameserver pinning, and the per-query timeout. Every setter returns `Self`.
...
Builder for TLS interception, used in [`NetworkBuilder::tls(|t| t...)`](#network-tls). Creating it enables interception. Every setter returns `Self`.
...
PEM file used as the intercepting CA's certificate. Pair with [`intercept_ca_key()`](#tls-intercept_ca_key) to provide a stable CA across sandbox restarts. If unset, a CA is auto-generated and persisted.
...
PEM file with extra root CAs the proxy should trust only when the upstream SNI matches `pattern`. Pattern syntax matches [`bypass()`](#tls-bypass): exact hosts and `*.suffix` wildcards are supported.
...
Whether the proxy verifies upstream server certificates only when the upstream SNI matches `pattern`. Pattern syntax matches [`bypass()`](#tls-bypass): exact hosts and `*.suffix` wildcards are supported. Setting `verify` to `false` is the proxy-side equivalent of `curl -k` for matching hosts; TLS interception still runs.


## NetworkRateLimiterBuilder

Groups local rate limits by traffic direction. Supplied to [`NetworkBuilder::rate_limiter()`](#network-rate_limiter).
...
<p className="msb-backref">Held by <a href="#rule">Rule</a> · added by <a href="#port-1">port()</a> · <a href="#rule-port_range">port_range()</a></p>
...
Composable high-level access category accepted by [`NetworkPolicy::from_profiles()`](#networkpolicy-from_profiles).
...
<p className="msb-backref">Used by <a href="#rule">Rule</a> · <a href="#networkpolicy">NetworkPolicy</a> · <a href="#policy-default_egress">default_egress()</a></p>
...
<p className="msb-backref">Held by <a href="#destination">Destination</a> · committed by <a href="#rule-allow_public">RuleBuilder group adders</a></p>
...
<p className="msb-backref">Held by <a href="#rule">Rule</a> · set by <a href="#rule-tcp">RuleBuilder protocol setters</a></p>
...
<p className="msb-backref">Used by <a href="#dns-nameservers">nameservers()</a></p>
...
<p className="msb-backref">Used by <a href="#network-interface">interface()</a></p>

Per-sandbox guest interface overrides. Every field is optional; an omitted field is derived deterministically from the sandbox slot. Most callers only touch the pools via [`ipv4_pool()`](#network-ipv4_pool) / [`ipv6_pool()`](#network-ipv6_pool) rather than constructing this directly.
...
<p className="msb-backref">Built by <a href="#networkratelimiterbuilder">NetworkRateLimiterBuilder</a> · used by <a href="#network-rate_limiter">rate_limiter()</a></p>
...
<p className="msb-backref">Returned by <a href="#policy-build">NetworkPolicyBuilder::build()</a> · wrapped by <a href="#networkbuilder">NetworkBuilder</a></p>
...
| `InvalidTcpAcceptQueueSize { source }` | `tcp_accept_queue_size()` got a value outside `1..=i32::MAX` |
| `InvalidOutboundProxy { reason }` | [`SandboxBuilder::proxy()`](/sdk/rust/proxies#proxy) got an invalid proxy configuration |
...
<p className="msb-backref">Used by <a href="#network-secret_violation_action">secret_violation_action()</a></p>

Action taken when a secret placeholder is sent to a disallowed host. Also documented on the [Secrets](/sdk/rust/secrets#secretviolationaction) page, where it pairs with [`SecretBuilder`](/sdk/rust/secrets#secretbuilder).
