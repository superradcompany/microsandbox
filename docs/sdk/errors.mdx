...
func getOrCreate(ctx context.Context, name string) (*microsandbox.Sandbox, error) {
    handle, err := microsandbox.GetSandbox(ctx, name)
    if err != nil {
        if microsandbox.IsKind(err, microsandbox.ErrSandboxNotFound) {
            return microsandbox.CreateSandbox(ctx, name,
...
case microsandbox.IsKind(err, microsandbox.ErrExecTimeout):
...
for {
    ...
    case microsandbox.ExecEventFailed:
        f := ev.Failure // *microsandbox.ExecFailure
        switch f.Kind {
        case "not_found":
...
if microsandbox.IsKind(err, microsandbox.ErrSandboxAlreadyExists) {
...
```go Go
// The Go SDK currently surfaces boot failures as a generic *microsandbox.Error
...
_, err := microsandbox.CreateSandbox(ctx, "svc", microsandbox.WithImage("alpine"))
var me *microsandbox.Error
if errors.As(err, &me) {
...
```go Go
sb, err := microsandbox.CreateSandbox(ctx, "temp",
    microsandbox.WithImage("python"),
