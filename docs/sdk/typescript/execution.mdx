...
`create()` boots the sandbox without executing its image workload. `execDefault()` resolves the effective OCI ENTRYPOINT and CMD; `execDefaultStream()` streams it and `attachDefault()` runs it in an interactive PTY. The `*With` variants accept the existing execution-option builders. They throw `NoDefaultCommandError` when no executable default exists and never fall back to a shell.
...
const sandbox = await Sandbox.builder("worker")
  ...
  .cmd(["worker.py", "--once"])
  .create();
const out = await sandbox.execDefaultWith((e) => e.env("LOG_LEVEL", "debug"));
```
## Sandbox
