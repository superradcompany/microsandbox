---
title: Execution
"og:title": TypeScript SDK Execution reference
description: Run commands in a microVM sandbox with the TypeScript SDK. Stream output, send input, and inspect exit results.
...
|---|---|---|
| <span id="output-code"></span>`code` | `number` | Getter. Process exit code |
| <span id="output-success"></span>`success` | `boolean` | Getter. `true` when `code === 0` |
| <span id="output-status"></span>`status` | [`ExitStatus`](#exitstatus) | Getter. Exit status object |
