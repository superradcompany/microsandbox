...
keywords: ["TypeScript SDK", "TypeScript execution", "microsandbox execution"]
---
...
#### <span className="msb-recv">execOptions.</span><span className="msb-hn">capture()</span>

```typescript
capture(enabled)
```

Record this command's output to the sandbox's `exec.log`, where `logs()` reads it. Default: `false`. An ad-hoc exec's output, a shell's above all, can carry anything typed or printed, so it is written to the host's disk only when asked. The sandbox's workload (`execDefault`, `execDefaultWith`, `attachDefault`, `attachDefaultWith`) is recorded without asking; pass `capture(false)` there to opt out. No effect on a sandbox built with `disableExecLog()`.
...
#### <span className="msb-recv">attachOptions.</span><span className="msb-hn">capture()</span>

```typescript
capture(enabled)
```

Record this session's output to the sandbox's `exec.log`. Default: `false`: an interactive session's transcript can carry anything typed or printed, so it is recorded only when asked. `attachDefault` / `attachDefaultWith` record the workload without asking; pass `capture(false)` to opt out.
