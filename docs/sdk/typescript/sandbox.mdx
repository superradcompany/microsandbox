...
#### <span className="msb-recv">.</span><span className="msb-hn">cmd()</span>
...
#### <span className="msb-recv">.</span><span className="msb-hn">thp()</span>
...
#### <span className="msb-recv">.</span><span className="msb-hn">proxy()</span>
...
| `cursor(cursor)` | Continue after a previous page's `nextCursor` |
| `label(key, value)` | Require one label; repeated calls are AND-matched |
| `labels(record)` | Add several AND-matched labels |
