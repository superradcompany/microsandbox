...
Explicitly refresh the running sandbox's idle activity. This sends `core.touch`, receives `core.touched`, and advances the guest activity sequence used by the runtime idle-timeout monitor. It does not start stopped sandboxes and it does not bypass `maxDuration`. Experimental: requires `MSB_EXPERIMENTAL_MODIFY=1` in the environment.
...
Set the boot-time maximum hotpluggable guest memory in MiB. This reserves the envelope a sandbox can use after restart-backed changes and future live memory activation; it does not increase the effective memory by itself. Experimental: requires `MSB_EXPERIMENTAL_MODIFY=1` in the environment.
