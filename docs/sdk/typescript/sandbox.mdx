...
#### Sandbox.createDetachedWithProgress()

```typescript
static createDetachedWithProgress(config: SandboxConfig): Promise<PullSession>
```

Like [`Sandbox.createWithProgress()`](#sandbox-createwithprogress), but spawns the sandbox in detached mode so it survives after the parent process exits.
...
| Type | Description |
|------|-------------|
| [`PullSession`](#pullsession) | Async iterable of pull events. Call `result()` to obtain the running sandbox. |

---

#### Sandbox.createWithProgress()

```typescript
static createWithProgress(config: SandboxConfig): Promise<PullSession>
```

Create a sandbox and observe image pull and materialize progress. Returns a [`PullSession`](#pullsession) that yields [`PullEvent`](#pullevent)s as the image is resolved, downloaded, and materialized. Call `session.result()` after iteration to obtain the live sandbox.
...
Image pull and materialize progress event emitted by [`PullSession`](#pullsession). `eventType` is the discriminator; only a subset of the other fields is populated per variant — see [`PullEventType`](#pulleventtype) for which fields apply to each variant.

| Field | Type | Description |
|-------|------|-------------|
| eventType | [`PullEventType`](#pulleventtype) | Variant discriminator |
| reference? | `string` | Image reference being pulled |
| manifestDigest? | `string` | Resolved manifest digest |
| layerCount? | `number` | Number of layers in the image |
| totalDownloadBytes? | `number` | Sum of compressed layer sizes. May be absent if the manifest omits sizes. |
| layerIndex? | `number` | Index of the layer this event refers to |
| digest? | `string` | Layer digest |
| diffId? | `string` | Layer diff ID (uncompressed content hash) |
| downloadedBytes? | `number` | Bytes downloaded so far for this layer |
| totalBytes? | `number` | Total bytes for this layer. May be absent if the manifest omits sizes. |
| bytesRead? | `number` | Bytes read during materialize |

### PullEventType

String discriminator identifying which variant a [`PullEvent`](#pullevent) represents and which of its fields are populated.

| Value | Populated fields on `PullEvent` |
|-------|----------------------------------|
| `'resolving'` | `reference` |
...
| `'stitch_writing_fsmeta'` | (none) |
| `'stitch_writing_vmdk'` | (none) |
| `'stitch_complete'` | (none) |
...
Async iterable session returned by [`Sandbox.createWithProgress()`](#sandbox-createwithprogress) and [`Sandbox.createDetachedWithProgress()`](#sandbox-createdetachedwithprogress). Yields [`PullEvent`](#pullevent)s as the image is resolved, downloaded, and materialized. Call `result()` after iteration to obtain the live sandbox.

| Method | Returns | Description |
|--------|---------|-------------|
| [Symbol.asyncIterator] | `AsyncGenerator<`[`PullEvent`](#pullevent)`>` | Use with `for await...of` |
| recv() | `Promise<`[`PullEvent`](#pullevent)` \| null>` | Receive next event. Returns `null` when the stream ends. |
| result() | `Promise<`[`Sandbox`](#instance-methods)`>` | Resolve the underlying creation task and return the running sandbox. Single-shot: a second call throws `Error("result already consumed")`. |
