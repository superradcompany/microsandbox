...
#### <span className="msb-recv">.</span><span className="msb-hn">diskSize()</span>
<div className="msb-tags"><span className="msb-tag is-builder">builder</span></div>
...
Set the writable disk size (the OCI `upper.ext4` overlay) in MiB. Only valid for an OCI-image rootfs.

<p className="msb-label">Parameters</p>

<div className="msb-params">
  <div className="msb-param">
    <div className="msb-param-key"><code>sizeMib</code><span className="msb-type">number</span></div>
    <div className="msb-param-desc">Writable disk size in MiB.</div>
  </div>
</div>
...
Configure an explicit rootfs source. Use this for disk images when the filesystem type can't be auto-detected. The writable disk size is set at the sandbox level via [`diskSize()`](#disksize).
...
```typescript
const sandbox = await Sandbox.builder("worker")
  .imageWith((i) => i.disk("./ubuntu.qcow2").fstype("ext4"))
  .create();
```
