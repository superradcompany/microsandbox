...
| `init(program)` | Hand off PID 1 to `program` (absolute path or `"auto"`) after agentd setup. See [Init handoff](/sandboxes/customization#init-handoff) |
| `initWith(program, i => ...)` | Like `init` but with a closure-builder for argv and env (`.arg`, `.args`, `.env`, `.envs`) |
