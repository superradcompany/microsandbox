...
keywords: ["TypeScript SDK", "TypeScript sandbox", "microsandbox sandbox"]
---
...
handle.connectOrStart(options?): Promise<Sandbox>
handle.waitForStatus(status): Promise<SandboxHandle>
handle.restart(options?): Promise<Sandbox>
handle.destroy(options?): Promise<void>
sandbox.id: string
handle.id: string
```
...
`Sandbox.restore()` leaves host resources unmapped unless explicitly supplied or inherited. `.externalMountPolicy("strict")` validates supplied filesystem bindings; `"relaxed"` accepts supported mismatches without granting access to omitted resources. Inspect `await sandbox.restoreWarnings()` for unmapped paths and accepted mismatches. Each warning has `guestPath`, `reason`, and `staleInodes: readonly bigint[]`. See [Snapshot external mounts](/sandboxes/snapshots#external-mounts).

### Creation and restore progress

Use `createWithProgress()` for fresh creation and `restoreWithProgress()` for snapshots. Ignoring events does not delay the operation; `awaitSandbox()` supplies the authoritative result.

```typescript
const creation = await Sandbox.restore("saved")
  .name("child")
  .forked()
  .restoreWithProgress();
for await (const event of creation) {
  if (event.kind === "startup") console.log(event.phase, event.completedBytes, event.totalBytes);
}
const sandbox = await creation.awaitSandbox();
```

Startup phases report preparing, waiting for RAM backing, preparing RAM backing, reusing backing, syncing, and activating. Totals are absent for indeterminate work. Preparation does not consume the activation timeout. Call `creation.cancel()` to cancel creation, then await its result. `createWithPullProgress()` remains image-only.
...
A running `Sandbox` exposes `name` (`string`), `id` (`string`), and `ownsLifecycle` (`boolean`) as read-only properties. The stable opaque `id` changes when a removed name is recreated. Local attached handles own lifecycle; local detached handles and all cloud handles do not, because the cloud worker owns the sandbox process.
...
Lifecycle calls do not wait for unrelated guest operations on the same handle to finish. For example, you can pause and resume while an `exec()` promise is pending. Await operations explicitly when their order matters; stopping a sandbox may interrupt pending commands or filesystem requests.
...
Get the full configuration the sandbox was created with - image, cpus, memory, env, mounts, and the rest. The shape mirrors [`SandboxBuilder.build()`](#sandboxbuilder-build).
...
After detaching, this handle and its existing `fs()` objects reject new guest operations. Already-started operations retain their connection; await them before detaching if you need their results first.
...
const settled = await sandbox.waitUntilResized(60_000);
...
A live CPU or memory resize can take a moment to settle. The new limits are enforced immediately. The returned plan's `resizeStatus` is a single reading taken when the change was accepted; use [`waitUntilResized()`](#sandbox-waituntilresized) to wait for the guest. See [`SandboxModificationPlan`](#sandboxmodificationplan).
...
Wait indefinitely for graceful shutdown and release of the targeted runtime's ownership. Success allows immediate removal without racing teardown. No implicit kill is requested. Paused or unreachable sandboxes return errors; resume or kill explicitly. Ignoring a Promise does not cancel its operation.

#### <span className="msb-recv">sandbox.</span><span className="msb-hn">waitForStatus()</span>

```typescript
waitForStatus(status: SandboxStatus): Promise<SandboxHandle>
```

Wait without a built-in timeout until this exact persisted sandbox reaches `status`. Returns a refreshed metadata handle and throws `SandboxReplacedError` rather than following a reused name. Use `Promise.race` when the caller needs its own deadline; the underlying wait is not cancellable yet.
...
```typescript
destroy(options?: { force?: boolean; timeoutMs?: number }): Promise<void>
```

Stop and remove this exact sandbox. Defaults to graceful shutdown with a 10_000 ms timeout. Identity checks throw `SandboxReplacedError` instead of deleting a replacement that reused the name.
...
Apply one total budget to graceful dispatch and completion. Expiry throws `StopTimeoutError` without killing; `0` expires before dispatch. A delivered request can complete later. Only explicit `kill()` requests force termination.
...
<div className="msb-params">
  <div className="msb-param">
    <div className="msb-param-key"><code>timeoutMs</code><span className="msb-type">number</span></div>
    <div className="msb-param-desc">Total graceful-completion budget, as an integer from 0 to 4294967295 milliseconds. Zero expires before dispatch.</div>
  </div>
</div>
...
#### <span className="msb-recv">sandbox.</span><span className="msb-hn">resizeStatus()</span>
...
```typescript
resizeStatus(): Promise<ResourceResizeStatus[]>
```

Read the current live CPU and memory resize status without changing anything. Returns an empty array when the sandbox is not running.

#### <span className="msb-recv">sandbox.</span><span className="msb-hn">waitUntilResized()</span>

<Tooltip tip="Live resize is not available on microsandbox cloud."><span className="msb-badge-local">Local-only <Icon icon="circle-info" size={11} /></span></Tooltip>

```typescript
waitUntilResized(timeoutMs?: number): Promise<ResourceResizeStatus[]>
```

Wait until every resource is `"applied"`, `"guest-refused"`, or `"failed"`, and return the settled status. Returns an empty array when the sandbox is not running. Without a timeout a guest that never converges keeps this polling. On timeout, `ResizeTimeoutError.status` holds the last observed status, empty if no read completed before the deadline. The host still enforces the new limit if the wait times out.
...
<div className="msb-params">
  <div className="msb-param">
    <div className="msb-param-key"><code>timeoutMs</code><span className="msb-type">number</span></div>
    <div className="msb-param-desc">Budget in milliseconds. Omitted waits indefinitely and <code>0</code> checks once; expiry throws <code>ResizeTimeoutError</code>.</div>
  </div>
</div>
...
Materialize the [`SandboxConfig`](#sandboxconfig) without booting the sandbox. Validates the configuration and consumes the builder. For booting, use [`create`](#sandboxbuilder-create) instead - it builds internally.
...
Build and boot the sandbox. By default, a local handle is attached and stops when its `await using` binding goes out of scope. A cloud handle does not own the service-managed VM, so stop or remove it explicitly. Call [`detached(true)`](#sandboxbuilder-detached) first for local background mode.
...
#### <span className="msb-recv">sandboxBuilder.</span><span className="msb-hn">connectOrCreate()</span>

```typescript
connectOrCreate(): Promise<Sandbox>
```

Reuse or create the sandbox with this name. The method connects when it is running, waits while it is starting, starts it when it is created, stopped, or crashed, and creates it only when the name is unused. Builder settings apply only to a new sandbox. Concurrent callers reuse the same sandbox. Calling [`replace()`](#sandboxbuilder-replace) or [`replaceWithTimeout()`](#sandboxbuilder-replacewithtimeout) on this builder is invalid.
...
Set the root filesystem source. Accepts OCI image names (`"alpine"`), local directory paths, or disk image paths. The format is auto-detected. **Required** for creation; use `Sandbox.restore()` for snapshots.
...
Like [`init`](#sandboxbuilder-init), but with a closure-builder for argv and env vars. The builder exposes `.arg`, `.args`, `.env`, and `.envs`. Calling `init` or `initWith` more than once overwrites.
...
#### <span className="msb-recv">sandboxBuilder.</span><span className="msb-hn">proxy()</span>

```typescript
proxy(configure: (proxy: OutboundProxyBuilder) => Socks4ProxyBuilder | Socks5ProxyBuilder): this
```

Configure the single proxy used for outbound sandbox connections. See [Proxies](/sdk/typescript/proxies) for the protocol builders and examples.

<p className="msb-label">Parameters</p>

<div className="msb-params">
  <div className="msb-param">
    <div className="msb-param-key"><code>configure</code><span className="msb-type">function</span></div>
    <div className="msb-param-desc">Callback that selects and configures one proxy protocol.</div>
  </div>
</div>
...
Publish a TCP port from the sandbox to the host. The default host bind address is `127.0.0.1`. For an explicit bind address, use [`portBind`](#sandboxbuilder-portbind).
...
Same as [`replace()`](#sandboxbuilder-replace) with a custom SIGTERM timeout in milliseconds. `0` skips SIGTERM and force-kills immediately. Implies `replace()`.
...
Set a guest resource limit (a soft and hard limit at the same value). For separate soft/hard values, use [`rlimitRange`](#sandboxbuilder-rlimitrange).
...
<p className="msb-backref">Returned by <a href="#sandboxbuilder-createwithpullprogress">createWithPullProgress()</a></p>

Async iterable creation handle returned by [`createWithPullProgress()`](#sandboxbuilder-createwithpullprogress). Yields [`PullProgress`](#pullprogress) events as the image is resolved, downloaded, and materialized. Call `awaitSandbox()` after iteration to obtain the live sandbox.
...
Opaque stable identity captured by this handle. Receiver lifecycle methods remain bound to this value and throw `SandboxReplacedError` if the reusable name now points to another sandbox.
...
#### <span className="msb-recv">handle.</span><span className="msb-hn">connectOrStart()</span>

```typescript
connectOrStart(options?: { detached?: boolean }): Promise<Sandbox>
```

Connect when this exact sandbox is running, wait through `starting`, or start it when it is `created`, `stopped`, or `crashed`. `draining` and `paused` are rejected. Set `detached` only to make a required local start run in the background; connecting to an already-running sandbox does not change ownership.

#### <span className="msb-recv">handle.</span><span className="msb-hn">waitForStatus()</span>

```typescript
waitForStatus(status: SandboxStatus): Promise<SandboxHandle>
```

Wait without a built-in timeout until this exact sandbox reaches `status`, returning a refreshed handle. Throws `SandboxReplacedError` rather than rebinding to a replacement.
...
Restart this exact sandbox with the same state and option semantics as [`sandbox.restart()`](#sandbox-restart).
...
Stop and remove this exact sandbox. A stale handle cannot destroy a same-name replacement.
...
Wait indefinitely for graceful completion and release of the targeted runtime's ownership. Never implicitly kills.
...
Same completion contract as `stop()` under one total milliseconds budget. Expiry throws `StopTimeoutError` without killing; zero expires before dispatch.
...
#### <span className="msb-recv">handle.</span><span className="msb-hn">resizeStatus()</span>
...
`Promise<ResourceResizeStatus[]>`

#### <span className="msb-recv">handle.</span><span className="msb-hn">waitUntilResized()</span>

<Tooltip tip="Live resize is not available on microsandbox cloud."><span className="msb-badge-local">Local-only <Icon icon="circle-info" size={11} /></span></Tooltip>

```typescript
waitUntilResized(timeoutMs?)
```

Wait until every live resize is terminal. Returns an empty array when the sandbox is not running. Omitted waits indefinitely, even while a guest never converges, and `0` checks once; expiry throws `ResizeTimeoutError` with the last observed `status`.
...
Capture a disk snapshot. Locally, the name selects a member within the sandbox's snapshot group. See [Snapshots](/sdk/typescript/snapshots).
...
Fluent builder for OCI registry connection settings. Obtain it through [`SandboxBuilder.registry()`](#sandboxbuilder-registry); the callback's returned builder is stored in the sandbox configuration.
...
<p className="msb-backref">Used by <a href="#sandboxbuilder-loglevel">logLevel()</a></p>
...
<p className="msb-backref">Used by <a href="#sandboxbuilder-pullpolicy">pullPolicy()</a></p>
...
<p className="msb-backref">Returned by <a href="#sandbox-config">config()</a> · <a href="#sandboxbuilder-build">build()</a></p>

Configuration object produced by [`build()`](#sandboxbuilder-build) and returned by [`config()`](#sandbox-config). You generally should not construct this by hand; use the builder.
...
| state | `"accepted" \| "converging" \| "applied" \| "guest-refused" \| "failed"` | `"applied"` when requested, actual, and enforced match; `"converging"` while the guest is still onlining or offlining CPUs or plugging or releasing memory; `"guest-refused"` when the guest would not cooperate (the host enforces the new limit anyway) |
...
| `"starting"` | Runtime is booting or waiting for capacity |
