...
Gracefully shut down the sandbox and wait until stopped state is observed. Local waits up to 10,000 ms, then force-kills the sandbox if it is still running. Cloud waits up to 6 minutes because shutdown may include creating a durable disk checkpoint. If that deadline expires, this throws `SandboxStopTimedOutError` without cancelling the accepted server-side stop, which may still complete afterward.
...
Gracefully shut down the sandbox with an explicit observation timeout. Local force-kills after the timeout. Cloud throws `SandboxStopTimedOutError` instead, while the accepted server-side stop may continue. `0` requests immediate force termination and is therefore local-only.
...
<div className="msb-params">
  <div className="msb-param">
    <div className="msb-param-key"><code>timeoutMs</code><span className="msb-type">number</span></div>
    <div className="msb-param-desc">Milliseconds to wait for stopped state. On Local, expiry force-kills; on Cloud, expiry throws without cancelling the accepted stop. <code>0</code> is local-only.</div>
  </div>
</div>
...
```typescript
fromSnapshot(snapshot: string | Snapshot | SnapshotHandle): this
```

Boot from a previously captured snapshot instead of a fresh image. Passing a
snapshot object uses its stable backend-relative reference. See
...
<div className="msb-params">
  <div className="msb-param">
    ...
    <div className="msb-param-desc">Backend-relative string or snapshot object.</div>
  </div>
</div>
...
Set the root filesystem source. Accepts OCI image names (`"alpine"`), local directory paths, or disk image paths. The format is auto-detected. **Required** unless [`fromSnapshot`](#sandboxbuilder-fromsnapshot) is used.
...
Gracefully shut down and wait until stopped. Local waits up to 10,000 ms, then force-kills. Cloud waits up to 6 minutes and throws `SandboxStopTimedOutError` on expiry without cancelling the accepted server-side stop.
...
Same as `stop()` with an explicit timeout in milliseconds. Local force-kills on expiry; Cloud throws `SandboxStopTimedOutError` while the accepted stop may continue. `0` requests immediate force termination and is local-only.
