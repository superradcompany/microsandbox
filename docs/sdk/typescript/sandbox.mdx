---
title: Sandbox
"og:title": TypeScript SDK Sandbox reference
description: Create and manage microVM sandboxes with the TypeScript SDK. Configure resources, connect to running sandboxes, and control their lifecycle.
keywords: ["TypeScript SDK", "TypeScript sandbox", "microsandbox sandbox"]
---

Use the microsandbox TypeScript SDK to create a microVM sandbox for untrusted code, run commands, stream logs and metrics, and shut it down. Each sandbox has its own guest kernel. See [Overview](/sandboxes/overview) for configuration examples and [Lifecycle](/sandboxes/lifecycle) for state management.
...
#### <span className="msb-recv">sandbox.</span><span className="msb-hn">restoreWarnings()</span>

```typescript
restoreWarnings(): Promise<ExternalMountWarning[]>
```
...
| `staleInodes` | `readonly bigint[]` | Permanently invalid captured node IDs; empty when the whole export is unavailable. |
...
| Field | Type | Description |
|---|---|---|
| <span id="entry-timestamp"></span>`timestamp` | `Date` | Wall-clock capture time on the host |
| <span id="entry-source"></span>`source` | [`LogSource`](#logsource) | Where the chunk came from |
| <span id="entry-sessionid"></span>`sessionId` | `number \| null` | Relay-monotonic session id; `null` for `"system"` entries |
| <span id="entry-data"></span>`data` | `Uint8Array` | The captured chunk's bytes (UTF-8 lossy decoded by default) |
| <span id="entry-cursor"></span>`cursor` | `string` | Opaque resume token; pass to [`LogStreamOptions.fromCursor`](#logstreamoptions) to resume |
...
| Field | Type | Description |
|---|---|---|
| <span id="handle-name"></span>`name` | `string` | Sandbox name, up to 128 UTF-8 bytes |
| <span id="handle-id"></span>`id` | `string` | Opaque stable identity captured by this handle. Receiver lifecycle methods remain bound to this value and throw `SandboxReplacedError` if the reusable name now points to another sandbox. |
| <span id="handle-status"></span>`status` | [`SandboxStatus`](#sandboxstatus) | Current status |
| <span id="handle-configjson"></span>`configJson` | `string` | Raw JSON configuration |
| <span id="handle-createdat"></span>`createdAt` | `Date \| null` | Creation timestamp |
| <span id="handle-updatedat"></span>`updatedAt` | `Date \| null` | Last update timestamp |
