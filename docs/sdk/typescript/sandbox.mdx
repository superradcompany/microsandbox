...
Image pull and materialize progress event emitted by [`PullSession`](#pullsession). `eventType` is the discriminator; only a subset of the other fields is populated per variant.
...
|-------|------|-------------|
| eventType | `string` | Variant discriminator |
