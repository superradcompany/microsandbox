...
Apply is all-or-nothing: if any change conflicts, is unsupported, or needs a restart you did not allow, the call rejects and nothing changes.

`cpus` and `memory` take effect immediately on a running sandbox as long as the new values stay within [`maxCpus`](#maxcpus) / [`maxMemory`](#maxmemory); raising those limits requires a restart. Env and workdir changes affect future execs only; running processes keep their current environment (the plan notes this as a warning). The CLI equivalent is [`msb modify`](/cli/sandbox-commands#msb-modify).
