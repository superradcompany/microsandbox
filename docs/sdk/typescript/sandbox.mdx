...
| `libkrunfwPath(path)` | Override the host-side `libkrunfw` shared library for this sandbox. Use for custom kernel/firmware builds |
...
`libkrunfwPath(path)` is a host path, not a guest path. It takes precedence over the global `paths.libkrunfw` setting for this sandbox only, and the path must point to the platform shared library (`.so` on Linux, `.dylib` on macOS).
...
| libkrunfwPath | `string \| null` | Host-side custom `libkrunfw` path |
