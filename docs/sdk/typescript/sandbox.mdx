...
#### <span className="msb-recv">sandboxBuilder.</span><span className="msb-hn">diskOnly()</span>

```typescript
diskOnly(): this
```

Cold-boot only the disk state carried by a full snapshot. Chain after [`fromSnapshot()`](#fromsnapshot). Without it, a full snapshot resumes its captured execution and does not run the image CMD.
