...
Wait until every resource is `"applied"`, `"guest-refused"`, or `"failed"`, and return the settled status. The host still enforces the new limit if the wait times out.
...
Wait until every live resize is terminal. Omitted waits indefinitely and `0` checks once; expiry throws `ResizeTimeoutError`.
