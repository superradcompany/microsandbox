...
#### <span className="msb-recv">.</span><span className="msb-hn">txRateLimiter()</span>

```typescript
txRateLimiter(configure: (r: RateLimiterBuilder) => RateLimiterBuilder): this
```

<Accordion title="Example">

```typescript
.network((n) => n.txRateLimiter((r) => r
...
#### <span className="msb-recv">.</span><span className="msb-hn">rxRateLimiter()</span>

```typescript
rxRateLimiter(configure: (r: RateLimiterBuilder) => RateLimiterBuilder): this
```

Limit guest download (ingress) traffic. Same shape as [`txRateLimiter()`](#txratelimiter).
...
| txRateLimiter | [`RateLimiterConfig`](#ratelimiterconfig) ` \| null` | Guest upload (egress) rate limiter |
| rxRateLimiter | [`RateLimiterConfig`](#ratelimiterconfig) ` \| null` | Guest download (ingress) rate limiter |
...
<p className="msb-backref">Used by <a href="#networkconfig">NetworkConfig.txRateLimiter / rxRateLimiter</a></p>
