...
keywords: ["TypeScript SDK", "TypeScript networking", "microsandbox networking"]
---
...
<p className="msb-backref">Used by <a href="#network-policy">NetworkBuilder.policy()</a> · returned by <a href="#networkpolicy">NetworkPolicy factories</a></p>
...
Start the fluent [`NetworkPolicyBuilder`](#networkpolicybuilder). Equivalent to `new NetworkPolicyBuilder()`. String inputs (`.ip()`, `.cidr()`, `.domain()`, `.domainSuffix()`) are stored raw and parsed at [`build()`](#policy-build), so the chain stays clean and the first parse or validation failure surfaces there.
...
<p className="msb-backref">Used by <a href="#networkpolicy">NetworkPolicy.rules</a> · built via <a href="#rule-allowegress">Rule factories</a></p>
...
<p className="msb-backref">Used by <a href="#rule">Rule.destination</a> · built via <a href="#destination-any">Destination factories</a></p>
...
<p className="msb-backref">Used by <a href="#rule">Rule.ports</a> · built via <a href="#portrange-single">PortRange factories</a></p>
...
Set the policy. Accepts a [`NetworkPolicy`](#networkpolicy) literal or factory result, or a [`NetworkPolicyBuilder`](#networkpolicybuilder) (routed through the native bridge so lazy parse/validation errors surface at this call site).
...
<div className="msb-params">
  <div className="msb-param">
    <div className="msb-param-key"><code>enabled</code><span className="msb-type">boolean</span></div>
    <div className="msb-param-desc">Enable strict hostname policy mode.</div>
  </div>
</div>

#### tcpListenBacklog()

`network.tcpListenBacklog(4096)` sets how many not-yet-accepted connections each published TCP port's host listener queues. The default is 1,024, and values from 1 to 2,147,483,647 are accepted; any other number, including a fraction, makes the `tcpListenBacklog()` call itself throw. Connections arriving while the queue is full never reach the sandbox, so raise this when a burst of parallel connections, such as a reverse proxy fanning out one page load, exceeds it. The host kernel caps the effective depth at its own `somaxconn` (4,096 by default on Linux, 128 on macOS). `Sandbox.restore(...).tcpListenBacklog(n)` applies it to the listeners a restored child publishes.

#### maxUdpConnections()

`network.maxUdpConnections(512)` sets the runtime UDP relay session limit. Omission is unlimited in single-tenant mode and defaults to 1,024 in multi-tenant mode; zero explicitly selects unlimited. At a finite limit, a new session evicts the least recently active session. Sessions expire after 60 seconds of inactivity.

#### <span className="msb-recv">network.</span><span className="msb-hn">maxTcpConnections()</span>

`maxConnections` remains a deprecated alias for this builder method. The last call sets the TCP limit. Either method can be combined with the UDP limit.

Restore uses the same host-side limits without changing the captured guest interface:

```typescript
const restored = await Sandbox.restore("saved").name("child")
  .maxTcpConnections(64).maxUdpConnections(128).restore();
```

Omitted restore limits use destination defaults; zero selects unlimited. The same setters work with `restoreWithProgress()`, and `maxConnections()` remains a deprecated TCP-only alias.
...
#### <span className="msb-recv">network.</span><span className="msb-hn">secretViolationAction()</span>

```typescript
secretViolationAction(action: "block" | "block-and-log" | "block-and-terminate"): this
```

Set the sandbox-wide blocking action. Passthrough hosts are configured per secret with `allowPlaceholderFor()`.
...
Add a secret with full configuration. See [`SecretBuilder`](/sdk/typescript/secrets).
...
Three-arg auto-placeholder shorthand. Auto-generates the placeholder as `$MSB_<envVar>`, so it is the terse counterpart to [`secretEnv()`](#network-secretenv) when you do not need a custom placeholder. The full secret API also lives on the [secrets page](/sdk/typescript/secrets#networkbuilder-secretenvsimple).
...
Add `icmpv6` to the protocols set. Egress-only; same rules as [`icmpv4()`](#rule-icmpv4).
...
Add an inclusive port range. `lo > hi` records an error surfaced at [`build()`](#policy-build) time.
...
Allow the `loopback` group (`127.0.0.0/8`, `::1`). The **guest's own** loopback, not the host. To reach a service on the host's localhost, use [`allowHost()`](#rule-allowhost) instead. See the [loopback-vs-host watch-out](/networking/overview#reaching-the-host).
...
Allow the `host` group: per-sandbox gateway IPs that back `host.microsandbox.internal`. This is the right shortcut for "let the sandbox reach my host's localhost", not [`allowLoopback()`](#rule-allowloopback).
...
Add three allow rules atomically: `loopback + link-local + host`. Each uses the callback's current state. `metadata` is intentionally not included; opt in via [`allowMeta()`](#rule-allowmeta) separately.
...
Verify upstream server certificates only when the upstream SNI matches `pattern`. Pattern syntax matches [`bypass()`](#tls-bypass): exact hosts and `*.suffix` wildcards are supported. Setting `verify` to `false` is the proxy-side equivalent of `curl -k` for matching hosts; TLS interception still runs.
...
Path to a PEM file with extra root CAs the proxy should trust only when the upstream SNI matches `pattern`. Pattern syntax matches [`bypass()`](#tls-bypass): exact hosts and `*.suffix` wildcards are supported.


## NetworkRateLimiterBuilder

Groups local rate limits by traffic direction. Supplied to [`NetworkBuilder.rateLimiter()`](#network-ratelimiter).
...
| ports | `readonly `[`PublishedPort`](#publishedport)`[]` | Published ports |
...
| secretViolationAction | [`ViolationAction`](/sdk/typescript/secrets#violationaction) ` \| null` | Action on disallowed secret use |
| maxTcpConnections | `number \| null` | Maximum concurrent TCP connections |
| maxUdpConnections | `number \| null` (optional) | Maximum concurrent UDP relay sessions |
| tcpListenBacklog | `number \| null` (optional) | Accept-queue depth for published TCP port listeners; absent means the default, 1,024 |
| strict | `boolean` | Strict hostname policy mode |
...
| outboundProxy | [`OutboundProxy`](/sdk/typescript/proxies#outboundproxy)` \| null` | Canonical outbound proxy configuration stored in the sandbox network spec; `null` when unset |

### NetworkProfile

Composable high-level access category accepted by [`NetworkPolicy.fromProfiles()`](#networkpolicy-fromprofiles).
...
<p className="msb-backref">Used by <a href="#rule">Rule.action</a> · <a href="#networkpolicy">NetworkPolicy defaults</a></p>
...
<p className="msb-backref">Used by <a href="#rule">Rule.direction</a></p>
...
<p className="msb-backref">Used by <a href="#destination-group">Destination.group()</a> · <a href="#rd-group">RuleDestinationBuilder.group()</a></p>
...
<p className="msb-backref">Used by <a href="#rule">Rule.protocols</a></p>
