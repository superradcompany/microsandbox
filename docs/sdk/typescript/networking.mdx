...
#### <span className="msb-recv">.</span><span className="msb-hn">egressRateLimiter()</span>

```typescript
egressRateLimiter(configure: (r: RateLimiterBuilder) => RateLimiterBuilder): this
```

<Accordion title="Example">

```typescript
.network((n) => n.egressRateLimiter((r) => r
...
#### <span className="msb-recv">.</span><span className="msb-hn">ingressRateLimiter()</span>

```typescript
ingressRateLimiter(configure: (r: RateLimiterBuilder) => RateLimiterBuilder): this
```

Limit inbound (ingress) traffic. Same shape as [`egressRateLimiter()`](#egressratelimiter).
...
| egressRateLimiter | [`RateLimiterConfig`](#ratelimiterconfig) ` \| null` | Outbound (egress) rate limiter |
| ingressRateLimiter | [`RateLimiterConfig`](#ratelimiterconfig) ` \| null` | Inbound (ingress) rate limiter |
...
<p className="msb-backref">Used by <a href="#networkconfig">NetworkConfig.egressRateLimiter / ingressRateLimiter</a></p>
