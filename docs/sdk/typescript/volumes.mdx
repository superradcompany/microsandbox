---
title: Volumes
"og:title": TypeScript SDK Volumes reference
description: Create and mount persistent sandbox volumes with the TypeScript SDK.
keywords: ["TypeScript SDK", "TypeScript volumes", "microsandbox volumes"]
---
...
|---|---|---|
| <span id="volume-name"></span>`name` | `string` | The volume's name. |
| <span id="volume-path"></span>`path` | `string` | Absolute host path to the volume's directory. By default volumes live under `~/.microsandbox/volumes/<name>/`. <Tooltip tip="Only local volumes expose a path on the computer running the SDK."><span className="msb-badge-local">Local-only <Icon icon="circle-info" size={11} /></span></Tooltip> |
...
Get a filesystem handle for this volume. Local reads and writes use the managed host directory; Cloud reads and writes use the authenticated volume API. No sandbox needs to be running. See [`VolumeFs`](#volumefs) for the full API.
...
Fluent builder for a named volume. Obtained via [`Volume.builder(name)`](#volume-builder). Directory-backed volumes are the default; call [`disk()`](#volumebuilder-disk) plus [`size()`](#volumebuilder-size) for a disk-backed volume. Every setter returns the builder, so calls chain.
...
Create a raw ext4 disk-backed volume. Pair with [`size()`](#volumebuilder-size) to set the capacity.
...
Set the disk capacity in MiB. Required after [`disk()`](#volumebuilder-disk).
...
Materialize the volume configuration without creating the volume on disk. To create it, use [`create()`](#volumebuilder-create) instead. The returned config object is the internal `NapiVolumeConfig` shape (not a public export).
...
| Field | Type | Description |
|---|---|---|
| <span id="handle-name"></span>`name` | `string` | Volume name |
| <span id="handle-kind"></span>`kind` | `string` | Volume kind (`"dir"` or `"disk"`) |
| <span id="handle-quotamib"></span>`quotaMib` | `number \| null` | Recorded storage quota in MiB |
| <span id="handle-usedbytes"></span>`usedBytes` | `number` | Current disk usage in bytes |
| <span id="handle-capacitybytes"></span>`capacityBytes` | `number \| null` | Disk capacity in bytes (disk volumes) |
| <span id="handle-diskformat"></span>`diskFormat` | `string \| null` | Disk image format (disk volumes) |
| <span id="handle-diskfstype"></span>`diskFstype` | `string \| null` | Inner disk filesystem (disk volumes) |
| <span id="handle-labels"></span>`labels` | `ReadonlyArray<readonly [string, string]>` | Metadata labels |
...
<p className="msb-backref">Returned by <a href="#volumefs">VolumeFs.readStream()</a></p>
...
<p className="msb-backref">Returned by <a href="#volumefs">VolumeFs.writeStream()</a></p>
...
#### <span className="msb-recv">mount.</span><span className="msb-hn">owned()</span>

```typescript
owned(options?: OwnedVolumeOptions): this
```

Allocate local storage for this sandbox. The storage survives stop/start and is deleted with the sandbox. Cloud does not support owned mounts. See [Volumes](/sandboxes/volumes) for usage.

`OwnedVolumeOptions` accepts `kind` (`"dir"` by default or `"disk"`), `quotaMib` for directories, and `sizeMib` for disks. Disks require a positive capacity and use raw ext4. Common mount flags remain on `MountBuilder`; `owner()` and metadata policies apply only to directories.
...
Set the guest owner for bind and directory-backed named mounts when no per-file stat override exists. Both values must be integers from `0` through `4294967295`. Host inode ownership is unchanged. Unsupported for tmpfs, disk mounts, and `statVirtualization("off")`.
