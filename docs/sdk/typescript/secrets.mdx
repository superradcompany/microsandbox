...
| `allowPassthroughHost(host)` | Allow the placeholder to pass through unchanged to a specific host. Does not enable substitution. |
| `allowPassthroughHostPattern(pattern)` | Allow the placeholder to pass through unchanged to hosts matching a wildcard. Does not enable substitution. |
...
| passthroughHosts | `readonly string[]` | Hosts allowed to receive the placeholder unchanged |
| passthroughHostPatterns | `readonly string[]` | Wildcard host patterns allowed to receive the placeholder unchanged |
