...
Exact and wildcard allow-list entries require both a matching TLS SNI and an observed DNS answer tying the original guest destination IP to that host. `allowAnyHostDangerous(true)` is the explicit opt-out from host pinning. Empty allow-lists are rejected by `SecretBuilder.build()`. Intercepted HTTP/1 requests must also have a `Host` header matching the TLS SNI, ignoring case and an optional port.
...
| `injectBody(enabled)` | Allow substitution in HTTP/1 request bodies with no body content encoding. `Content-Length` bodies up to 16 MiB are buffered and their length header is updated; larger fixed-length bodies are blocked. Chunked bodies are decoded and re-encoded with fresh chunk sizes. Default `false`. |
