...
Allow an exact host or wildcard pattern to receive the placeholder unchanged when it cannot be substituted. Repeat this method for multiple hosts. `allowPassthroughFor()` remains a deprecated alias. This never grants permission to receive the real secret; use [`allow()`](#secret-allow) for substitution. With no passthrough match, the configured blocking action applies.
