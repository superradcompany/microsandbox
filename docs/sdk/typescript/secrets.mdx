...
#### <span className="msb-recv">secret.</span><span className="msb-hn">substituteInHeaderFields()</span>

```typescript
substituteInHeaderFields(fields: string[]): this
```

Enable header substitution but restrict it to the given header field names (matched case-insensitively), for example `["authorization"]`. A placeholder found in any other header is treated as a disabled location and is blocked unless the destination matches the secret's passthrough list. An empty list restores the default of substituting in every header. Names must be valid HTTP field names.
...
<div className="msb-params">
  <div className="msb-param">
    <div className="msb-param-key"><code>fields</code><span className="msb-type">string[]</span></div>
...
| `headerFields` | `string[]` | `[]` (all) | When non-empty, restrict header substitution to these field names. |
