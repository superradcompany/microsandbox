...
Select flat-snapshot free-space compaction. `off` is the default and avoids the ext4 scan, `auto` is best effort, and `on` requires a flat root and successful host deallocation. The created snapshot exposes the resolved `SnapshotCompactionInfo` through `snap.compaction`.
