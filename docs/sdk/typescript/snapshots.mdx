...
Snapshot scope: `"disk"` for a disk-only snapshot or `"resumable"` for disk plus VM execution state. See [`SnapshotScope`](#snapshotscope-type).
...
| `checkpoint` | `{ kind: "verified"; root: string }` | Verified checkpoint closure identity when state is resumable. |
