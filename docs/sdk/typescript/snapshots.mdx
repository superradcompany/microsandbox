---
title: Snapshots
"og:title": TypeScript SDK Snapshots reference
description: Create and restore microVM snapshots with the TypeScript SDK. Save disk state, or capture memory and running processes locally.
...
|---|---|---|
| <span id="snap-id"></span>`id` | `string` | Stable opaque `snap_...` identity, separate from the descriptor digest. |
| <span id="snap-path"></span>`path` | `string` | Local artifact directory. Throws `UnsupportedError` for cloud snapshots; use the snapshot object or its typed reference for backend-neutral operations. |
| <span id="snap-reference"></span>`reference` | `string` | Stable backend-relative value. Pass the snapshot object to `Sandbox.restore(snapshot)` to preserve both this value and its reference kind. |
| <span id="snap-referencekind"></span>`referenceKind` | `"id" \| "path"` | How the selected backend resolves `reference`. Most callers can pass the snapshot object directly and never inspect this value. |
| <span id="snap-digest"></span>`digest` | `string` | Canonical descriptor digest (`sha256:hex`), separate from the stable snapshot ID. |
| <span id="snap-sizebytes"></span>`sizeBytes` | `bigint \| null` | Backend-reported stored payload size in bytes, or `null` when unavailable. |
| <span id="snap-imageref"></span>`imageRef` | `string` | Image reference the snapshot was taken from. |
| <span id="snap-imagemanifestdigest"></span>`imageManifestDigest` | `string` | OCI manifest digest of the pinned image. |
| <span id="snap-format"></span>`format` | `"raw" \| "qcow2" \| null` | On-disk format of the upper layer. |
| <span id="snap-scope"></span>`scope` | `SnapshotScope` | Snapshot scope: `"disk"` for a disk-only snapshot or `"full"` for disk plus VM execution state. See [`SnapshotScope`](#snapshotscope-type). |
| <span id="snap-fstype"></span>`fstype` | `string \| null` | Filesystem type inside the upper (e.g. `"ext4"`). |
| <span id="snap-parent"></span>`parent` | `string \| null` | Manifest digest of the parent snapshot, or `null` for a root. |
| <span id="snap-createdat"></span>`createdAt` | `string` | RFC 3339 timestamp when the snapshot was created. |
| <span id="snap-labels"></span>`labels` | `ReadonlyArray<readonly [string, string]>` | User-supplied labels (sorted by key in canonical form), as `[key, value]` pairs. |
| <span id="snap-sourcesandbox"></span>`sourceSandbox` | `string \| null` | Best-effort source-sandbox name, if recorded. `null` when the manifest has no source recorded. |
...
| Field | Type | Description |
|---|---|---|
| <span id="snapshothandle-digest"></span>`digest` | `string` | Descriptor digest (`sha256:hex`), separate from the stable snapshot ID. |
| <span id="snapshothandle-name"></span>`name` | `string \| null` | Member name within its group, or `null` when no alias is recorded. |
| <span id="snapshothandle-parentdigest"></span>`parentDigest` | `string \| null` | Parent snapshot's manifest digest, or `null` for a root. |
| <span id="snapshothandle-scope"></span>`scope` | [`SnapshotScope`](#snapshotscope-type) | Snapshot payload scope (`"disk"` today). |
| <span id="snapshothandle-imageref"></span>`imageRef` | `string` | Image reference the snapshot was taken from. |
| <span id="snapshothandle-format"></span>`format` | `"raw" \| "qcow2"` | On-disk format of the upper layer. |
| <span id="snapshothandle-sizebytes"></span>`sizeBytes` | `bigint \| null` | Apparent size of the upper file at index time. |
| <span id="snapshothandle-createdat"></span>`createdAt` | `Date` | Snapshot creation time (from manifest). |
| <span id="snapshothandle-path"></span>`path` | `string` | Local artifact directory path. |
...
| Field | Type | Description |
|---|---|---|
| <span id="snapshothandle-reference"></span>`reference` | `string` | Backend-relative snapshot reference. |
| <span id="snapshothandle-referencekind"></span>`referenceKind` | `"id" \| "path"` | How the backend resolves the reference. |
