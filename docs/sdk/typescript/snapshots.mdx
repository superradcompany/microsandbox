...
Destination bindings use `.volume(guest, v => ...)`, `.port(...)`, `.portBind(...)`, `.portUdp(...)`, `.portUdpBind(...)`, `.vsock(...)`, and `.vsockDgram(...)`. Use `v => v.captured()` for a captured private volume. `.user(...)` sets the default user for new execs; `.logLevel(...)` configures host runtime logging. Host resources are not inherited unless `.dangerouslyInheritResources()` is selected. Full restore rejects missing external filesystems and additional disks by default; `.allowMissingResources()` opts into unavailable devices with warnings. `.externalMountPolicy("strict" | "relaxed")` validates supplied filesystem objects, independently of missing-resource permission. Root and owned storage remain required.
...
Look up an indexed snapshot by digest, name, or path. Returns a lightweight [`SnapshotHandle`](#snapshothandle) backed by the local index row.
...
Bundle a snapshot into a `.tar.zst` archive. The recorded manifest is archived as-is, so create the snapshot with [`recordIntegrity()`](#snapshot-recordintegrity) if receivers must verify content. See [`SaveOpts`](#saveopts-interface) for bundling options.
...
#### <span className="msb-recv">Snapshot.</span><span className="msb-hn">clone()</span>
<div className="msb-tags"><span className="msb-tag is-static">static</span><span className="msb-tag is-async">async</span></div>

```typescript
static clone(source: string, newName: string, opts?: CloneOpts): Promise<Snapshot>
```

Clone `source` (path, name, or digest) into a new snapshot named `newName`. Always writes a new artifact rather than mutating `source` in place: `source` and anything referencing it by digest are left untouched. With `opts.sparsify`, also reclaims host disk space for blocks the guest filesystem has already freed. With `opts.rootDiskSizeMib`, grows the clone's root disk in place (grow-only). If `source` recorded content integrity, the new artifact's integrity is recomputed fresh.

<p className="msb-label">Parameters</p>

<div className="msb-params">
  <div className="msb-param">
    <div className="msb-param-key"><code>source</code><span className="msb-type">string</span></div>
    <div className="msb-param-desc">Path, name, or digest of the snapshot to clone.</div>
  </div>
  <div className="msb-param">
    <div className="msb-param-key"><code>newName</code><span className="msb-type">string</span></div>
...
<div className="msb-params">
  <div className="msb-param">
    <div className="msb-param-key"><a className="msb-type" href="#snapshot">Promise&lt;Snapshot&gt;</a></div>
    <div className="msb-param-desc">The cloned artifact.</div>
  </div>
</div>
...
#### <span className="msb-recv">snapshot.</span><span className="msb-hn">sparsify()</span>
<div className="msb-tags"><span className="msb-tag is-builder">builder</span></div>
...
### CloneOpts <span className="msb-tag is-type">interface</span>

Options for [`Snapshot.clone()`](#snapshot-clone). All fields optional; an empty object clones into the default snapshots directory without sparsifying or resizing.

<p className="msb-backref">Used by <a href="#snapshot-clone">Snapshot.clone()</a></p>

| Field | Type | Description |
|-------|------|-------------|
| `destDir` | `string` | Parent directory for the new artifact. Defaults to the snapshots directory. |
| `labels` | `Record<string, string>` | Labels for the new snapshot. Not inherited from the source. |
| `force` | `boolean` | Overwrite an existing artifact at the destination. |
| `sparsify` | `boolean` | Deallocate host storage for blocks the guest ext4 filesystem has already freed, while cloning. |
| `rootDiskSizeMib` | `number` | Grow the clone's ext4 filesystem to this size in MiB, offline, before recording the artifact. Grow-only: a target at or below the source's current size throws. |
