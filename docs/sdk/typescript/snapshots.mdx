...
Create a sandbox from a snapshot artifact. Disk snapshots cold-boot; full snapshots resume captured execution. Mutually exclusive with [`.image()`](/sdk/typescript/sandbox#image); the snapshot already pins the image. Documented in full on the [Sandbox](/sdk/typescript/sandbox#fromsnapshot) page.

Chain `.diskOnly()` after `.fromSnapshot()` to cold-boot only the disk state from a full snapshot. Without it, a full snapshot resumes its captured execution and does not run the image CMD.
...
<div className="msb-params">
  <div className="msb-param">
    ...
    <div className="msb-param-desc">Bare name (resolved under the default snapshots directory) or filesystem path to an artifact directory or archive file.</div>
  </div>
</div>
...
await Snapshot.load("changes.tar.zst", undefined, "checkpoint-a");
const child = await Sandbox.builder("child")
  .fromSnapshot("changes.tar.zst").snapshotBase("checkpoint-a").create();
```

The count includes the oldest base but excludes the writable head. Omit `layers` to compact all sealed layers. `lastLayers` selects the newest N sealed export layers instead of `since`. Results expose physical counts, `materializedBytes`, `totalUs`, and `pauseUs`; materialized bytes are not reclaimed space. See [disk-chain maintenance](/sandboxes/snapshots#export-changes-and-compact-a-disk-chain) for dependency and retention rules.
