...
Clone `source` (path, name, or digest) into a new snapshot named `newName`. Always writes a new artifact rather than mutating `source` in place: `source` and anything referencing it by digest are left untouched. With `opts.compact`, also reclaims host disk space for blocks the guest filesystem has already freed. With `opts.rootDiskSizeMib`, grows the clone's root disk in place (grow-only). If `source` recorded content integrity, the new artifact's integrity is recomputed fresh.
...
#### <span className="msb-recv">snapshot.</span><span className="msb-hn">compact()</span>
<div className="msb-tags"><span className="msb-tag is-builder">builder</span></div>
...
Options for [`Snapshot.clone()`](#snapshot-clone). All fields optional; an empty object clones into the default snapshots directory without compacting or resizing.
...
| `compact` | `boolean` | Deallocate host storage for blocks the guest ext4 filesystem has already freed, while cloning. |
