...
Boot a fresh sandbox from a snapshot artifact. Mutually exclusive with [`.image()`](/sdk/typescript/sandbox#sandboxbuilder-image); the snapshot already pins the image. Documented in full on the [Sandbox](/sdk/typescript/sandbox#sandboxbuilder-fromsnapshot) page.
...
Request a `"resumable"` snapshot (disk plus VM state). Accepted by the builder, but [`.create()`](#snapshot-create) currently returns an `Unsupported` error; resumable snapshots have not landed yet.
