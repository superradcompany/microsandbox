...
Use `.forked()` for private CoW RAM, `.diskOnly()` for only a full snapshot's disk, and `.snapshotBase(base)` for a dependent archive. `restoreWithProgress()` returns progress events and an `awaitSandbox()` result. Image, CPU/RAM size, replacement, and startup-command setters are not accepted.
