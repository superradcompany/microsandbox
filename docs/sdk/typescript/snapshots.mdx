...
<Note>
  ...
  `listDir`, `reindex`, `save`/`saveTo`, `copyTo`, `load`, and `verify` keep the same API
...
[`image()`](/sdk/typescript/sandbox#sandboxbuilder-image); the snapshot already pins the
image. Passing a `Snapshot` or `SnapshotHandle` preserves its backend-relative
reference automatically. Documented in full on the
[Sandbox](/sdk/typescript/sandbox#sandboxbuilder-fromsnapshot) page.
...
storage. [`.destDir()`](#snapshot-destdir) selects a local parent or cloud
host-volume directory. The source sandbox is set with
[`.fromSandbox()`](#snapshot-fromsandbox), which is required. See
[`SnapshotBuilder`](#snapshotbuilder) for all setters.
...
[`recordIntegrity()`](#snapshot-recordintegrity) if receivers must verify content. See
