...
`Sandbox.create(...)` boots the sandbox without executing its image workload. `exec_default()` resolves the effective OCI ENTRYPOINT and CMD; `exec_default_stream()` streams it and `attach_default()` runs it in an interactive PTY. All three accept the same per-call options as their literal counterparts. They raise `NoDefaultCommandError` when no executable default exists and never fall back to a shell.
...
out = await sandbox.exec_default(env={"LOG_LEVEL": "debug"})
```
## Sandbox
