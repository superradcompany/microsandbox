...
keywords: ["Python SDK", "Python execution", "microsandbox execution"]
---
...
<div className="msb-params">
  ...
  <div className="msb-param">
    <div className="msb-param-key"><code>capture</code><span className="msb-type">bool | None</span></div>
    <div className="msb-param-desc">Record this command's output to the sandbox's <code>exec.log</code>, where <code>logs()</code> reads it. <code>None</code> (the default) means off for <code>exec</code> and <code>shell</code>: an ad-hoc command's output, a shell's above all, can carry anything typed or printed, so it reaches the host's disk only when asked. The default-workload helpers (<code>exec_default</code>, <code>attach_default</code>) record unless passed <code>capture=False</code>. No effect on a sandbox created with <code>disable_exec_log=True</code>.</div>
  </div>
</div>
...
<div className="msb-params">
  ...
  <div className="msb-param">
    <div className="msb-param-key"><code>cwd, user, env, timeout, stdin, tty, rlimits, capture</code></div>
...
| capture | `bool` | Record the command's output to `exec.log` (off by default for `exec`/`shell`) |
