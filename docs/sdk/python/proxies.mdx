---
title: Proxies
"og:title": Python SDK Proxies reference
description: Route outbound sandbox connections through a SOCKS4 or SOCKS5 proxy with the Python SDK.
---

Set `proxy=` on [`Sandbox.create()`](/sdk/python/sandbox#sandbox-create). SOCKS4 and SOCKS5 are mutually exclusive. See [Outbound proxies](/networking/outbound-proxy) for setup examples.

See [Proxies](/networking/outbound-proxy) for routing behavior, security considerations, and limits.
