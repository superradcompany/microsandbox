...
Gracefully shut down the sandbox and wait until stopped state is observed. Local waits up to ten seconds by default, then force-kills the sandbox if it is still running. Cloud waits up to six minutes because shutdown may include creating a durable disk checkpoint. If that deadline expires, this raises `SandboxStopTimedOutError` without cancelling the accepted server-side stop, which may still complete afterward.
...
<div className="msb-params">
  <div className="msb-param">
    ...
    <div className="msb-param-desc">Seconds to wait for stopped state. On Local, expiry force-kills; on Cloud, expiry raises without cancelling the accepted stop. <code>None</code> uses the backend default.</div>
  </div>
</div>
...
Gracefully shut down and wait until stopped state is observed. Local waits ten seconds by default, then force-kills. Cloud waits six minutes and raises `SandboxStopTimedOutError` on expiry without cancelling the accepted server-side stop.
...
| from_snapshot | `str \| os.PathLike[str] \| Snapshot \| SnapshotHandle` | - | Backend-relative snapshot reference or object to boot from instead of `image=`. Mutually exclusive with `image=` |
