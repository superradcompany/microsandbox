...
The returned `Sandbox` is an async context manager. Use `async with` to guarantee cleanup; on exit, the context manager kills the sandbox and removes its persisted state.
