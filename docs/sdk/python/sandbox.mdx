...
Wait until every resource is `APPLIED`, `GUEST_REFUSED`, or `FAILED`, and return the settled status. The host still enforces the new limit if the wait times out.
...
Wait until every live resize is terminal. Expiry raises `ResizeTimeoutError`.
