---
title: Sandbox
"og:title": Python SDK Sandbox reference
description: Create and manage microVM sandboxes with the Python SDK. Configure resources, connect to running sandboxes, and control their lifecycle.
keywords: ["Python SDK", "Python sandbox", "microsandbox sandbox"]
---

Use the microsandbox Python SDK to create a microVM sandbox for untrusted code, run commands, stream logs and metrics, and shut it down. Each sandbox has its own guest kernel. See [Overview](/sandboxes/overview) for configuration examples and [Lifecycle](/sandboxes/lifecycle) for state management.
...
|---|---|---|
| <span id="handle-name"></span>`name` | `str` | Sandbox name, up to 128 UTF-8 bytes |
| <span id="handle-id"></span>`id` | `str` | Opaque stable identity captured by this metadata handle. Receiver lifecycle calls remain bound to this value and raise `SandboxReplacedError` when the reusable name now points to another sandbox. |
| <span id="handle-status"></span>`status` | [`SandboxStatus`](#sandboxstatus) | Current status |
| <span id="handle-config_json"></span>`config_json` | `str` | Raw JSON configuration |
| <span id="handle-created_at"></span>`created_at` | `float \| None` | Creation timestamp (ms since epoch) |
| <span id="handle-updated_at"></span>`updated_at` | `float \| None` | Last update timestamp (ms since epoch) |
...
| Field | Type | Description |
|---|---|---|
| <span id="entry-timestamp_ms"></span>`timestamp_ms` | `float` | Wall-clock capture time (ms since Unix epoch, UTC) |
| <span id="entry-source"></span>`source` | [`LogSource`](#logsource) | Where the chunk came from |
| <span id="entry-session_id"></span>`session_id` | `int \| None` | Relay-monotonic session id; `None` for `"system"` entries |
| <span id="entry-cursor"></span>`cursor` | `str` | Opaque resume token; pass back via `log_stream(from_cursor=...)` |
| <span id="entry-data"></span>`data` | `bytes` | The chunk's raw bytes |
