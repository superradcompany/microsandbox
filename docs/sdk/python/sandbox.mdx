...
keywords: ["Python SDK", "Python sandbox", "microsandbox sandbox"]
---
...
await handle.connect_or_start(detached=False) -> Sandbox
await handle.wait_for_status(status) -> SandboxHandle
await handle.restart(force=False, timeout=None, detached=False) -> Sandbox
await handle.destroy(force=False, timeout=None) -> None
await sandbox.id -> str
handle.id -> str
```
...
<div className="msb-params">
  ...
  <div className="msb-param">
    ...
    <div className="msb-param-desc">Configuration fields: <code>image</code>, <code>cpus</code>, <code>memory</code>, <code>volumes</code>, <code>ports</code>, <code>network</code>, <code>proxy</code>, <code>secrets</code>, <code>detached</code>, and more.</div>
  </div>
</div>
...
Same parameters as [`create()`](#sandbox-create) but returns a [`PullSession`](#pullsession) that reports image preparation and local startup. This factory is synchronous; the async work happens through the session. Startup events have `event_type == PullEventType.STARTUP`, with `phase`, `completed_bytes`, and optional `total_bytes`. They distinguish backing preparation, cache waits/reuse, syncing, and activation. Preparation does not consume the activation timeout. Ignoring progress does not cancel creation; await `session.result()` for the outcome.
...
- `**kwargs`: the same creation fields accepted by [`Sandbox.create()`](#sandbox-create), used only if creation is necessary.
...
```python
identity: str = await sb.id
```

Await the opaque stable identity of this persisted sandbox. It remains unchanged across stop and restart and changes when the reusable name is removed and recreated. Use it for equality, logging, and correlation; do not parse it.
...
settled = await sb.wait_until_resized(timeout=60)
...
Plan or apply a configuration change. The returned plan uses [`ModificationDisposition`](#modificationdisposition) to classify when each change takes effect.
...
A live CPU or memory resize can take a moment to settle. The new limits are enforced immediately. `resize_status` is a single reading taken when the change was accepted; use [`wait_until_resized()`](#sb-wait_until_resized) to wait for the guest.
...
Wait indefinitely for graceful shutdown and release of the targeted runtime's ownership. Success allows immediate removal. Cancelling the await does not kill the sandbox or undo a delivered shutdown request. Paused or unreachable sandboxes return errors; resume or kill explicitly. The optional `timeout` retains its call syntax and uses the same total budget as `stop_with_timeout()`.
...
<div className="msb-params">
  <div className="msb-param">
    ...
    <div className="msb-param-desc">Total graceful-completion budget in seconds. <code>None</code> waits indefinitely; expiry raises <code>StopTimeoutError</code> without killing.</div>
  </div>
</div>
...
Use `await sb.stop_with_timeout(seconds)` for an explicit total graceful-completion budget. Expiry raises `StopTimeoutError`; zero expires before dispatch. A delivered request may finish later, but the timeout never requests force termination.
...
```python
async def wait_until_resized(self, *, timeout: float | None = None) -> list[ResourceResizeStatus]
```

Wait until every resource is `APPLIED`, `GUEST_REFUSED`, or `FAILED`, and return the settled status. Returns an empty list when the sandbox is not running. Without a timeout a guest that never converges keeps this polling. On timeout, `ResizeTimeoutError.status` holds the last observed status, empty if no read completed before the deadline. The host still enforces the new limit if the wait times out.
...
<div className="msb-params">
  <div className="msb-param">
    <div className="msb-param-key"><code>timeout</code><span className="msb-type">float | None</span></div>
    <div className="msb-param-desc">Budget in seconds. <code>None</code> waits indefinitely and <code>0</code> checks once; expiry raises <code>ResizeTimeoutError</code>.</div>
  </div>
</div>
...
Wait without a built-in timeout until this exact sandbox reaches `status`, returning a refreshed metadata handle. Raises `SandboxReplacedError` rather than following a reused name. Wrap the coroutine with `asyncio.wait_for()` when a deadline is required.
...
Stop and remove this exact sandbox. Defaults to graceful shutdown with the SDK's ten-second timeout. Raises `SandboxReplacedError` rather than deleting a same-name replacement.
...
Opaque stable identity captured by this metadata handle. Receiver lifecycle calls remain bound to this value and raise `SandboxReplacedError` when the reusable name now points to another sandbox.
...
Wait without a built-in timeout until this exact sandbox reaches `status`, returning a refreshed handle. Raises `SandboxReplacedError` rather than rebinding to a replacement.
...
Restart this exact sandbox with the same state and option semantics as [`sb.restart()`](#sb-restart).
...
Wait indefinitely for graceful completion and runtime ownership release. `await handle.stop_with_timeout(seconds)` supplies one total budget; expiry raises `StopTimeoutError` without killing, and zero expires before dispatch. The optional `stop(timeout=...)` argument uses the same behavior.
...
#### <span className="msb-recv">handle.</span><span className="msb-hn">resize_status()</span>
...
`Awaitable[list[ResourceResizeStatus]]`

#### <span className="msb-recv">handle.</span><span className="msb-hn">wait_until_resized()</span>

<Tooltip tip="Live resize is not available on microsandbox cloud."><span className="msb-badge-local">Local-only <Icon icon="circle-info" size={11} /></span></Tooltip>

```python
wait_until_resized(*, timeout=None)
```

Wait until every live resize is terminal. Returns an empty list when the sandbox is not running. Without a timeout it keeps polling while a guest never converges; expiry raises `ResizeTimeoutError` with the last observed `status`.
...
|-------|------|---------|-------------|
| image | `str \| os.PathLike[str] \| `[`ImageSource`](/sdk/python/images) | - | OCI image, local path, or disk image. Required; use `Sandbox.restore()` for snapshots. Use `Image.oci("python:3.12", root_disk=RootDisk.managed(8192))` to set a managed OCI root-disk size |
...
| proxy | [`OutboundProxy`](/sdk/python/proxies#outboundproxy)` \| None` | `None` | Single proxy used for outbound connections. See [Proxies](/sdk/python/proxies) |
| secrets | `Sequence[`[`SecretEntry`](/sdk/python/secrets#secretentry)`]` | `[]` | Secret injection |
| secret_violation_action | [`ViolationAction`](/sdk/python/secrets#violationaction) | `BLOCK_AND_LOG` | Sandbox-wide default action when a secret placeholder would leak to a non-eligible destination. Shorthand for `Network.secret_violation_action`; if both are provided, this top-level value takes precedence. |
...
| `ResourceConvergenceState.APPLIED` | `"applied"` | Desired, actual, and enforced values match. A shrink is applied only after the guest releases the capacity |
...
`APPLIED`, `GUEST_REFUSED`, and `FAILED` are terminal.
