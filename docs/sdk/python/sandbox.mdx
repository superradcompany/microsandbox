...
| image | `str \| ImageSource` | OCI image, local path, or disk image. Required unless `snapshot=` is passed. Pass `disk_size_mib=` to set the writable disk size for an OCI image |
