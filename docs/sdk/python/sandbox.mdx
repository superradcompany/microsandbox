...
| libkrunfw_path | `str` | Host path to a custom `libkrunfw` shared library for this sandbox. Use for custom kernel/firmware builds |
...
`libkrunfw_path` is a host path, not a guest path. It must point to a platform shared library (`.so` on Linux, `.dylib` on macOS) and overrides the global/bundled `libkrunfw` resolution for this sandbox only.
...
| hostname | `str` | - | Guest hostname |
| libkrunfw_path | `str` | - | Host path to a custom `libkrunfw` shared library for this sandbox. Overrides global/bundled resolution |
| user | `str` | - | Default guest user |
| entrypoint | `list[str]` | - | Override the image's stored ENTRYPOINT. Consulted by `msb exec` / `msb run` (CLI command resolution), **not** by `sb.exec` / `sb.shell` — those pass `cmd` literally |
