...
| max_cpus | `int` | same as `cpus` | Boot-time maximum possible virtual CPUs (experimental, requires `MSB_EXPERIMENTAL_MODIFY=1`) |
| memory | `int` | `512` | Guest memory in MiB. This is a limit, not a reservation |
| max_memory | `int` | same as `memory` | Boot-time maximum hotpluggable memory in MiB (experimental, requires `MSB_EXPERIMENTAL_MODIFY=1`) |
