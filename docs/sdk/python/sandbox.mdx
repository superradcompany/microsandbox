...
|-------|------|---------|-------------|
| image | `str \| `[`ImageSource`](/sdk/python/images#imagesource) | - | OCI image, local path, or disk image. Required unless `snapshot=` is passed. Use `Image.oci("python:3.12", upper_size_mib=8192)` to set an OCI upper size |
