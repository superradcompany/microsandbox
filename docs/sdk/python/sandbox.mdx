...
| init | `str \| tuple \| dict \|` [`InitConfig`](#initconfig) | Hand off PID 1 to a guest init binary. See [Init handoff](/sandboxes/customization#init-handoff) and [`InitConfig`](#initconfig) for accepted shapes |
...
| user | `str` | - | Default guest user |
| entrypoint | `list[str]` | - | Override image entrypoint |
| init | `str \| tuple \| dict \|` [`InitConfig`](#initconfig) | - | Hand off PID 1 to a guest init binary. See [Init handoff](/sandboxes/customization#init-handoff) |
...
Guest init-handoff specification. Pass it (or one of the equivalent shorthand shapes) as the `init=` kwarg to [`Sandbox.create()`](#sandboxcreate) to hand PID 1 inside the guest off to your own init binary after agentd's setup. See [Init handoff](/sandboxes/customization#init-handoff) for image picks, shutdown semantics, and tradeoffs.
...
| Field | Type | Description |
|-------|------|-------------|
| program | `str` | Absolute path to the init binary inside the guest rootfs |
| args | `tuple[str, ...]` | Supplemental argv (`argv[0]` is implicitly `program`) |
| env | `dict[str, str]` | Extra env vars merged on top of the inherited env |

The `init=` kwarg accepts four shapes for convenience:

| Form | Equivalent to |
|------|---------------|
| `init="/sbin/init"` | `InitConfig(program="/sbin/init")` |
| `init=("/sbin/init", ["--foo"])` | `InitConfig(program="/sbin/init", args=("--foo",))` |
| `init=("/sbin/init", {"args": [...], "env": {...}})` | full `InitConfig` |
| `init=InitConfig(program="/sbin/init", args=("--foo",))` | itself |

```python
from microsandbox import InitConfig, Sandbox

# Bare string
sb = await Sandbox.create("worker", image="alpine", init="/sbin/init")
