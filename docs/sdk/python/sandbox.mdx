...
Read captured output from the sandbox's `exec.log`. Backed by an on-disk JSON Lines file the runtime writes via the relay tap. It holds the sandbox's workload output and any exec that asked with `capture=True`; a sandbox created with `disable_exec_log=True` records nothing. Works on running and stopped sandboxes alike; there is no protocol traffic. The same method is available on [`SandboxHandle`](#sandboxhandle) for callers that don't want to start the sandbox first.
...
| disable_exec_log | `bool` | `False` | Do not record exec output to `exec.log` at all, not even the workload's. `logs()` then returns nothing |
