...
| image | `str \| ImageSource` | OCI image, local path, or disk image (required). Use `Image.oci("python:3.12", upper_size_mib=8192)` to set an OCI upper size |
...
Pass per-execution options as keyword-only arguments. For compatibility with existing callers, the second positional value can also be an options mapping with `args`, `cwd`, `user`, `env`, `timeout`, `stdin`, `tty`, and `rlimits` keys.
...
| args | `list[str] \| Mapping[str, Any] \| None` | Command arguments, or an options mapping with an `args` key |
| cwd | `str \| None` | Working directory |
| user | `str \| None` | Guest user |
| env | `Mapping[str, str] \| None` | Environment variables |
...
Like [`exec()`](#exec), pass per-execution options as keyword-only arguments. The second positional value may also be an options mapping for compatibility with existing callers.
...
| result() | `Awaitable[`[`Sandbox`](#instance-methods)`]` | Await once to get the final running sandbox |
