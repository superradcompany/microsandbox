...
| header_fields | `tuple[str, ...]` | `()` (all) | When non-empty, restrict header substitution to these field names (for example `("authorization",)`). |
...
Restricting `header_fields` matters for confidentiality, not just correctness: with every header substitutable, an untrusted guest can place the placeholder in a header the upstream host reflects back in its response and read the real secret from that response. Allow only the intended credential header (typically `Authorization`). A placeholder in a header outside the list is blocked unless the destination matches the secret's passthrough list.
