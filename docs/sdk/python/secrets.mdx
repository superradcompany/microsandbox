...
Raw values are persisted at rest. Prefer source references through [live modification](/sandboxes/secrets#change-while-running) when the secret is available in the host environment. Rotating or removing an existing secret does not require a restart; adding a secret or changing its guest-visible placeholder does. Live modification is local-only.
