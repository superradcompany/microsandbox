...
<div className="msb-params">
  ...
  <div className="msb-param">
    <div className="msb-param-key"><code>mode</code><a className="msb-type" href="#volumecreatemode">VolumeCreateMode | None</a></div>
...
| kind | [`VolumeKind`](#volumekind) | Volume storage kind |
...
| disk_format | [`DiskImageFormat`](#diskimageformat)` \| None` | Disk image format |
...
| named_mode | [`VolumeCreateMode`](#volumecreatemode)` \| None` | `None` | Named-volume creation behavior |
...
| Value | Description |
|-------|-------------|
| `"dir"` | Directory-backed volume mounted through virtiofs |
...
| Value | Description |
|-------|-------------|
| `"existing"` | Require the named volume to exist |
| `"create"` | Create a new named volume |
| `"ensure-exists"` | Reuse the named volume if present, otherwise create it |
...
| Value | Description |
|-------|-------------|
| `"strict"` | Fully virtualize guest-visible ownership and mode metadata |
| `"relaxed"` | Apply relaxed metadata virtualization |
| `"off"` | Expose host stat metadata directly |
...
| Value | Description |
|-------|-------------|
| `"private"` | Keep host-side permissions private to microsandbox |
| `"mirror"` | Mirror relevant host permissions into the guest view |
