---
title: Volumes
"og:title": Python SDK Volumes reference
description: Create and mount persistent sandbox volumes with the Python SDK.
...
|---|---|---|
| <span id="volume-name"></span>`name` | `str` | Volume name. |
| <span id="volume-path"></span>`path` | `str` | Host path to the volume's directory. <Tooltip tip="Only local volumes expose a path on the computer running the SDK."><span className="msb-badge-local">Local-only <Icon icon="circle-info" size={11} /></span></Tooltip> |
...
| Field | Type | Description |
|---|---|---|
| <span id="handle-name"></span>`name` | `str` | Volume name |
| <span id="handle-kind"></span>`kind` | [`VolumeKind`](#volumekind) | Volume storage kind |
| <span id="handle-quota_mib"></span>`quota_mib` | `int \| None` | Storage quota in MiB |
| <span id="handle-used_bytes"></span>`used_bytes` | `int` | Current disk usage in bytes |
| <span id="handle-capacity_bytes"></span>`capacity_bytes` | `int \| None` | Disk capacity in bytes |
| <span id="handle-disk_format"></span>`disk_format` | [`DiskImageFormat \| None`](#diskimageformat) | Disk image format |
| <span id="handle-disk_fstype"></span>`disk_fstype` | `str \| None` | Disk filesystem type |
| <span id="handle-labels"></span>`labels` | `dict[str, str]` | Metadata labels |
| <span id="handle-created_at"></span>`created_at` | `float \| None` | Creation timestamp (ms since epoch) |
| <span id="handle-fs"></span>`fs` | [`VolumeFs`](#volumefs) | Host-side filesystem handle |
