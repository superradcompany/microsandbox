...
await Sandbox.remove("image-cache-demo")
...
for layer in detail.layers:
    print(layer.position, layer.diff_id, layer.blob_digest)

report = await Image.prune()
print(report.image_refs_removed, report.layers_removed)
```
