...
```python
@staticmethod
async def save(reference: str, *, output_path: str, format: str = "docker") -> None
```

Export a cached image to an archive file. Raises [`ImageNotFoundError`](#errors) when the reference is not in the local cache, and `ValueError` for an unknown `format`.
...
<div className="msb-params">
  <div className="msb-param">
    <div className="msb-param-key"><code>reference</code><span className="msb-type">str</span></div>
    <div className="msb-param-desc">Cached image reference to export.</div>
  </div>
  <div className="msb-param">
