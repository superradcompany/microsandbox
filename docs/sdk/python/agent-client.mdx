---
title: Agent client
"og:title": Python SDK Agent client reference
description: Send requests to the sandbox guest agent with the Python SDK.
keywords: ["Python SDK", "Python agent client", "microsandbox agent client"]
---
...
Resolve a sandbox's `agentd` relay socket path **without connecting**. Returns the same path [`connect_sandbox()`](#agentclient-connect_sandbox) would dial. Use it to talk to `agentd` over a raw byte transport instead of this frame client, for example a transparent relay that splices bytes to and from the socket. The sandbox need not be running. Sandbox names are limited to 128 UTF-8 bytes.
