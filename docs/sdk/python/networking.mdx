...
| on_secret_violation | [`ViolationAction`](/sdk/python/secrets#violationaction) | Sandbox-wide action when a secret placeholder reaches a disallowed host |
| secret_passthrough_hosts | `tuple[str, ...]` | Hosts allowed to receive secret placeholders unchanged. Does not enable substitution |
| secret_passthrough_host_patterns | `tuple[str, ...]` | Wildcard host patterns allowed to receive secret placeholders unchanged. Does not enable substitution |
