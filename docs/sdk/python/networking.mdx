...
| rate_limiter | [`NetworkRateLimiter`](#networkratelimiter)` \| None` | `None` | Local rate limits grouped by direction. Cloud network configuration does not expose this field |
...
Frozen dataclass grouping local network limits by traffic direction. An omitted direction is unlimited. Cloud network configuration does not expose rate limits. Import it from `microsandbox`.
...
Frozen dataclass limiting one traffic direction. Caps bandwidth (bytes) and packet rate (frames) independently; a `None` bucket leaves that dimension unlimited. Import it from `microsandbox`.
