...
Network configuration is attached to a sandbox via the `network=` argument on `Sandbox.create(...)`. Pass a [`Network`](#network) instance, optionally carrying a custom [`NetworkPolicy`](#networkpolicy), [`DnsConfig`](#dnsconfig), and [`TlsConfig`](#tlsconfig).

```python
from microsandbox import Sandbox, Network, TlsConfig
...
sb = await Sandbox.create(
    ...
    network=Network(
        ...
        tls=TlsConfig(bypass=("*.googleapis.com",)),
    ),
)
```

For port publishing, set `ports=` on `Sandbox.create(...)` or pass `ports={host: guest, ...}` on the `Network` itself.
...
|-------|------|-------------|
| policy | `str \| NetworkPolicy \| None` | **Default:** `None`. Preset name (`"none"`, `"public_only"`, `"allow_all"`) or custom [`NetworkPolicy`](#networkpolicy) |
| ports | `Mapping[int, int]` | **Default:** `{}`. Port mappings from host to guest |
| deny_domains | `tuple[str, ...]` | **Default:** `()`. Deny egress to these exact domains. Each entry adds a `deny Domain("...")` policy rule that fires at DNS resolution (REFUSED), TLS first-flight (SNI), and TCP egress (cache fallback). Prepended onto the policy so it takes precedence over later allow rules |
| deny_domain_suffixes | `tuple[str, ...]` | **Default:** `()`. Deny egress to all subdomains of these suffixes. Adds `deny DomainSuffix("...")` rules; same enforcement layers as `deny_domains` |
| dns | [`DnsConfig`](#dnsconfig) ` \| None` | **Default:** `None`. DNS interception configuration |
| tls | [`TlsConfig`](#tlsconfig) ` \| None` | **Default:** `None`. TLS interception configuration |
| max_connections | `int \| None` | **Default:** `None`. Maximum concurrent connections |
| trust_host_cas | `bool \| None` | **Default:** `False`. Ship the host's trusted root CAs into the guest at boot so outbound TLS works behind corporate MITM proxies (Cloudflare Warp Zero Trust, Zscaler, Netskope, etc.). These proxies install a gateway CA on the host that's unknown to the guest's stock Mozilla bundle. Opt-in |
...
| Field | Type | Description |
|-------|------|-------------|
| default_egress | [`Action`](#action) | **Default:** `Action.DENY`. Action when no egress-applicable rule matches |
| default_ingress | [`Action`](#action) | **Default:** `Action.ALLOW`. Action when no ingress-applicable rule matches |
| rules | `tuple[`[`Rule`](#rule)`, ...]` | **Default:** `()`. Rules evaluated first-match-wins per direction |
...
| direction | [`Direction`](#direction) | **Default:** `Direction.EGRESS`. Which evaluator considers this rule. `Direction.ANY` matches in either direction |
| destination | `str \| None` | **Default:** `None`. Target filter: a [`DestGroup`](#destgroup) value, domain, CIDR range, domain suffix (prefixed with `"."`), or `"*"` for any. Domain and suffix strings are validated at sandbox creation; invalid names raise `ValueError` |
| protocol | [`Protocol`](#protocol) ` \| None` | **Default:** `None`. Protocol filter |
| port | `int \| str \| None` | **Default:** `None`. Single port (`443`) or range (`"8000-9000"`) |
...
|------|------|-------------|
| direction | [`Direction`](#direction) | **Default:** `Direction.EGRESS`. Traffic direction |
| protocol | [`Protocol`](#protocol) ` \| None` | **Default:** `None`. Protocol filter |
| port | `int \| str \| None` | **Default:** `None`. Port or port range |
| destination | `str \| None` | **Default:** `None`. Destination filter |
...
| Field | Type | Description |
|-------|------|-------------|
| nameservers | `tuple[str, ...]` | **Default:** `()`. Nameservers (`IP`, `IP:PORT`, `HOST`, or `HOST:PORT`). Overrides the host's `/etc/resolv.conf` when set. Hostnames are resolved once at startup via the host's OS resolver |
| query_timeout_ms | `int \| None` | **Default:** `None` (5000 ms server-side). Per-DNS-query timeout in milliseconds |
| rebind_protection | `bool` | **Default:** `True`. Block DNS responses resolving to private IPs |
...
| Field | Type | Description |
|-------|------|-------------|
| block_quic | `bool` | **Default:** `False`. Block QUIC/HTTP3 (UDP) on intercepted ports, forcing TCP/TLS fallback |
| bypass | `tuple[str, ...]` | **Default:** `()`. Domains to skip interception. Use for domains with certificate pinning |
| ca_cert | `str \| None` | **Default:** `None`. Path to a custom interception CA certificate PEM file |
| ca_cn | `str \| None` | **Default:** `None`. Common name for the generated interception CA |
| ca_key | `str \| None` | **Default:** `None`. Path to a custom interception CA private key PEM file |
| intercepted_ports | `tuple[int, ...]` | **Default:** `(443,)`. TCP ports where TLS interception is active |
| verify_upstream | `bool` | **Default:** `True`. Verify upstream server certificates. Set to `False` only for self-signed servers |
