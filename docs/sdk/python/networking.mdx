...
| tx_rate_limiter | [`RateLimiter`](#ratelimiter)` \| None` | `None` | Guest upload (egress) rate limiter. `None` means unlimited |
| rx_rate_limiter | [`RateLimiter`](#ratelimiter)` \| None` | `None` | Guest download (ingress) rate limiter. `None` means unlimited |
...
<p className="msb-backref">Used by <a href="#network">Network(tx_rate_limiter=... / rx_rate_limiter=...)</a></p>
