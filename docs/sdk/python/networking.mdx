...
Body for the `403 Forbidden` the gateway returns to HTTP clients (and HTTPS clients on intercepted ports) when a domain rule denies the connection. `{host}` is substituted with the blocked hostname. `None` uses the default agent-facing message. See [What a denied HTTP request sees](/networking/overview#what-a-denied-http-request-sees)
