...
| egress_rate_limiter | [`RateLimiter`](#ratelimiter)` \| None` | `None` | Outbound (egress) rate limiter. `None` means unlimited |
| ingress_rate_limiter | [`RateLimiter`](#ratelimiter)` \| None` | `None` | Inbound (ingress) rate limiter. `None` means unlimited |
...
<p className="msb-backref">Used by <a href="#network">Network(egress_rate_limiter=... / ingress_rate_limiter=...)</a></p>
...
```python
from microsandbox import Network, RateLimiter, Sandbox, TokenBucket
...
Frozen dataclass limiting one traffic direction. Caps bandwidth (bytes) and packet rate (frames) independently; a `None` bucket leaves that dimension unlimited. Limits apply on the next sandbox start; cloud sandboxes reject rate limiters as unsupported for now. Import it from `microsandbox`.
...
Frozen dataclass for one token bucket of a rate limiter. The bucket starts full and refills continuously at `size` tokens per `refill_time_ms`; the one-time burst is spent before the regular budget and never refills.

| Field | Type | Description |
|-------|------|-------------|
| size | `int` | Bucket capacity in tokens: bytes for bandwidth, frames for ops. Must be greater than zero |
| refill_time_ms | `int` | Time to refill `size` tokens, in milliseconds. Must be greater than zero |
