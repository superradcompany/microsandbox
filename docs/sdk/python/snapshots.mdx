...
Supply destination bindings with `volumes`, `ports`, and `vsock`, using the same typed values as creation. `captured_volumes=["/data"]` selects private captured disks. `user` changes the default for new execs, and `log_level` configures host logging. Resources are not inherited unless `dangerously_inherit_resources=True` is explicit. Full restore rejects missing external filesystems and additional disks by default; `allow_missing_resources=True` opts into unavailable devices with warnings. `external_mount_policy="strict"` (default) or `"relaxed"` validates supplied filesystem objects, independently of missing-resource permission. Root and owned storage remain required.
...
<div className="msb-params">
  ...
  <div className="msb-param">
    <div className="msb-param-key"><code>sparsify</code><span className="msb-type">bool</span></div>
    <div className="msb-param-desc">Deallocate host storage for blocks the guest ext4 filesystem has already freed, before recording the artifact. Never changes guest-visible content, and a sparsification failure never fails creation. Default <code>False</code>.</div>
  </div>
</div>
...
Clone `source` (path, name, or digest) into a new snapshot named `new_name`. Always writes a new artifact rather than mutating `source` in place: `source` and anything referencing it by digest are left untouched. With `sparsify=True`, also reclaims host disk space for blocks the guest filesystem has already freed. With `root_disk_size_mib`, grows the clone's root disk in place (grow-only). If `source` recorded content integrity, the new artifact's integrity is recomputed fresh.
...
<div className="msb-params">
  ...
  <div className="msb-param">
    <div className="msb-param-key"><code>dest_dir</code><span className="msb-type">str | os.PathLike[str] | None</span></div>
    <div className="msb-param-desc">Parent directory to create the new artifact in. Defaults to the snapshots directory.</div>
  </div>
  <div className="msb-param">
    <div className="msb-param-key"><code>labels</code><span className="msb-type">dict[str, str] | None</span></div>
    <div className="msb-param-desc">Labels for the new snapshot. Not inherited from the source.</div>
  </div>
  <div className="msb-param">
    <div className="msb-param-key"><code>force</code><span className="msb-type">bool</span></div>
    <div className="msb-param-desc">Overwrite an existing artifact at the destination. Default <code>False</code>.</div>
  </div>
  <div className="msb-param">
    <div className="msb-param-key"><code>sparsify</code><span className="msb-type">bool</span></div>
    <div className="msb-param-desc">Deallocate host storage for blocks the guest ext4 filesystem has already freed, while cloning. Default <code>False</code>.</div>
  </div>
  <div className="msb-param">
    <div className="msb-param-key"><code>root_disk_size_mib</code><span className="msb-type">int | None</span></div>
    <div className="msb-param-desc">Grow the clone's ext4 filesystem to this size in MiB, offline, before recording the artifact. Grow-only: a target at or below the source's current size raises.</div>
  </div>
</div>
