...
```python
@staticmethod
async def create(name: str, *, from_snapshot: str | os.PathLike | None = None, disk_only: bool = False, **kwargs) -> Sandbox
```

Create a sandbox from a snapshot artifact by passing `from_snapshot=` as a peer of `image=`. Disk snapshots cold-boot; full snapshots resume captured execution. The two sources are mutually exclusive: pass exactly one. See [`Sandbox.create()`](/sdk/python/sandbox#sandbox-create) for the full set of configuration kwargs.
...
<div className="msb-params">
  ...
  <div className="msb-param">
    ...
    <div className="msb-param-desc">Snapshot bare name, artifact directory, or archive file to boot from instead of <code>image=</code>.</div>
  </div>
  <div className="msb-param">
    <div className="msb-param-key"><code>disk_only</code><span className="msb-type">bool</span></div>
    <div className="msb-param-desc">Cold-boot only the disk state carried by a full snapshot. Default <code>False</code> resumes full snapshot execution.</div>
  </div>
</div>
...
await Snapshot.save("checkpoint-b", "changes.tar.zst", since="checkpoint-a")
await Snapshot.load("changes.tar.zst", base="checkpoint-a")
child = await Sandbox.create("child", from_snapshot="changes.tar.zst", snapshot_base="checkpoint-a")
```

The count includes the oldest base but excludes the writable head. Omit `layers` to compact all sealed layers. Use `last_layers=n` instead of `since` to export the newest N sealed layers. Results are dictionaries with `input_layers`, `selected_layers`, `output_layers`, `materialized_bytes`, `total_us`, `pause_us`, and `dry_run`. See [disk-chain maintenance](/sandboxes/snapshots#export-changes-and-compact-a-disk-chain) for dependency and retention rules.
...
Create a disk snapshot from a stopped or crashed sandbox, or set `full=True` to checkpoint a running sandbox. `name` is resolved under the default snapshots directory (`~/.microsandbox/snapshots/<name>/`), or under `dest_dir=` when given; `from_sandbox=` names the sandbox to capture and is required.
...
<div className="msb-params">
  ...
  <div className="msb-param">
    ...
    <div className="msb-param-desc">Capture disk, memory, execution, and device state from a running sandbox. Default <code>False</code> selects stopped-sandbox disk capture.</div>
  </div>
</div>
