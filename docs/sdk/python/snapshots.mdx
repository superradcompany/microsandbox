...
<div className="msb-params">
  ...
  <div className="msb-param">
    <div className="msb-param-key"><code>compact</code><span className="msb-type">bool</span></div>
    <div className="msb-param-desc">Deallocate host storage for blocks the guest ext4 filesystem has already freed, before recording the artifact. Never changes guest-visible content, and a compaction failure never fails creation. Default <code>False</code>.</div>
  </div>
</div>
...
Clone `source` (path, name, or digest) into a new snapshot named `new_name`. Always writes a new artifact rather than mutating `source` in place: `source` and anything referencing it by digest are left untouched. With `compact=True`, also reclaims host disk space for blocks the guest filesystem has already freed. With `root_disk_size_mib`, grows the clone's root disk in place (grow-only). If `source` recorded content integrity, the new artifact's integrity is recomputed fresh.
