...
- `ValueError` — both `image=` and `snapshot=` were passed, or neither
- `SnapshotNotFoundError` — the snapshot artifact doesn't exist
- `SnapshotIntegrityError` — the artifact's manifest validation failed
...
| `format` | `str` | `"raw"` or `"qcow2"` (always `"raw"` in v1) |
...
| `parent_digest` | `str \| None` | Parent snapshot digest (None in v1) |
