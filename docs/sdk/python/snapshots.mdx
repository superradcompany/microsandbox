...
Create disk snapshots and full checkpoints in local snapshot groups. See [Snapshots](/sandboxes/snapshots) for usage and lifecycle concepts.
...
Use `forked=True` for private CoW RAM, `disk_only=True` for only a full snapshot's disk, and `snapshot_base="group:member"` for a dependent archive. `Sandbox.restore_with_progress()` accepts the same options and returns a `PullSession`. Image, CPU/RAM size, replacement, and startup-command options are rejected.
