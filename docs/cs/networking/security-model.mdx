...
description: "Jak microsandbox zabezpečuje síťovou vrstvu"
icon: "shield-halved"
---
...
- **Host trust store integrity.** When [`trust_host_cas`](/cs/sdk/rust/networking#trust_host_cas) is opted into, an attacker who can plant a CA on the host has that trust inside every sandbox. Off by default for exactly this reason.

## See also

- [DNS](/cs/networking/dns): the controls that ride on the DNS interceptor
- [TLS interception](/cs/networking/tls): plaintext visibility for per-host policy and secret injection
