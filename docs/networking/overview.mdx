...
When a domain rule denies a connection, the gateway never dials upstream. For plain HTTP, and for HTTPS on an intercepted port (see [TLS interception](/networking/tls)), the gateway answers the guest with `403 Forbidden` and a plain-text body instead of a bare connection reset. The default body is written for an agent running inside the sandbox:
...
Set `http_deny_message` to replace the body; `{host}` is substituted with the blocked hostname. Use it to name the tool or workflow your agent should use to request access.
...
let sb = Sandbox::builder("agent")
    ...
    .network(|n| n
        ...
        .http_deny_message("{host} is blocked. Call the `RequestHostAccess` tool to ask the user for it."))
    .create()
    .await?;
```
...
await using sb = await Sandbox.builder("agent")
    ...
    .network((n) => n
        .policy(policy)
        .tls((t) => t)
        .httpDenyMessage("{host} is blocked. Call the `RequestHostAccess` tool to ask the user for it."))
    .create();
```
