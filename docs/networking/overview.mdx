...
The Rust builder accepts string inputs (`.ip("10.0.0.5")`, `.cidr("10.0.0.0/24")`, `.domain("api.example.com")`, etc.) and parses them at `.build()` time, so the chain stays clean. TypeScript, Python, and Go accept policy objects matching the wire format. Direction strings are `"egress"`, `"ingress"`, or `"any"`, and group keywords match the CLI grammar. See the [Rust reference](/sdk/rust/networking) for the full Rust surface.
