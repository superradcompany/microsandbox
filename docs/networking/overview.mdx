...
// No network access
await using sb1 = await Sandbox.builder("isolated")
    .image("python")
    .network((n) => n.policy(NetworkPolicy.none()))
    .create()

// Public internet only (default, blocks private ranges)
await using sb2 = await Sandbox.builder("web-agent")
    .image("python")
    .network((n) => n.policy(NetworkPolicy.publicOnly()))
    .create()

// Unrestricted access
await using sb3 = await Sandbox.builder("dev")
    .image("python")
...
await using sb = await Sandbox.builder("secure-agent")
    ...
    .network((n) => n.policy({
        ...
        rules: [
            ...
            {
                direction: "egress",
                destination: Destination.group("metadata"),
                protocols: [],
                ports: [],
                action: "deny",
            },
            {
                direction: "egress",
                destination: Destination.group("private"),
                protocols: [],
                ports: [],
                action: "deny",
            },
...
const policy = {
    defaultEgress: "deny",
    defaultIngress: "allow",
    rules: [Rule.allowEgress(Destination.group("host"))],
}
```
