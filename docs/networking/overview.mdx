...
keywords: ["sandbox networking", "network policy", "port forwarding", "egress"]
icon: "network-wired"
---
...
All outbound TCP connections, including HTTP, HTTPS, TLS, SSH, and database protocols, can be routed through an HTTP CONNECT proxy. Set `upstream_proxy` in the sandbox network configuration, or use `MSB_UPSTREAM_PROXY`. When the explicit setting is absent, `HTTPS_PROXY`, `HTTP_PROXY`, and `ALL_PROXY` are checked in that order. Gateway loopback connections remain direct so published host ports continue to work.

```rust
let sb = Sandbox::builder("proxied")
    .image("python")
    .network(|n| n.upstream_proxy("http://proxy.example:3128"))
    .create()
    .await?;
```

## Guest HTTP forward proxy

Enable the guest-facing HTTP proxy with `network.proxy`. It listens on the
gateway/host alias and accepts HTTP absolute-form requests and `CONNECT
host:port`. When `upstream_proxy` is set, the proxy opens each tunnel through
that HTTP CONNECT proxy without resolving the destination hostname locally.

```yaml
network:
  proxy:
    enabled: true
    # Omit port (or set it to 0) to assign a free port per sandbox.
    port: 0
  upstream_proxy: http://proxy.example:8080
```

When enabled, `HTTP_PROXY`, `HTTPS_PROXY`, and `ALL_PROXY` (plus lowercase
variants) are set in the guest unless explicitly supplied. The host alias and
loopback addresses are added to `NO_PROXY`.

Hostname allow-list entries are forwarded to the upstream proxy unchanged.
Therefore, the upstream proxy's DNS resolution is part of the trust boundary:
only use an upstream proxy whose DNS results you trust for the allowed hosts.
...
For exact network APIs, see [TypeScript](/sdk/typescript/networking), [Rust](/sdk/rust/networking), [Python](/sdk/python/networking), or [Go](/sdk/go/networking). For CLI flags and configuration fields, see [Sandbox commands](/cli/sandbox-commands#network-profiles) and [Configuration file](/cli/configuration#network).

## Next

- [DNS](/networking/dns): domain blocking, pinned nameservers, and query timeouts
- [Proxy](/networking/outbound-proxy): route outbound TCP and non-DNS UDP through an external SOCKS proxy
- [TLS MITM](/networking/tls): inspect HTTPS with an auto-generated CA
- [VSock](/networking/host-sockets): connect guest applications to local host IPC without opening a TCP port
