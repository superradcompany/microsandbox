...
On Windows, the first published port may trigger a Windows Defender Firewall prompt for `msb.exe` because the runtime opens a host listening socket. For local development, keep the bind address on `127.0.0.1`. Only allow private or public network access in the firewall prompt when you intentionally bind a published port beyond loopback.
