...
`Group::Host` covers the sandbox gateway, which is where DNS is delivered — that's why DNS rules typically target it under a deny-by-default policy. See the [DNS-as-egress](/networking/dns#dns-as-egress) breakdown for the full rule, including why other IP-based destinations can't grant DNS.
...
```go
sb, err := microsandbox.CreateSandbox(ctx, "secure-agent",
    ...
    microsandbox.WithNetwork(&microsandbox.NetworkConfig{
        ...
        Rules: []microsandbox.PolicyRule{
            ...
            },
            {
                ...
                Direction:   microsandbox.PolicyDirectionEgress,
                Destination: "*",
                Protocol:    microsandbox.PolicyProtocolUDP,
                Port:        "53",
            },
        },
    }),
...
```go Go
sb, err := microsandbox.CreateSandbox(ctx, "api",
    microsandbox.WithImage("python"),
    microsandbox.WithPorts(map[uint16]uint16{8080: 80}),
    microsandbox.WithPortsUDP(map[uint16]uint16{5353: 5353}),
)
```
...
```go
sb, err := microsandbox.CreateSandbox(ctx, "dev-agent",
    microsandbox.WithImage("python"),
    microsandbox.WithNetwork(&microsandbox.NetworkConfig{
        DefaultEgress: microsandbox.PolicyActionDeny,
        Rules: []microsandbox.PolicyRule{
            {Action: microsandbox.PolicyActionAllow, Direction: microsandbox.PolicyDirectionEgress, Destination: "public"},
            {Action: microsandbox.PolicyActionAllow, Direction: microsandbox.PolicyDirectionEgress, Destination: "host"},
        },
    }),
)
```
