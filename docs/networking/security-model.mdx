...
Rebind protection catches this at the response level: when the DNS interceptor sees an answer resolving a domain to a private, loopback, or link-local IP, the response is rewritten to `NXDOMAIN` and the connection never gets made. This is on by default. You can turn it off when you genuinely need domains to resolve to internal hosts (local development, split-horizon DNS):
...
network := &m.NetworkConfig{
    DNS: &m.DNSConfig{RebindProtection: &rebindProtection},
}
```
...
The same mechanism gates host-scoped secret injection. When a `SecretEntry` is restricted via `allowed_hosts`, the secret is only injected when both checks pass: the TLS SNI matches an allowed host pattern, and the original guest destination IP is in the DNS pin set for that host. `HostPattern::Any` is the explicit opt-out from host pinning. An exfiltration attempt that hard-codes an arbitrary IP and claims an allowed SNI won't receive the injection in the first place.

## SNI spoofing

A TLS client picks the SNI in its ClientHello — it's plaintext, not signed by anything. Some servers don't reject a mismatched SNI: they complete the handshake against their default certificate anyway. That gives a workload a way to claim an allowed name in the ClientHello while actually connecting somewhere else.

Concretely: the guest opens a TCP connection to some IP, sends `SNI=allowed.com`, and the lax server on the other end accepts the handshake against whatever cert it has. The policy saw `allowed.com` and would otherwise let it through.

To prevent this, domain rules check two things when SNI is present, and both must pass:

1. The SNI matches the rule's `Domain` (or fits the suffix for `DomainSuffix`).
2. The destination IP is bound to the claimed name in the resolved-hostname pin set — the same set the [TOCTOU defense](#dns-to-ip-binding-toctou) uses.

Real traffic clears check 2 because the client calls `getaddrinfo` before connecting, and the interceptor records the answer. The spoof fails it because no DNS lookup ever tied the claimed name to that IP.

One legitimate pattern this rejects: hard-coding an IP and forging a non-matching SNI (e.g. `curl --resolve allowed.com:443:<ip>`). Route the lookup through the gateway, or add a `tls.bypass` entry for that name.

For intercepted HTTP traffic, microsandbox also checks the decrypted request authority: each HTTP/1 request's `Host` header, and each observed HTTP/2 request's `:authority` pseudo-header, must match the TLS SNI, ignoring case and an optional port. This closes domain-fronting attempts where the TLS layer claims one host but the HTTP request is addressed to another. Bypassed TLS remains opaque, so this request-level check only applies to intercepted connections.
