...
Rebind protection catches this at the response level. When the DNS interceptor sees an answer resolving a domain to a private, loopback, or link-local IP, it rewrites the response to `NXDOMAIN` and the connection never gets made. This is on by default. You can turn it off when you genuinely need domains to resolve to internal hosts (local development, split-horizon DNS):
