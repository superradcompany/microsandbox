...
For intercepted HTTP/1 traffic, microsandbox also checks the decrypted request authority: each request's `Host` header must match the TLS SNI, ignoring case and an optional port. This closes domain-fronting attempts where the TLS layer claims one host but the HTTP request is addressed to another. Bypassed TLS remains opaque, so this request-level check only applies to intercepted connections.
