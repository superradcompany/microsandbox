...
A DNS query is itself an egress action that the policy evaluates before the forwarder will answer. The walk runs once per query, against the transport the guest used to send it (UDP/53, TCP/53, or DoT TCP/853). It returns the action of the first matching rule, or `default_egress` if nothing matches:
...
Values can be plain IPs, `IP:PORT`, hostnames, or `HOST:PORT`. microsandbox resolves hostnames once at sandbox startup using the host's OS resolver.
