...
Order matters. Resolvers are tried in the order listed, and the sandbox moves on to the next one when a query times out or the transport fails, so put your preferred resolver first. A resolver that answers is used as-is, including when it answers `SERVFAIL` or `REFUSED` — those are answers, not failures, so they do not trigger a retry against the next resolver. The same ordering applies to resolvers discovered from the host. Note that each unreachable resolver costs up to `query_timeout_ms` before the next is tried.
