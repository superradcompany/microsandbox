...
1. **DNS resolution** — denied names get a local `NXDOMAIN` response (a definitive negative, so stub resolvers fail fast instead of hanging); the upstream resolver never sees them.
