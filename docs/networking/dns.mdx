...
DNS over TLS uses TCP port `853` and needs its own allow rule. It also requires [TLS inspection](/networking/tls).
...
| DNS over TLS on TCP port `853` | Intercepted when TLS inspection is enabled |
