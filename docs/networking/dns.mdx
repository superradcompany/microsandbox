...
Under a deny-by-default policy whose rules are all IP-based (`Cidr` / non-`Host` `Group`), nothing matches at DNS-decision time and every query is refused. The default `public_only` and `non_local` presets ship with an explicit `Group::Host udp/53 + tcp/53 allow` rule (available as `Rule::allow_dns()` in Rust) so DNS works under those policies; custom deny-by-default policies need an equivalent allow.
