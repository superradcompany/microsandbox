...
- **Bypassed traffic is opaque.** Policy checks that depend on inspecting request content (URL-based rules, secret injection with `require_tls_identity`, and HTTP `Host` / SNI alignment checks) don't apply on bypassed domains.
