...
<Tooltip tip="TLS inspection and host CA trust are not available on microsandbox cloud."><span className="msb-badge-local">Local-only <Icon icon="circle-info" size={11} /></span></Tooltip>

TLS inspection lets microsandbox inspect HTTPS traffic. This enables URL policy checks, request logging, and secure secret injection for matching hosts.
...
from microsandbox import (
    Network, Sandbox, ScopedUpstreamCACert, ScopedVerifyUpstream, TlsConfig,
)

sb = await Sandbox.create(
    "agent",
    image="python",
    network=Network(tls=TlsConfig(
        upstream_ca_certs=("/etc/ssl/corp-root.pem",),
        scoped_upstream_ca_certs=(
            ScopedUpstreamCACert("api.internal.example.com", "./certs/api-ca.pem"),
        ),
...
sb, err := m.CreateSandbox(ctx, "agent",
    ...
    m.WithNetwork(&m.NetworkConfig{
        TLS: &m.TLSConfig{
            UpstreamCACerts: []string{"/etc/ssl/corp-root.pem"},
            ScopedUpstreamCACerts: []m.ScopedUpstreamCACert{
                {Pattern: "api.internal.example.com", Path: "./certs/api-ca.pem"},
            },
            ScopedVerifyUpstream: []m.ScopedVerifyUpstream{
...
TLS inspection covers configured ports, with port `443` enabled by default. TLS inspection and host CA trust solve different problems:

| Feature | Covers |
| --- | --- |
| TLS inspection | Configured TLS ports and hosts that are not bypassed |
...
For exact TLS and network APIs, see [TypeScript](/sdk/typescript/networking#tlsbuilder), [Rust](/sdk/rust/networking#tlsbuilder), [Python](/sdk/python/networking#tlsconfig), or [Go](/sdk/go/networking#tlsconfig). For CLI fields and flags, see [Sandbox configuration](/cli/configuration#network) and [Sandbox commands](/cli/sandbox-commands).

## Related

- [DNS](/networking/dns) explains DNS policy and DNS over TLS.
- [Proxies](/networking/outbound-proxy) explains routing outbound TCP and non-DNS UDP through an external SOCKS proxy.
