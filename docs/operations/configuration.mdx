---
title: Configuration
"og:title": Microsandbox configuration precedence
description: Understand how global defaults, sandbox YAML, CLI flags, SDK inputs, and managed settings determine sandbox configuration.
icon: "sliders"
---
...
or an SDK. Administrator-managed settings apply above these sources. Which sources apply depends on how the sandbox is created, so the
...
<Tabs>
  <Tab title="Command line">
    <Steps>
      ...
      <Step title="Global sandbox defaults">
        ...
        These defaults apply to local and cloud creation, subject to backend support.
        Cloud workers supply image metadata and service-side defaults.
      ...
      <Step title="CLI input">
        Explicit flags and positional arguments override user files.
        Use them to specialize a file for one invocation.

        [Sandbox commands and flags](/cli/sandbox-commands)
      </Step>
      <Step title="Managed overrides">
        Administrator settings apply last and override user files and explicit
        CLI or SDK input. See [Managed deployment](/enterprise/managed-configuration).
      </Step>
    </Steps>
  </Tab>
  ...
  <Tab title="SDKs">
    <Steps>
      ...
      <Step title="Global sandbox defaults">
        Local and cloud backends automatically load `sandbox_defaults` from
        `~/.microsandbox/config.json`, subject to backend support.

        [Global sandbox defaults reference](/configuration#sandbox_defaults)
      </Step>

      <Step title="SDK input">
        Explicit arguments, options, or builder methods override user defaults.
...
| Owned by a user | [Global `config.json`](/configuration) | New local and cloud sandboxes, subject to backend support |
| Enforced by an administrator | [Managed configuration](/enterprise/managed-configuration) | Supported settings on the device, including cloud requests |

## References

<CardGroup cols={2}>
  <Card title="Global configuration" icon="list" href="/configuration">
