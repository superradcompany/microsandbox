...
<Note>
  This page covers the global JSON configuration. For reusable per-sandbox YAML loaded with `--conf`, see [Configuration file](/cli/configuration). For every global field and accepted value, use the [Global config reference](/configuration). To enforce administrator settings across employee devices, use [Managed deployment](/enterprise/managed-configuration).
</Note>
...
| --- | --- | --- |
| Sandbox defaults | CPU, memory, shell, and workdir for new sandboxes | Local and cloud, subject to backend support |
| Host policy | Deployment isolation, CPU and NUMA placement, and block writeback | Local |
| Registry access | Credentials, private CAs, and plain-HTTP registry exceptions for host image pulls | Local; cloud uses explicit or cloud-stored credentials |
| Backend profiles | Named local or cloud targets and API-key references | Everywhere |
| Host integration | Data paths, database pooling, and metrics registry capacity | Local |
| SSH sessions | Inactivity timeout for CLI and SDK connections | Local and cloud |
...
This selects local by default and gives new local and cloud sandboxes two vCPUs, 1 GiB of memory, and Bash, unless overridden. Select the cloud profile for one command without editing the file:
...
For most sandbox defaults, values resolve in this order:
...
microsandbox resolves these values before creating a local sandbox or sending a cloud create request. A new CLI invocation or newly constructed backend picks up file changes; existing sandbox records keep their settings. Use `msb inspect <name> --format json` to inspect a sandbox's effective configuration.
