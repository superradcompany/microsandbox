...
```sh
msb snap create office-tools --sandbox office-base --integrity
```

Verify the snapshot before using it:

```sh
msb snap verify office-base:office-tools
```
...
<CodeGroup>
```sh macOS & Linux
msb snap restore office-base:office-tools --name document-worker \
  --cpus 2 --memory 2G \
...
```sh
msb snap rm office-base:office-tools
```
