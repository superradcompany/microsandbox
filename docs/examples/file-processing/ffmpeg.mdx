...
```sh
msb snap create ffmpeg-tools --sandbox ffmpeg-base --integrity
```

Verify the snapshot before using it:

```sh
msb snap verify ffmpeg-base:ffmpeg-tools
```
...
<CodeGroup>
```sh macOS & Linux
msb snap restore ffmpeg-base:ffmpeg-tools --name ffmpeg-worker \
  --cpus 2 --memory 2G \
...
```sh
msb snap rm ffmpeg-base:ffmpeg-tools
```
