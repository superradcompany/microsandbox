...
<CodeGroup>
```sh macOS & Linux
msb snap create scrapy-runtime \
...
```sh
msb snap verify scrapy-base:scrapy-runtime
```
...
<CodeGroup>
```sh macOS & Linux
msb snap restore scrapy-base:scrapy-runtime --name scrapy-books \
...
```sh
msb snap rm scrapy-base:scrapy-runtime
```
