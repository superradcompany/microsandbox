...
Stream the committed project tree into the worker:
...
Rootfs patches such as `--copy-dir` cannot be combined with `--from-snapshot`, so the worker boots first and receives the committed tree through a stream. `git archive` excludes `.git`, checkout credentials, and untracked files such as a local `.env`; review the committed tree for secrets before sending it. Initializing a new repository inside the worker preserves useful diff workflows without copying host remotes or credentials. Each streamed or interactive workload sets its own process, file-descriptor, and per-file limits.
