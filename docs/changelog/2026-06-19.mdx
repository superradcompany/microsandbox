...
description: "Local and cloud backend routing in the SDKs, detached image init entrypoints, upper disk usage metrics, runtime-owned ephemeral cleanup, shared sandbox spec types, secret substitution through HTTP CONNECT tunnels and plain HTTP, and snapshot import hardening."
icon: "bolt"
---

<Tip>
  **Released this week:** [v0.5.8](https://github.com/superradcompany/microsandbox/releases/tag/v0.5.8)
</Tip>
...
Sandbox metrics now include optional upper filesystem usage fields for OCI-backed roots: `upper_used_bytes`, `upper_free_bytes`, and `upper_host_allocated_bytes`. The CLI exposes them in `msb metrics --format json`, the Rust, Python, TypeScript, and Go SDKs surface them on sandbox metrics objects, and the metrics collector exports matching OTel gauges for live monitoring. The runtime starts sampling after `core.ready`, so early boot does not poll krun metrics before the guest metrics path is available.

```json
{
  "upper_used_bytes": 53248,
  "upper_free_bytes": 450527232,
  "upper_host_allocated_bytes": 2506752
}
```

See the [sandbox metrics guide](/sandboxes/metrics).

**Runtime-owned ephemeral sandbox cleanup**

Unnamed `msb run` sandboxes are now treated as ephemeral, including detached runs, and the host runtime owns their cleanup after the VM exits. Runtime startup also performs a bounded maintenance sweep for stale active rows and missed ephemeral cleanup, so one-off sandboxes do not depend on the CLI process staying alive. SDK callers can opt into the same policy explicitly with the Rust builder's `ephemeral(true)` setting.

```rust
let sandbox = Sandbox::builder("one-off")
    .image("alpine")
    .ephemeral(true)
    .build()
    .await?;
...
See the [sandbox lifecycle guide](/sandboxes/lifecycle).
...
- Detached `msb run -- CMD` now persists the startup command as runtime-owned startup intent, runs it after `agentd` is ready, and shuts down the VM when that startup workload exits instead of dropping the command when the CLI exits.
