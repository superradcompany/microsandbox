...
- Sandbox names longer than 64 bytes no longer fail to boot with `sethostname: EINVAL`. The spawn path derives a UTS-compatible 64-byte hostname when no explicit hostname is set, while still honoring the 128-byte sandbox name limit.
