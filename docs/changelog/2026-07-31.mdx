...
`msb completion <shell>` prints a completion script for `bash`, `zsh`, `fish`, `elvish`, or `powershell` to stdout, so you or a packager can wire up tab completion without a helper package.
