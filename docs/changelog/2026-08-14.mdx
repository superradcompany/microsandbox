...
```python
from microsandbox import Sandbox, Network, NetworkRateLimiter, RateBucket
...
sb = await Sandbox.create(
    ...
    network=Network(
        rate_limiter=NetworkRateLimiter(
            egress=RateBucket(bandwidth_bps=10_000_000),
        ),
    ),
)
```
...
See [CPU placement](/sandboxes/cpu-placement).
