...
You can also supply a unique member name. If you need to reuse an existing name, explicitly remove that member first using [`Snapshot.remove()`](/sdk/python/snapshots#snapshot-remove). Removal still checks for dependent snapshots, and deleting a member does not preserve it if the next capture fails.
