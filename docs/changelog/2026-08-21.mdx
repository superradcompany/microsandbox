...
- **Precompiled Ruby platform gems.** `gem install microsandbox` now pulls a prebuilt binary on `x86_64-linux-gnu`, `aarch64-linux-gnu`, and `arm64-darwin` for Ruby 3.1 through 3.4. On those targets you no longer need a Rust toolchain to install. musl, Windows, and other platforms continue to build from source.
