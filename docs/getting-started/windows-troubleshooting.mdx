...
Windows Terminal, PowerShell, and cmd are supported for normal interactive attach flows. Run interactive commands from a real console, not from a redirected CI shell or background task. This lets `msb` read console input, enter terminal mode, forward resize events, and restore the console when the session exits.
