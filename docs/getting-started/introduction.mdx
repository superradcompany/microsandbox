...
Each sandbox is a lightweight VM with its own Linux kernel, filesystem, and network boundary. Start one locally with the `msb` CLI or an SDK, or run it on [microsandbox cloud](/cloud/overview) with an API key. Either way, you decide what the sandbox can access.
