...
```go Go
sb, err := m.CreateSandbox(ctx, "sandbox",
    m.WithImage("python"),
    m.WithMemory(512),
    m.WithCPUs(2),
    m.WithSecrets(
...
sb, err := m.CreateSandbox(ctx, "sandbox",
    ...
    m.WithNetwork(&m.NetworkConfig{
        ...
        Rules: []m.PolicyRule{
            {Action: m.PolicyActionAllow, Direction: m.PolicyDirectionEgress, Destination: "api.openai.com"},
            {Action: m.PolicyActionAllow, Direction: m.PolicyDirectionEgress, Destination: "pypi.org"},
        },
    }),
)
```
...
// directory directly:
sb, err := m.CreateSandbox(ctx, "encrypted",
    m.WithImage("python"),
    m.WithMounts(map[string]m.MountConfig{
        "/secrets": m.Mount.Bind("/data/secrets", m.MountOptions{}),
    }),
)
```
...
```go Go
sb, err := m.CreateSandbox(ctx, "baseline", m.WithImage("python"))
...
// Boot 10 workers from the same snapshot
workers := make([]*m.Sandbox, 10)
for i := range workers {
    workers[i], err = m.CreateSandbox(ctx,
...
// Planned shape:
//
// sub, err := sb.OnEvent("task.progress", func(event m.EventData) {
