...
```go Go
sb, err := microsandbox.CreateSandbox(ctx, "sandbox",
    microsandbox.WithImage("python"),
    microsandbox.WithMemory(512),
    microsandbox.WithCPUs(2),
    microsandbox.WithSecrets(
        microsandbox.Secret.Env("OPENAI_API_KEY", os.Getenv("OPENAI_API_KEY"),
...
- **Multi-language SDKs.** Rust, TypeScript, Python, and Go, all with consistent APIs.
...
sb, err := microsandbox.CreateSandbox(ctx, "sandbox",
    ...
    microsandbox.WithNetwork(&microsandbox.NetworkConfig{
        ...
        Rules: []microsandbox.PolicyRule{
            {Action: microsandbox.PolicyActionAllow, Direction: microsandbox.PolicyDirectionEgress, Destination: "api.openai.com"},
            {Action: microsandbox.PolicyActionAllow, Direction: microsandbox.PolicyDirectionEgress, Destination: "pypi.org"},
        },
    }),
)
```
...
```go Go
// Filesystem hooks are coming soon for Go. For now, mount the host
// directory directly:
sb, err := microsandbox.CreateSandbox(ctx, "encrypted",
    microsandbox.WithImage("python"),
    microsandbox.WithMounts(map[string]microsandbox.MountConfig{
        "/secrets": microsandbox.Mount.Bind("/data/secrets", microsandbox.MountOptions{}),
    }),
)
```
...
```go Go
sb, err := microsandbox.CreateSandbox(ctx, "baseline", microsandbox.WithImage("python"))
_, err = sb.Exec(ctx, "pip", []string{"install", "-r", "requirements.txt"})
_, err = sb.StopAndWait(ctx)
_ = sb.Close()

h, err := microsandbox.GetSandbox(ctx, "baseline")
_, err = h.Snapshot(ctx, "after-pip-install")

// Boot 10 workers from the same snapshot
workers := make([]*microsandbox.Sandbox, 10)
for i := range workers {
    workers[i], err = microsandbox.CreateSandbox(ctx,
        fmt.Sprintf("worker-%d", i),
        microsandbox.WithSnapshot("after-pip-install"),
    )
}
```
...
```go Go
// Events are coming soon for Go.
// Planned shape:
//
// sub, err := sb.OnEvent("task.progress", func(event microsandbox.EventData) {
//     fmt.Println(event.Data)
// })
// err = sb.Emit(ctx, "task.start", map[string]any{
//     "input_file": "/data/input.txt",
//     "output_dir": "/data/output",
// })
```
