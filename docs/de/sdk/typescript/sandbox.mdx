...
Get a filesystem handle for reading and writing files inside the running sandbox. See [Filesystem](/de/sdk/typescript/filesystem) for the full API.

**Returns**

| Type | Description |
|------|-------------|
| [`SandboxFs`](/de/sdk/typescript/filesystem) | Filesystem handle |
...
| Type | Description |
|------|-------------|
| [`ExitStatus`](/de/sdk/typescript/execution#exitstatus) | Exit code and success flag |
...
| `init(cmd, args?)` | Hand off PID 1 to `cmd` (absolute path or `"auto"`) after agentd setup. See [Custom init system](/de/sandboxes/customize#custom-init-system) |
...
| `volume(guestPath, m => ...)` | Mount a volume — see [Volumes](/de/sdk/typescript/volumes) |
| `patch(p => ...)` | Modify the rootfs before boot — see [Snapshots](/de/sdk/typescript/snapshots) |
...
| `network(n => ...)` | Configure DNS / TLS / policy / secrets — see [Networking](/de/sdk/typescript/networking) |
| `secret(s => ...)` | Add a secret via [`SecretBuilder`](/de/sdk/typescript/secrets#secretbuilder) |
