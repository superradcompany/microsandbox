...
Configured via [`NetworkBuilder::on_secret_violation()`](/ru/sdk/rust/networking#on_secret_violation). Determines what happens when the guest sends a request containing a secret placeholder to a host that is **not** in the secret's allow list.
