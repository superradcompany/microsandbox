...
| Field | Default | Description |
|-------|---------|-------------|
| `block_writeback_limit` | `"auto"` | On Linux, `auto` prefers a 512 MiB VMM-enforced hard budget for each writable raw disk and shrinks it when the derived host pool is smaller; `{"mib": N}` sets a fixed per-disk budget of at least 128 MiB; `off` disables the bound without weakening the guest flush durability boundary. Other hosts resolve an unmodified `auto` to `off` and reject explicit active policies. |
