...
| Field | Default | Description |
|-------|---------|-------------|
| `block_writeback` | `{ "mode": "auto" }` | Buffered host writeback containment and admission. `auto` uses the measured 1536 MiB per-disk window and a derived aggregate pool on Linux. On macOS and Windows, default `auto` is a no-op because this host-page-cache controller is Linux-specific. |

### `runtime.block_writeback`

| Field | Modes | Default | Description |
|-------|-------|---------|-------------|
| `mode` | all | `"auto"` | `auto` uses the measured 1536 MiB per-disk window, `fixed` requires `per_disk_mib`, and `off` disables both the VMM hard bound and host-global admission. |
