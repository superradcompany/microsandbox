...
| `block_writeback_pool_mib` | `null` | Optional aggregate dirty-credit admission pool. With `auto`, `null` derives the pool as the lower of 10% of physical RAM and a conservative userspace estimate of Linux's configured dirty-background threshold, using the exact byte threshold when configured or the ratio applied to current `MemAvailable`. A fixed per-disk limit stays unpooled unless this field is set. The pool must fit at least one per-disk budget. |
