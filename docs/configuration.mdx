...
| Field | Default | Description |
|-------|---------|-------------|
| `block_writeback` | `{ "mode": "auto" }` | Buffered host writeback containment and admission. `auto` uses up to the measured 1536 MiB per-disk window, bounded by the aggregate pool, on Linux. On macOS and Windows, default `auto` is a no-op because this host-page-cache controller is Linux-specific. |

### `runtime.block_writeback`

| Field | Modes | Default | Description |
|-------|-------|---------|-------------|
| `mode` | all | `"auto"` | `auto` uses the smaller of the measured 1536 MiB per-disk ceiling and the available aggregate pool, `fixed` requires an exact `per_disk_mib`, and `off` disables both the VMM hard bound and host-global admission. |
