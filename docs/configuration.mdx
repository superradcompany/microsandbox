...
| Field | Default | Description |
|-------|---------|-------------|
| `block_writeback_limit` | `"auto"` | On Linux, `auto` gives each writable raw disk a 512 MiB VMM-enforced hard budget for buffered host dirty data; `off` disables the bound without weakening the guest flush durability boundary. Other hosts resolve `auto` to `off`. |
