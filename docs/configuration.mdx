...
| Field | Default | Description |
|-------|---------|-------------|
| `msb` | `{home}/bin/msb` | `msb` binary. Set this together with `libkrunfw`; runtime resolution returns a matched pair or fails closed. |
...
| `agentd` | embedded in `msb` | Linux guest Agentd executable. `MSB_AGENTD_PATH` takes precedence; an explicitly configured missing or invalid file fails closed. |
