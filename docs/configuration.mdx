...
| `metrics_sample_interval_ms` | `1000` | Runtime metrics sampling interval in milliseconds. `0` disables sampling entirely (no `sandbox_metric` rows; `Sandbox.metrics()` returns `MetricsDisabled`). |
...
<Note>
  Very small intervals are accepted but will produce a heavy stream of
  `sandbox_metric` rows. The default `1000` ms is recommended for most
  workloads; intervals below `500` ms are discouraged.
</Note>
