...
| Field | Default | Description |
|-------|---------|-------------|
| `block_writeback_limit` | `"off"` | `off` leaves the hard host-dirty bound and aggregate admission disabled without weakening the guest flush durability boundary. On Linux, explicit `auto` prefers a 512 MiB VMM-enforced hard budget for each writable raw disk and shrinks it when the derived host pool is smaller; `{"mib": N}` sets a fixed per-disk budget of at least 128 MiB. Other hosts reject explicit active policies. |
| `block_writeback_pool_mib` | `null` | Optional aggregate dirty-credit admission-pool override. Whenever `block_writeback_limit` is active, `null` derives the pool as the lower of 10% of physical RAM and a conservative userspace estimate of Linux's configured dirty-background threshold, using the exact byte threshold when configured or the ratio applied to current `MemAvailable`. An explicit value replaces that derived capacity. The pool must fit at least one per-disk budget. |
