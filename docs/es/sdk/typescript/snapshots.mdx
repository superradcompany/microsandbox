...
Disk-only snapshots of a stopped sandbox. See [Snapshots](/es/sandboxes/snapshots) for concepts and walkthroughs; this page is the TypeScript SDK reference.
...
Boot a fresh sandbox from a snapshot artifact. Mutually exclusive with [`image()`](/es/sdk/typescript/sandbox#image) — the snapshot already pins the image.
