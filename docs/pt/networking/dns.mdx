---
title: "DNS"
description: "Resolução DNS e aplicação de políticas"
...
The knobs below cover the day-to-day controls. For the rebinding and TOCTOU protections that also ride on the interceptor, see the [security model](/pt/networking/security-model).
...
- DNS over UDP (UDP/53)
- DNS over TCP (TCP/53)
- DNS over TLS (DoT, TCP/853): requires [TLS interception](/pt/networking/tls)
...
- [Security model](/pt/networking/security-model): rebinding protection and DNS-to-IP binding (TOCTOU defense)
- [TLS interception](/pt/networking/tls): domains that get intercepted also go through TLS inspection
