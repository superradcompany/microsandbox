---
title: "Visão geral"
description: "Políticas de rede, DNS e interceptação TLS"
icon: "network-wired"
---
...
The builder accepts string inputs (`.ip("10.0.0.5")`, `.cidr("10.0.0.0/24")`, `.domain("api.example.com")`, etc.) and parses them at `.build()` time, so the chain stays clean. See the [Rust reference](/pt/sdk/rust/networking) for the full surface (per-category shortcuts, the explicit-rule sub-builder, shadow-rule warnings).
...
Domain rules match flows where the guest resolved the name through the sandbox's DNS interceptor. See [DNS](/pt/networking/dns) for how the resolved-hostname cache works.
...
- **DNS** queries are intercepted on the host side. See [DNS](/pt/networking/dns) for blocking, pinned resolvers, and query timeouts.
...
- [DNS](/pt/networking/dns): domain blocking, pinned nameservers, query timeouts
- [TLS interception](/pt/networking/tls): HTTPS inspection with an auto-generated CA
- [Security model](/pt/networking/security-model): how the stack defends against SSRF, rebinding, and metadata exfiltration
