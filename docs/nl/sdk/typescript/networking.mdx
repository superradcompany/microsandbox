---
title: "Netwerken"
description: "Netwerken-API-referentie"
icon: "network-wired"
---


See [Networking](/nl/networking/overview) for conceptual overview and [TLS Interception](/nl/networking/tls) for TLS proxy details.
...
Allow the `Loopback` group (`127.0.0.0/8`, `::1`). The **guest's own** loopback, not the host. To reach a service on the host's localhost, use [`allowHost()`](#allowhost) instead. See the [loopback-vs-host watch-out](/nl/networking/overview#loopback-vs-host-a-common-trap).
...
Set the action taken when a secret reaches a disallowed host. See [`ViolationAction`](/nl/sdk/typescript/secrets#violationaction).
...
Add a secret. See [`SecretBuilder`](/nl/sdk/typescript/secrets#secretbuilder).
...
| secretViolation | [`ViolationAction`](/nl/sdk/typescript/secrets#violationaction) ` \| null` | Action on disallowed secret use |
