...
Disk-only snapshots of a stopped sandbox. See [Snapshots](/nl/sandboxes/snapshots) for concepts and walkthroughs; this page is the Rust SDK reference.
...
Boot a fresh sandbox from a snapshot artifact. Mutually exclusive with [`image()`](/nl/sdk/rust/sandbox#image) and [`image_with()`](/nl/sdk/rust/sandbox#image-with) — the snapshot already pins the image.
