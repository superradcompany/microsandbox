---
title: "ネットワーキング"
description: "ネットワーキング API リファレンス"
icon: "network-wired"
---


See [Networking](/ja/networking/overview) for conceptual overview and [TLS Interception](/ja/networking/tls) for TLS proxy details.
...
Allow the `Loopback` group (`127.0.0.0/8`, `::1`). The **guest's own** loopback, not the host. To reach a service on the host's localhost, use [`allowHost()`](#allowhost) instead. See the [loopback-vs-host watch-out](/ja/networking/overview#loopback-vs-host-a-common-trap).
...
Set the action taken when a secret reaches a disallowed host. See [`ViolationAction`](/ja/sdk/typescript/secrets#violationaction).
...
Add a secret. See [`SecretBuilder`](/ja/sdk/typescript/secrets#secretbuilder).
...
| secretViolation | [`ViolationAction`](/ja/sdk/typescript/secrets#violationaction) ` \| null` | Action on disallowed secret use |
