...
```bash
$ msb run jrei/systemd-debian:12 --init /lib/systemd/systemd -- cat /proc/1/comm
systemd
```

### Argv and env

Pass extra argv to the init via `init_with` (Rust/TypeScript) or by giving `init=` a tuple/dict (Python). On the CLI, repeat `--init-arg` once per entry and `--init-env KEY=VAL` for env vars. Argv defaults to `[<program>]` when no `--init-arg` is given; env is merged on top of the inherited environment.
