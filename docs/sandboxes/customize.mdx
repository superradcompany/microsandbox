...
`clone=auto` prefers a native copy-on-write clone and falls back to a sparse copy. It uses `FICLONE` on Linux, `clonefile` on macOS, and block cloning on Windows volumes that advertise block refcounting, including supported ReFS and Dev Drive configurations. Choose `copy` for deterministic cross-filesystem behavior, or `reflink` when sharing extents is mandatory and failure is preferable to copying. Strict `reflink` never silently copies the full image; `auto` remains safe on ext4, NTFS, cross-volume destinations, and other unsupported filesystems. Flat ext4 roots support stopped-sandbox snapshots and restore; rootfs patches and cloud execution remain unsupported. The layered root remains the default for compatibility.

See [Performance optimization](/optimization/overview#storage-layout-layered-or-flat) for workload tradeoffs, host filesystem expectations, and how to verify the clone strategy actually used.
