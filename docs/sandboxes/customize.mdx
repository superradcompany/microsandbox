...
OCI sandboxes use the layered EROFS plus OverlayFS layout by default. Flat layout is an explicit alternative that materializes the merged image into one ext4 base, creates a private sparse or reflink clone for the sandbox, grows that clone to the requested root-disk capacity, and mounts it directly without guest OverlayFS.
...
let sb = Sandbox::builder("worker")
    .image_with(|image| image.oci("python:3.12").flat().root_disk(8192))
    .create()
    .await?;
```
...
sb = await Sandbox.create(
    "worker",
    image=Image.oci("python:3.12", layout="flat", root_disk=RootDisk.managed(8192)),
)
```
...
sb, err := m.CreateSandbox(ctx, "worker",
    ...
    m.WithRootfsLayout(m.RootfsLayoutFlat),
    m.WithRootDisk(m.RootDisk.Managed(8192)),
)
```
</CodeGroup>

The first flat version supports managed root disks. Rootfs patches, tmpfs or user-supplied root disks, snapshots, and cloud execution are rejected. Layered remains the default for compatibility.
