...
sb, err := microsandbox.CreateSandbox(ctx, "worker",
    ...
    microsandbox.WithPatches(
        ...
        microsandbox.Patch.Text("/etc/motd", "Custom message of the day.\n", microsandbox.PatchOptions{Replace: true}),
        microsandbox.Patch.Mkdir("/app", microsandbox.PatchOptions{Mode: &mode755}),
        microsandbox.Patch.Text("/app/config.json", `{"debug": true}`, microsandbox.PatchOptions{Mode: &mode644}),
        microsandbox.Patch.CopyFile("./cert.pem", "/etc/ssl/cert.pem", microsandbox.PatchOptions{}),
...
- Go: [`Patch`](/sdk/go/sandbox#patch) (factory) and [`PatchConfig`](/sdk/go/sandbox#patchconfig) (the value type)
...
```go Go
sb, err := microsandbox.CreateSandbox(ctx, "worker",
    microsandbox.WithImage("ghcr.io/superradcompany/debian-systemd:12"),
