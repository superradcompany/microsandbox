...
Resize an OCI sandbox's root disk through `modify`:
...
- After a full snapshot has rolled a root disk onto a qcow2 chain, root-disk growth is refused for now. Microsandbox will not resize the chain's sealed raw ancestor; chain-aware online growth is tracked separately.
