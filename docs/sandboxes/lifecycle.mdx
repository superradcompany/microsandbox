...
`msb restart` follows the same lifecycle semantics as `msb stop` followed by `msb start`. If the sandbox is already stopped or crashed, it starts it directly. Restart is experimental and requires `MSB_EXPERIMENTAL_MODIFY=1` in the environment.

## Ping and touch

<Note>
Experimental: ping and touch require `MSB_EXPERIMENTAL_MODIFY=1` in the environment, in both the CLI and the SDKs.
</Note>
