...
`msb modify` and the SDK `modify()` methods change a running sandbox in place: CPUs, memory, env vars, labels, workdir, and secrets. Changes apply all-or-nothing.

CPU and memory resize live within the `max_cpus` / `max_memory` ceilings set at create time, so set those above your starting values if the sandbox may need to grow.
...
Add `--dry-run` to preview. Changes that can't apply live are saved for the next start, or applied now with `--restart` (`policy` in the SDKs).

See [`msb modify`](/cli/sandbox-commands#msb-modify) for all flags.
