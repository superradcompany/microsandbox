...
```go Go
// Attached: sandbox stops when your process exits
sb, err := microsandbox.CreateSandbox(ctx, "worker", microsandbox.WithImage("python"))

// Detached: sandbox survives after your process exits
detached, err := microsandbox.CreateSandbox(ctx, "worker",
...
// Later, resume where you left off
sb, err := microsandbox.StartSandbox(ctx, "worker")
```
...
```go Go
// Pause and resume are coming soon for Go.
```
...
```go Go
err := microsandbox.RemoveSandbox(ctx, "worker")
```
...
sb, err := microsandbox.CreateSandbox(ctx, "worker",
    microsandbox.WithImage("python"),
    microsandbox.WithMaxDuration(time.Hour),
    microsandbox.WithIdleTimeout(5*time.Minute),
)
```
