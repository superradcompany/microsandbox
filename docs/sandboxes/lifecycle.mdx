...
// Attached: sandbox stops when your process exits (or scope ends with `await using`).
...
const resumed = await Sandbox.start("worker")
```
...
```typescript TypeScript
// TODO: pause/resume is not yet exposed by the Node.js SDK.
```
...
console.log(exitStatus.code, exitStatus.success)
```
...
console.log(handle.status) // "running" | "stopped" | "crashed" | "draining"
```
