...
When you create a sandbox, microsandbox spawns several processes on the host and boots a guest VM. Understanding what runs where helps make sense of the lifecycle methods and supervisor policies.
...
graph TD
    subgraph Host["Host"]
        ...
        C["msbnet<br/><small>network policy, DNS, port forwarding</small>"]
        D["MicroVM Process<br/><small>msb microvm</small>"]
    end
    ...
    C <-. "socket pair (ethernet frames)" .-> D
    ...
    style C fill:#d4bfff,stroke:#8b5cf6,color:#1a1a1a
...
The **supervisor** is a separate host process that watches over the MicroVM and `msbnet` processes. It handles:
