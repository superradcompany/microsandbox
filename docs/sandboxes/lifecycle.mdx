...
Use `msb modify`, or the SDK `modify()` methods, to change a running sandbox without recreating it. You can resize its CPUs and memory, update environment variables, labels, or the working directory, and add, rotate, or remove secrets. Changes that can apply to a running sandbox take effect immediately; the rest take effect on its next start.
...
See [`msb modify`](/cli/sandbox-commands#msb-modify) and the SDK sandbox reference for all options.
