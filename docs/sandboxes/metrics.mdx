...
<Note>
Need continuous shipping to Grafana, Datadog, Prometheus, or any other OTel-compatible backend? See [Metrics Collection](/observability/metrics-collection/overview) for `msb-metrics`, a sidecar binary that reads the same data and ships it over OTLP.
</Note>
...
Get metrics for all sandboxes with active live metrics slots. Useful for dashboards or capacity planning.
