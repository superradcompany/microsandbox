...
```ruby Ruby
cache = Microsandbox::Volume.builder("pip-cache").directory.create
```
...
```ruby Ruby
docker_data = Microsandbox::Volume.builder("docker-data")
  .disk
  .size(20 * 1024)
  .create
```
...
```ruby Ruby
volumes = Microsandbox::Volume.list
Microsandbox::Volume.remove("old-cache")
```
...
A sandbox can combine host directories, individual files, named volumes, disk images, and tmpfs scratch space. In the CLI, repeat the mount flags.

Ruby currently supports named-volume management, but does not expose sandbox mount configuration.
...
```python Python
from microsandbox import DiskImageFormat, Sandbox, StatVirtualization, Volume
...
sb = await Sandbox.create(
    ...
    volumes={
        "/app/src": Volume.bind("./src", readonly=True, noexec=True),
        "/app/pyproject.toml": Volume.bind("./pyproject.toml", readonly=True),
        "/root/.cache/pip": Volume.named(
            "pip-cache", stat_virtualization=StatVirtualization.RELAXED,
        ),
        "/dataset": Volume.disk(
            "./dataset.qcow2", format=DiskImageFormat.QCOW2,
            fstype="ext4", readonly=True,
        ),
        "/tmp": Volume.tmpfs(size_mib=1024, noexec=True, nosuid=True, nodev=True),
    },
)
```

```go Go
sb, err := m.CreateSandbox(ctx, "full",
    m.WithImage("python"),
    m.WithMounts(map[string]m.MountConfig{
        "/app/src": m.Mount.Bind("./src", m.MountOptions{
            Readonly: true, Noexec: true,
        }),
        "/app/pyproject.toml": m.Mount.Bind("./pyproject.toml", m.MountOptions{
            Readonly: true,
        }),
        "/root/.cache/pip": m.Mount.Named("pip-cache", m.MountOptions{
            StatVirtualization: m.StatVirtualizationRelaxed,
        }),
        "/dataset": m.Mount.Disk("./dataset.qcow2", m.DiskOptions{
            Format: "qcow2", Fstype: "ext4", Readonly: true,
        }),
        "/tmp": m.Mount.Tmpfs(m.TmpfsOptions{
            SizeMiB: 1024, Noexec: true, Nosuid: true, Nodev: true,
        }),
    }),
)
if err != nil {
