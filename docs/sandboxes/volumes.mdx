...
```go Go
sb, err := microsandbox.CreateSandbox(ctx, "dev",
    microsandbox.WithImage("python"),
    microsandbox.WithMounts(map[string]microsandbox.MountConfig{
        "/app/src":  microsandbox.Mount.Bind("./src", microsandbox.MountOptions{Readonly: true}),
        "/app/data": microsandbox.Mount.Bind("./data", microsandbox.MountOptions{}),
    }),
)
```
...
```go Go
cache, err := microsandbox.CreateVolume(ctx, "pip-cache",
    microsandbox.WithVolumeQuota(1024),
)
```
...
sb, err := microsandbox.CreateSandbox(ctx, "worker",
    microsandbox.WithImage("python"),
    microsandbox.WithMounts(map[string]microsandbox.MountConfig{
        "/root/.cache/pip": microsandbox.Mount.Named(cache.Name(), microsandbox.MountOptions{}),
    }),
)
```
...
```go Go
// Writer
writer, err := microsandbox.CreateSandbox(ctx, "writer",
    microsandbox.WithImage("alpine"),
    microsandbox.WithMounts(map[string]microsandbox.MountConfig{
        "/data": microsandbox.Mount.Named("shared-data", microsandbox.MountOptions{}),
    }),
)
_, err = writer.Shell(ctx, "echo 'hello' > /data/message.txt")

// Reader (read-only)
reader, err := microsandbox.CreateSandbox(ctx, "reader",
    microsandbox.WithImage("alpine"),
    microsandbox.WithMounts(map[string]microsandbox.MountConfig{
        "/data": microsandbox.Mount.Named("shared-data", microsandbox.MountOptions{Readonly: true}),
    }),
)
...
```go Go
vol, err := microsandbox.GetVolume(ctx, "pip-cache")
...
```go Go
volumes, err := microsandbox.ListVolumes(ctx)
err = microsandbox.RemoveVolume(ctx, "old-cache")
```
...
sb, err := microsandbox.CreateSandbox(ctx, "worker",
    microsandbox.WithImage("alpine"),
    microsandbox.WithMounts(map[string]microsandbox.MountConfig{
        "/tmp/scratch": microsandbox.Mount.Tmpfs(microsandbox.TmpfsOptions{SizeMiB: 100}),
    }),
)
```
