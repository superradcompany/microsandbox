...
| `max_cpus` | same as `cpus` | Boot-time maximum possible virtual CPUs (experimental, requires `MSB_EXPERIMENTAL_MODIFY=1`) |
| `memory` | `512` | Guest memory limit in MiB |
| `max_memory` | same as `memory` | Boot-time maximum hotpluggable memory in MiB (experimental, requires `MSB_EXPERIMENTAL_MODIFY=1`) |
...
`max_cpus` and `max_memory` are boot-time reservations: they fix the CPU
topology and the hotpluggable memory range the VM boots with, so they are
chosen at create time. Because they default to the effective `cpus` and
`memory` values, a sandbox created without them has no growth headroom — a
later live grow with `msb modify` is impossible without a restart. Reserving
capacity costs almost nothing: vCPUs above the effective count boot offline
and stay parked, and reserved memory is backed lazily so unplugged capacity
uses no host memory. If a sandbox may need to grow, set
`max_cpus`/`max_memory` above the initial values when you create it.
