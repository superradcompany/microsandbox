...
topology and the hotpluggable memory range the VM boots with, so you choose
them at create time. They default to the effective `cpus` and `memory` values,
so a sandbox created without them has no growth headroom. A later live grow
with `msb modify` is impossible without a restart. Reserving capacity costs
almost nothing: vCPUs above the effective count boot offline and stay parked,
and reserved memory is backed lazily so unplugged capacity uses no host
memory. If a sandbox may need to grow, set `max_cpus`/`max_memory` above the
initial values when you create it.
...
Labels also select sandboxes in bulk: `list_with` in the SDKs, and `--label` on the CLI fleet commands. See [Labels](/sandboxes/labels) for the full picture.
