...
```typescript TypeScript
await using sb = await Sandbox.builder("worker").image("./my-rootfs").create()
```
...
```typescript TypeScript
// Auto-detect filesystem from extension
await using sb = await Sandbox.builder("worker")
    .image("./ubuntu-22.04.qcow2")
    .create()

// Explicit filesystem type via a disk mount on the rootfs
await using sb2 = await Sandbox.builder("worker")
    .image("./alpine.raw")
    .create()
```
...
const builder = Sandbox.builder("preview")
    .image("python")
    .memory(1024)
...
await using sb = await builder.create()
```
