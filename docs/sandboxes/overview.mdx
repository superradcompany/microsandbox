...
```go Go
_, err := microsandbox.CreateSandbox(ctx, "worker",
    microsandbox.WithImage("python"),
)
```
...
```go Go
_, err := microsandbox.CreateSandbox(ctx, "worker",
    microsandbox.WithImage("./my-rootfs"),
)
```
...
// Auto-detect the disk image format from the file extension.
_, err := microsandbox.CreateSandbox(ctx, "worker",
    microsandbox.WithImage("./alpine.qcow2"),
)

// Explicit root disk image filesystem selection is coming soon for Go.
// Additional disk-image mounts are available via Mount.Disk.
```
...
sb, err := microsandbox.CreateSandbox(ctx, "worker",
    microsandbox.WithImage("python"),
    microsandbox.WithReplaceWithGrace(30*time.Second),
)
```
...
```go Go
sb, err := microsandbox.CreateSandbox(ctx, "worker",
    microsandbox.WithImage("private.registry.io/org/app:v1"),
    microsandbox.WithRegistryAuth(microsandbox.RegistryAuth{
        Username: "deploy",
...
```go Go
// Config inspection is coming soon for Go.
// Create sandboxes directly for now:
sb, err := microsandbox.CreateSandbox(ctx, "preview",
    microsandbox.WithImage("python"),
    microsandbox.WithMemory(1024),
)
```
