...
Enable only the locations your API needs. Disabling headers also disables Basic authentication substitution. A detected placeholder in a disabled location is blocked unless the destination has explicit passthrough permission. To narrow header substitution to specific fields, set `header_fields` to the allowed field names (for example `[authorization]`); a placeholder in any other header is treated as a disabled location. An empty list keeps the default of every header. On the CLI, use `headers=[authorization,x-api-key]` (or `headers=*` for all headers).

Header scoping is a confidentiality control, not just tidiness. With every header substitutable, the guest can put the placeholder in a header the upstream host reflects back in its response and read the substituted secret from that response. Allow only the header the API actually reads for credentials.
