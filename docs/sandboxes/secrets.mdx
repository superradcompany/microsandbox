...
Let a selected host receive the **unchanged placeholder**, never the credential. For example, allow it to appear in an AI request:
...
## Sandboxes created with v0.6

Saved v0.6 secret settings are migrated automatically when the local catalog is upgraded. Old Basic Auth and header settings are combined: enabling either enables header substitution, including Basic Auth. If the old settings differed, substitution now also applies to the previously disabled kind of header. Query substitution and global or per-secret passthrough policies are retained. You do not need to edit the catalog or recreate the sandbox.

The selected runtime determines enforcement. On v0.7, a placeholder in a disabled substitution location triggers the violation policy unless explicit passthrough permits it. The SDK does not add passthrough permissions to preserve v0.6's previous behavior. v0.7 also requires at least one substitution location to be enabled.

`msb self downgrade` converts saved secret settings back to the target v0.6 format, checking representability before changing the database. A policy combining per-secret passthrough with a separate blocking override cannot be represented by v0.6 and prevents downgrade. Downgrade to v0.7.0–v0.7.2 is refused if saved or active secret settings contain a global passthrough default, which those releases cannot preserve. Installing an older SDK package alone does not downgrade the catalog. Stop processes using an older SDK before upgrading a shared local catalog; an already-open old SDK may not understand migrated settings.
