...
When you bind a secret to an environment variable, microsandbox puts a random placeholder in the guest instead of the real value. If the sandbox sends that placeholder to an allowed host, microsandbox swaps it for the real credential at the network boundary. Anywhere else, the placeholder remains meaningless.
