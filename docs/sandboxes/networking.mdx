...
microsandbox enforces network policies using a dedicated `msbnet` process that handles packet filtering, port forwarding, and DNS interception.
...
<Note>
  DNS interception requires the `msbnet` process, which runs alongside the sandbox's microVM. It handles both DNS queries and packet-level policy enforcement.
</Note>

## How it works

When networking is enabled, microsandbox spawns a `msbnet` process alongside each sandbox's microVM. Network traffic from the sandbox passes through `msbnet`, which enforces packet filtering rules based on the configured policy:
