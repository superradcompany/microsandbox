...
```typescript TypeScript
await using stream = await sb.fs().readStream("/app/data.bin")
...
```typescript TypeScript
// TODO: volume read/write hooks are not yet exposed by the Node.js SDK.
```
...
```typescript TypeScript
// TODO: custom FsBackend is not yet exposed by the Node.js SDK.
```
