...
keywords: ["sandbox filesystem", "read files", "write files", "file operations"]
icon: "folder-open"
---
...
```ruby Ruby
sb.fs.write("/app/config.json", '{"debug": true}')
```
</CodeGroup>
...
```ruby Ruby
content = sb.fs.read("/app/config.json").force_encoding(Encoding::UTF_8)
```
</CodeGroup>
...
```ruby Ruby
sb.fs.list("/app").each do |entry|
  puts "#{entry.fetch("path")}: #{entry.fetch("kind")}"
end
```
</CodeGroup>
...
```ruby Ruby
sb.fs.copy_from_host("./local-file.txt", "/app/remote-file.txt")
```
</CodeGroup>
...
For exact filesystem APIs, see [TypeScript](/sdk/typescript/filesystem), [Rust](/sdk/rust/filesystem), [Python](/sdk/python/filesystem), or [Go](/sdk/go/filesystem). For host-to-sandbox copies, see [`msb copy`](/cli/sandbox-commands#msb-copy).
