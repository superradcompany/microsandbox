...
```ruby Ruby
page = Microsandbox::Sandbox.list(labels: { "app" => "engine" })
```
...
<Tooltip tip="Label selection works on microsandbox cloud except with ping and touch, which are local-only."><span className="msb-badge-limited">Limited on cloud <Icon icon="circle-info" size={11} /></span></Tooltip>
...
Labels flow into a sandbox's metrics as dimensions, so you can break CPU, memory, and I/O down by any label you set, for example per user or per tenant. See [Labels in queries](/observability/msb-metrics#labels-in-queries) for the PromQL detail.
...
For exact label and sandbox-selection APIs, see [TypeScript](/sdk/typescript/sandbox), [Rust](/sdk/rust/sandbox), [Python](/sdk/python/sandbox), or [Go](/sdk/go/sandbox). For label-aware fleet commands, see [Sandbox commands](/cli/sandbox-commands).
