...
```typescript TypeScript
const output = await sb.execWith("python", (e) => e
    .args(["compute.py"])
    .cwd("/app")
    .env("PYTHONPATH", "/app/lib")
    .timeout(30_000)
...
```typescript TypeScript
await using handle = await sb.execStream("tail", ["-f", "/var/log/app.log"])
...
```typescript TypeScript
await using handle = await sb.execStreamWith("python", (e) =>
    e.stdinPipe().tty(true),
)
await using stdin = (await handle.takeStdin())!
...
```typescript TypeScript
// TODO: sessions API is not yet exposed by the Node.js SDK.
```
