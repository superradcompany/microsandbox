...
output, err := sb.Exec(ctx, "python", []string{"compute.py"},
    ...
    microsandbox.WithExecEnv(map[string]string{"PYTHONPATH": "/app/lib"}),
...
```go Go
// Interactive attach is coming soon for Go.
// Use ExecStream for programmatic streaming today.
```
...
```go Go
handle, err := sb.ExecStream(ctx, "python", nil, microsandbox.WithExecStdinPipe())
...
```go Go
// Session listing and reattach are coming soon for Go.
// ExecStream exposes the session ID today:
