...
```ruby Ruby
output = sb.exec("python", ["-c", "print('hello')"])
puts output.stdout     # "hello\n"
puts output.success?   # true
puts output.exit_code  # 0
```
...
```ruby Ruby
output = sb.shell("ls -la /app && echo done")
```
...
<Note>Ruby currently collects command output with `exec` and `shell`. It does not expose streaming handles, interactive attach, or session IDs.</Note>
...
async for event in handle:
    match event.event_type:
        case ExecEventType.STDOUT: print(event.data.decode(), end="")
        case ExecEventType.STDERR: print(event.data.decode(), end="", file=sys.stderr)
        case ExecEventType.EXITED: print(f"Exited: {event.code}")
```
...
exitCode, err = sb.AttachWith(ctx, "python", nil,
...
TypeScript, Rust, Python, and Ruby can pass input bytes with the command. Ruby does not expose a streaming stdin writer; Go uses the pipe API shown below.

<CodeGroup>
```typescript TypeScript
const output = await sandbox.execWith("cat", (e) => e.stdinBytes(Buffer.from("hello\n")));
console.log(output.stdout());
```

```rust Rust
let output = sandbox.exec_with("cat", |e| e.stdin_bytes(b"hello\n".to_vec())).await?;
println!("{}", output.stdout()?);
```

```python Python
output = await sandbox.exec("cat", stdin=b"hello\n")
print(output.stdout_text)
```

```ruby Ruby
output = sandbox.exec("cat", stdin: "hello\n")
puts output.stdout
```
</CodeGroup>
...
For exact execution APIs, see [TypeScript](/sdk/typescript/execution), [Rust](/sdk/rust/execution), [Python](/sdk/python/execution), or [Go](/sdk/go/execution). For command-line execution, see [`msb run`](/cli/sandbox-commands#msb-run) and [`msb exec`](/cli/sandbox-commands#msb-exec).
