...
<Note>
Flat snapshot compaction is **opt in**. Ordinary snapshot creation defaults to `off`, so it does not scan the complete ext4 filesystem or add trim work to the foreground path.
</Note>
