...
- **Reuse source bindings:** `--dangerously-inherit-resources` explicitly allows validated bindings from the source on the same host. Missing dependencies still cause restore to fail.
- **Select a captured disk:** Use `-v /data` to restore a private named disk volume. Arbitrary external disk images are not converted to owned volumes.
- **Allow missing resources:** `--allow-missing-resources` lets a full restore continue without an external filesystem or additional disk. Root and owned storage must still be complete. Access to missing resources returns `EIO`, which can disrupt applications or make filesystems read-only. Warnings remain visible with `--quiet`.

The SDK options for allowing missing resources are TypeScript `.allowMissingResources()`, Rust `.allow_missing_resources()`, Python `allow_missing_resources=True`, and Go `WithAllowMissingResources()`.

Allowing missing resources does not reuse source bindings or accept changed files in a supplied mapping; see [Validate mounts](#validate-mounts). Direct branching keeps its existing behavior for unavailable resources. Disk-only restore boots fresh rather than resuming open filesystem handles.
