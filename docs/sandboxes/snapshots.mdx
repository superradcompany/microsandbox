...
A snapshot is a stopped sandbox's writable layer, captured as a portable directory you can move with `scp`, archive as `.tar.zst`, or boot fresh sandboxes from.

<Note>
Snapshots are **disk-only and stopped-only**: they capture the writable filesystem layer of a sandbox that's not currently running. Live snapshots (memory + CPU state) and qcow2-backed checkpoint chains are future work.
</Note>
...
The snapshot lives at `~/.microsandbox/snapshots/after-pip-install/`.
...
```bash
# scp the directory directly (image must be cached or pullable on the target)
...
let h = Snapshot::import(Path::new("/tmp/snap.tar.zst"), None).await?;
...
const h = await Snapshot.import("/tmp/snap.tar.zst");
```
...
h = await Snapshot.import_("/tmp/snap.tar.zst")  # trailing _ avoids the keyword
```
