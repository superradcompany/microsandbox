...
Flat snapshot creation works on a private staged copy. Microsandbox replays any pending ext4 journal, validates the formatter-owned filesystem metadata and checksums, and then asks the host filesystem to deallocate ext4 blocks proven free. Linux uses hole punching, macOS uses APFS range deallocation, and Windows uses sparse zero-data ranges. If the host filesystem cannot deallocate ranges, snapshot creation retains the correct dense artifact rather than weakening validation or mutating the stopped sandbox disk.
