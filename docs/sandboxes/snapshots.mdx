...
Resolve a handle for the stopped sandbox, then snapshot it under a bare name or to an explicit path:
...
The `export` and `import` calls are exposed in every SDK if you need to drive bundling from code:
