...
<Note>
Keep the default flush policy for normal captures. If you plan to restore a full snapshot's disks without its memory, capture with `--guest-flush required`. This flushes filesystem caches, not application buffers or transactions. See [guest flush options](/cli/sandbox-commands#guest-filesystem-flush) for policy details and paused-sandbox requirements.
</Note>
