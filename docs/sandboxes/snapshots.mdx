...
<Tooltip tip="Cloud supports disk snapshots from stopped persistent sandboxes and disk restore. Full snapshots, branching, groups, and local archives are not yet available on cloud."><span className="msb-badge-limited">Limited on cloud <Icon icon="circle-info" size={11} /></span></Tooltip>
...
Use disk snapshots for installed dependencies, prepared workspaces, and saved files. Cloud requires a stopped persistent source. See [guest flushing](#guest-flushing) for how microsandbox handles pending writes.
...
```ruby Ruby
baseline = Microsandbox::Sandbox.get("baseline")

session = baseline.connect
session.exec("pip", ["install", "requests"])
...
```ruby Ruby
baseline = Microsandbox::Sandbox.get("baseline")
snapshot = baseline.snapshot("ready")
```
...
Snapshots do not include host-mounted files. Supply their host paths when restoring. Owned volumes restore automatically. Full restore requires every captured external filesystem and additional disk to be available.
...
In Go, always receive the result and close its sandbox handle when finished, even if you ignore progress events. Closing a restored handle does not stop the detached sandbox; call `Stop` or `Destroy` when needed. Cancel the context to cancel the operation.
...
```ruby Ruby
snapshots = Microsandbox::Snapshot.list

saved = Microsandbox::Snapshot.open("baseline:ready")
puts saved.digest

Microsandbox::Snapshot.remove("baseline:ready")
```
...
use microsandbox::{snapshot::LoadOpts, Snapshot};
...
use microsandbox::{snapshot::SaveOpts, Snapshot};
...
```ruby Ruby
Microsandbox::Snapshot.save("baseline:ready", "ready.msb", with_image: true)
```
...
use microsandbox::{snapshot::LoadOpts, Sandbox, Snapshot};
