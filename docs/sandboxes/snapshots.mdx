...
Creating a sandbox from the resulting snapshot restores into a child-owned disk generation, installs a fresh VM Generation ID, waits for the guest freshness acknowledgement, thaws the captured workload, and publishes network access last. Restore is eager: the archive or installed artifact is no longer needed after construction succeeds.
