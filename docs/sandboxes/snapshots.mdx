...
| Optional labels and integrity metadata | Network state |
...
Direct capture publishes only the archive and returns its snapshot ID and path. It creates no intermediate snapshot directory and no local index row. Add `--plain-tar` or the corresponding SDK option to skip zstd compression.
