...
```go Go
all, err := microsandbox.Snapshot.List(ctx)            // Indexed snapshots
h, err := microsandbox.Snapshot.Get(ctx, "after-pip-install") // By name, digest, or path
name := "-"
if h.Name() != nil {
    name = *h.Name()
}
...
err = microsandbox.Snapshot.Remove(ctx, "after-pip-install", false)
_, err = microsandbox.Snapshot.Reindex(ctx, "~/.microsandbox/snapshots")
```
...
```go Go
err := microsandbox.Snapshot.Export(ctx,
    "after-pip-install",
    "/tmp/snap.tar.zst",
    microsandbox.SnapshotExportOptions{WithImage: true},
)

snap, err := microsandbox.Snapshot.Import(ctx, "/tmp/snap.tar.zst", "")
```
...
```go Go
// Compute and record an integrity hash at create time
snap, err := microsandbox.Snapshot.Create(ctx, "baseline",
    microsandbox.SnapshotCreateOptions{
        Name:            "after-pip-install",
