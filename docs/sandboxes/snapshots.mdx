...
<Note>
Snapshots are **disk-only and stopped-only**: the sandbox can't be running when you take one.
</Note>
...
```go Go
err := m.Snapshot.Export(ctx,
    "after-pip-install",
    "/tmp/snap.tar.zst",
    m.SnapshotExportOptions{WithImage: true},
)

snap, err := m.Snapshot.Import(ctx, "/tmp/snap.tar.zst", "")
```
