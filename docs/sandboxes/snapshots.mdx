---
title: Snapshots
description: Capture a sandbox's disk or resumable execution state as a portable artifact
icon: "code-branch"
---
...
A snapshot is a portable artifact that can hold either a sandbox's writable disk state or a resumable checkpoint of a running sandbox. Move it with `scp`, archive it as `.tar.zst`, or create a child sandbox from it.

<Note>
Disk snapshots capture stopped or crashed sandboxes. Resumable snapshots use `--resumable` and capture a running sandbox without requiring the user to pause it first.
</Note>
...
| Resumable | Running sandbox | Disk, memory, vCPU, device, and admitted resource state | Eagerly resumes the captured execution in a child VM |
...
Use resumable mode when the child must continue the captured processes and memory instead of cold-booting from disk state:
...
```bash CLI
msb snapshot create worker-checkpoint --from worker --resumable
```
</CodeGroup>
...
Direct capture publishes only the archive and returns its snapshot ID and path. It creates no intermediate snapshot directory and no local index row. Add `--resumable` to capture a running sandbox directly to the same archive, or `--plain-tar` to skip zstd compression.
