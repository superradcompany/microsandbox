...
Use `msb image prune` to remove cached images that are not used by any sandbox and reclaim dangling image artifacts. Prune keeps images referenced by existing sandboxes.
