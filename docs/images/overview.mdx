...
```ruby Ruby
sb = Microsandbox::Sandbox.create("worker", image: "python", pull_policy: "always")
```
</CodeGroup>
...
Per-sandbox SDK options override user registry settings. [Managed registry settings](/configuration#auth-resolution-order) take precedence over both. Managed `insecure: false` requires TLS without changing credentials; managed `auth: null` forces anonymous access, and managed CA settings replace per-call custom certificates.
