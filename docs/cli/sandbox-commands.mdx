...
The pre-pull is optional: creating the first flat sandbox lazily materializes a missing flat artifact from the normal OCI layer cache. `clone=auto` prefers a native copy-on-write clone and falls back to a sparse copy, `clone=copy` forces the portable sparse-copy path, and `clone=reflink` requires reflink support and fails clearly if the destination filesystem cannot provide it. Native cloning uses `FICLONE` on Linux, `clonefile` on macOS, and block cloning on Windows volumes that advertise block refcounting, including supported ReFS and Dev Drive configurations. Flat v1 supports ext4 and stopped-sandbox snapshots; rootfs patches and cloud execution are rejected instead of silently changing their semantics. The normal layered root remains the built-in default, and `sandbox_defaults.oci.root_disk` can select flat for future local sandboxes without repeating the flag.
...
|------------|-------------|
| `doctor` (alias: `check`) | Check runtime files, host virtualization prerequisites, and non-blocking host performance capabilities |
...
See [Performance optimization](/optimization/overview) for interpreting these checks, choosing sandbox-level policies, and preparing the host storage and virtualization environment.
