...
| `--no-exec-log` | Do not record exec output to `exec.log`, not even the workload's |
...
| `--capture` | Record this command's output to `exec.log`, where `msb logs` shows it (off by default) |
| `-q`, `--quiet` | Suppress progress output |
