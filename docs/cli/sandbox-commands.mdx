...
`pause --guest-flush required` flushes captured persistent filesystems before pausing, so a later disk snapshot can reuse that boundary without resuming. The default `auto` and explicit `skip` omit optional writeback; mandatory storage barriers remain. An already-paused VM without the required flush fails rather than resuming implicitly. `resume` has no flush option. See [guest filesystem flush](#guest-filesystem-flush).
...
| `--guest-flush` | Guest writeback policy: `auto` (default), `required`, or `skip` |
...
`--guest-flush auto|required|skip` controls guest filesystem writeback during `msb snap create`, `msb branch`, and `msb pause`. It applies to the root and captured block disks, including owned disks. Host-backed directory synchronization, host disk-I/O draining, and snapshot durability remain enforced with every policy.

| Policy | Running disk-only capture | Full capture, branch, or pause |
| --- | --- | --- |
| `auto` (default) | Require guest writeback | Keep dirty pages in RAM; no optional block-filesystem flush |
...
Use `required` when a full snapshot must also support disk-only restore. It flushes filesystem caches, not application buffers or transactions, and does not make tmpfs persistent. With `skip`, a disk-only snapshot may omit buffered writes and need filesystem recovery. Full snapshots and branches preserve those writes in RAM.

A paused source must have acknowledged any required flush before pausing; capture fails rather than resuming it to flush. To prepare for a disk snapshot:
...
For stopped or crashed sources, `auto` and `skip` capture existing disk state; `required` fails because no guest is running. Older runtimes that cannot enforce a requested policy reject it before capture; restart the source with the updated runtime. Cloud supports only `auto`.
