...
| `--proxy` | Route eligible outbound traffic through a `socks4://IP:PORT` or `socks5://IP:PORT` proxy. Local-only. See [Proxy](#outbound-proxy) |
...
See [VSock](/networking/host-sockets) for guest connection details, limits, and SDK examples.
...
Restore a saved snapshot into a new local sandbox. Use `msb restore`, `msb sandbox restore`, or `msb sbx restore`; all three accept the same flags. Full snapshots resume captured processes; disk snapshots boot from the saved disk. Run new commands separately with `msb exec`.

```bash
msb restore devbox:ready --name worker --forked
msb exec worker -- echo hello
msb sbx restore ./saved.msb --name disk-copy --disk-only
```

| Flag | Description |
|------|-------------|
| `--name` | Required destination sandbox name |
...
Restore and branch do not reuse host resources by default. Map directories with `-v /host/path:/guest/path`, select a private captured disk with `-v /guest/path`, and publish new listeners with `-p HOST:GUEST`. An unmapped filesystem or additional disk stays present during full restore but backend requests return I/O errors; data already cached in guest RAM may still be readable. Warnings are shown even with `--quiet`. Disk errors can make the guest filesystem abort its journal or become read-only. The root disk is always restored from the snapshot and cannot be omitted. Custom vsock routes are not inherited; use `--vsock HOST_PATH:PORT` to authorize a service for clients to reconnect to.
...
```bash
msb branch devbox --name experiment
```
...
See [Optimization](/sandboxes/optimization) for interpreting these checks, choosing sandbox-level policies, and preparing the host storage and virtualization environment.
