...
| `--conf` | Load a sparse [sandbox configuration](/sandbox-config) |
| `--net-conf`, `--resource-conf`, `--runtime-conf`, `--fs-conf`, `--secret-conf`, `--script-conf` | Load an unwrapped, concern-specific [scoped config](/sandbox-config#scoped-config-files) |

Use `msb run --help` for the full flag list.

Sandbox configuration is never auto-discovered. Pass every root or scoped configuration file explicitly; positional images and explicit CLI flags can complete or override sparse files.
...
Flat v1 supports local OCI sandboxes with ext4. Rootfs patches, snapshots, and cloud execution are rejected instead of silently changing their semantics. The normal layered root remains the built-in default, and `sandbox_defaults.oci.root_disk` can select flat for future local sandboxes without repeating the flag.
...
| `--conf` | Load an explicit sparse [sandbox configuration](/sandbox-config) |
| `--net-conf`, `--resource-conf`, `--runtime-conf`, `--fs-conf`, `--secret-conf`, `--script-conf` | Load an explicit [scoped config](/sandbox-config#scoped-config-files) |
