...
| `--copy` | Copy a host file or directory into the guest rootfs before boot (`SRC:DST`) |
| `--copy-file` | Copy a host file into the guest rootfs before boot (`SRC:DST`) |
| `--copy-dir` | Copy a host directory into the guest rootfs before boot (`SRC:DST`) |
| `--mkdir` | Create a directory in the guest rootfs before boot |
| `--rm` | Hide/remove a path from the guest rootfs before boot |
| `--patch-file` | Load rootfs patch operations from a YAML file |
...
Rootfs patch flags are creation-time mutations. They apply before the sandbox starts and become part of the sandbox's initial writable layer:
...
Patch files use the same operation names:

```yaml
patches:
  - copy: ./config.toml:/etc/app/config.toml
  - copy_dir: ./certs:/etc/app/certs
  - mkdir: /var/cache/app
  - rm: /etc/motd
```

Use `msb cp` for live file transfer after a sandbox exists.
...
```bash
msb cp ./local.txt devbox:/tmp/local.txt
msb cp devbox:/tmp/out.txt ./out.txt
msb cp devbox:/tmp/a devbox:/tmp/b
```

Host-to-sandbox and sandbox-to-host forms follow `docker cp` syntax. The same-sandbox form is a microsandbox extension. Cross-sandbox copies are not supported yet; copy through the host if you need to move data between two sandboxes.

`msb cp` connects to an existing sandbox and starts it temporarily if needed, following the same lifecycle ownership behavior as `msb exec`.
