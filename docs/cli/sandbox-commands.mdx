...
| `--tcp-listen-backlog` | Accept-queue depth for published TCP ports, `1` to `2147483647` (default: `1024`). See [Published ports](#published-ports) |
...
Each published TCP port's host listener queues up to 1,024 connections that the runtime has not yet accepted. Connections that arrive while the queue is full never reach the sandbox, so a burst larger than the queue — for example a reverse proxy opening one upstream connection per request for a page that loads many assets at once — can fail at the proxy. Set `--tcp-listen-backlog` to change the depth for every published TCP port of the sandbox:

```bash
msb create node --name web -p 127.0.0.1:3000:3000 --tcp-listen-backlog 4096
```
...
| `--tcp-listen-backlog` | Accept-queue depth for the child's published TCP ports (default: `1024`) |
...
Volume, port, TCP listen backlog, user, vsock, inheritance, and mount-policy flags match [`restore`](#msb-restore).
