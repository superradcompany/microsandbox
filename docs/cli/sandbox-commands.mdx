...
Restore into a new sandbox. Disk snapshots boot fresh; full snapshots resume execution. Cloud supports disk restore. Use `msb exec` to run new commands.
...
```bash
msb restore saved.msb --name worker -v /srv/project:/work -v /data
msb restore saved.msb --name inspection --allow-missing-resources
```
