...
The pre-pull is optional: creating the first flat sandbox lazily materializes a missing flat artifact from the normal OCI layer cache. `clone=auto` prefers a copy-on-write reflink and falls back to a sparse copy, `clone=copy` forces the portable sparse-copy path, and `clone=reflink` requires reflink support and fails clearly if the destination filesystem cannot provide it. Flat v1 supports ext4; rootfs patches, snapshots, and cloud execution are rejected instead of silently changing their semantics. The normal layered root remains the default.
