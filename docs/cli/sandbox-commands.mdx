...
Live resize is not necessarily instant. When an accepted resize has not fully settled, the apply output includes a convergence table with a `STATE` column:

- `applied` — requested, actual, and enforced values match.
- `converging` — the guest is still onlining CPUs or plugging memory.
- `guest-refused` — the guest would not cooperate, but the host enforces the new limit anyway.
- `failed` — the resize did not complete.
...
Before a downgrade changes local state, `msb` runs several safety checks. It verifies the target release, confirms it can read the current local database, refuses targets below 0.6.0, refuses irreversible state changes, and blocks the operation while matching running sandboxes are active. When a database rollback is needed, `msb` creates a retained SQLite backup under `~/.microsandbox/db/` unless you pass `--no-backup`. If rollback metadata marks the image cache affected, `msb` purges the cache after the database rollback unless you pass `--keep-cache`. `msb` always installs the target binary fresh and checks it with `msb --version` before refreshing command links.
