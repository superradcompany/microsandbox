...
`--force` and a timeout that elapses both end in a force-kill. Pending writes that the workload hasn't `fsync`'d at that point may be lost, with the same durability semantics as a sudden power loss on a physical machine. For durable writes, workloads should `fsync` important data themselves.
