...
Both branch forms accept `--guest-flush auto|required|skip`. Auto preserves dirty memory without an optional root flush; required adds writeback. Skip never disables mandatory owned/external-storage barriers.
