...
On Linux AMD hosts, `msb doctor` reports whether KVM AVIC is active. AVIC is an optional host-wide network-performance capability, so a disabled result is a warning and does not make the host unhealthy. Doctor never reloads `kvm_amd` automatically.
