...
| `--rootfs-layout` | OCI rootfs layout: `layered` (default) or `flat` |
| `--root-disk` | Writable root disk capacity or backing, such as `8G`, `tmpfs:2G`, or `./rootfs.qcow2:format=qcow2,fstype=ext4` |
...
Flat layout merges the OCI layers into one reusable ext4 base, clones that base into the sandbox as `rootfs.raw`, grows the private clone to `--root-disk`, and attaches it directly through virtio-blk. The guest mounts ext4 as its root without OverlayFS.

```bash
msb pull python --materialize flat
msb run --rootfs-layout flat --root-disk 8G python -- python -c "print('hello')"
```

The pre-pull is optional: creating the first flat sandbox lazily materializes a missing flat artifact from the normal OCI layer cache. Flat v1 supports managed root disks only; rootfs patches, tmpfs or user-supplied root disks, and snapshots are rejected instead of silently changing their semantics. Layered remains the default.
