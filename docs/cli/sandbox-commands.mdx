...
msb modify api -m 8G --wait                 # Return once the guest converges
```
...
Resize can take time. The output and JSON `resize_status` report `applied`, `converging`, `guest-refused`, or `failed`. Without `--wait` this is a single reading taken right after the change is accepted. With `--wait`, modify polls until every resource is `applied`, `guest-refused`, or `failed`. The default wait is 60 seconds; `--timeout 0` checks once. On timeout, modify prints the last status, or the status from apply if no later read completed, and exits non-zero. A shrink stays `converging` until the guest releases the CPUs or memory. In every state, the host already enforces the new limit. `--wait` also covers a resize that is still in progress from an earlier modify, so `msb modify api --wait` alone waits for it.
...
| `--wait` | Wait for any pending live CPU and memory resize to converge |
| `--timeout` | Seconds to wait with `--wait` (default 60, `0` checks once) |
