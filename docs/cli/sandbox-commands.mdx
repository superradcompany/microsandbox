...
Change a running sandbox's configuration. Every change is planned, labeled (live, next start, requires restart, unsupported), and applied all-or-nothing. CPU and memory apply live within the `max_cpus` / `max_memory` ceilings; raising a ceiling requires `--restart` or `--next-start`.
...
`max_cpus` and `max_memory` are reserved at create time and default to the starting `cpus`/`memory`, leaving no headroom for live growth. Reserving more costs almost nothing, so set them higher at create time if the sandbox may need to grow.

A live resize may not settle immediately. The apply output then reports a `STATE` per resource (`resize_status` in `--format json`):

- `applied`: the new value is in effect.
- `converging`: the guest is still applying the change.
- `guest-refused`: the guest declined, but the host enforces the limit anyway.
