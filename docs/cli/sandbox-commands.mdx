...
still running after the timeout, `msb` force-kills it.
...
| `--secret`, `--secret-rm` | Add or rotate a secret from a host environment variable (`NAME@HOST`), or remove one. Inline `NAME=VALUE` is rejected; use an SDK for raw values |
