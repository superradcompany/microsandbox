...
| `downgrade <version>` | Downgrade msb and compatible local state to an older release. V1 supports targets at or above 0.6.0 and refuses on Windows before touching files or state |
...
Before a downgrade rolls back schema, `msb`:

- Checks the target release and reads that release's schema baseline.
- Refuses targets below 0.6.0 and refuses irreversible migration paths.
- Blocks the rollback while matching running sandboxes are active.

When rollback is needed, `msb` creates a retained SQLite backup under `~/.microsandbox/db/` unless you pass `--no-backup`. If rollback metadata marks the image cache as affected, `msb` purges the cache after schema rollback unless you pass `--keep-cache`. `msb` always installs the target binary fresh and verifies it with `msb --version` before refreshing command links.
...
| `--no-backup` | `downgrade` | Skip the database backup before schema rollback |
