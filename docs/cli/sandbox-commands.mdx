...
| `--label` | Attach a label for metric attribution (`KEY=VALUE`, or bare `KEY` for a valueless marker). Repeatable. Keys cannot start with `sandbox.`, `microsandbox.`, or `service.` |
