...
| `--max-cpus` | Boot-time maximum possible virtual CPUs for future resize capacity (experimental, requires `MSB_EXPERIMENTAL_MODIFY=1`) |
| `-m`, `--memory` | Memory limit, such as `512M` or `1G` |
| `--max-memory` | Boot-time maximum hotpluggable memory, such as `2G` or `8G` (experimental, requires `MSB_EXPERIMENTAL_MODIFY=1`) |
...
<Note>
Experimental: requires `MSB_EXPERIMENTAL_MODIFY=1` in the environment. The command is hidden from `msb --help` otherwise.
</Note>
