...
| `--materialize <MODE>` | Prepare `layered` (default), `flat`, or `all` rootfs artifacts |

`flat` creates a reusable, validated raw ext4 base with no guest OverlayFS in the final rootfs. The pull path reconstructs the final OCI filesystem from the content-addressed layer cache, so it does not depend on a guest builder or host mount. `all` explicitly prepares both representations; `flat` also retains the internal layer artifacts needed for OCI cache integrity and later re-materialization.
