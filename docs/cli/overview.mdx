...
On Windows, run the following from an elevated PowerShell if the first sandbox start reports that Windows Hypervisor Platform is unavailable:

```powershell
Enable-WindowsOptionalFeature -Online -FeatureName HypervisorPlatform -All -NoRestart
```

Reboot after the command completes. `HypervisorPlatform` is the WHP API surface used by libkrun and is separate from `VirtualMachinePlatform`, which WSL2 and Docker Desktop often enable.
...
msb self doctor          # Check runtime and host prerequisites
