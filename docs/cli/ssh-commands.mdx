...
SSH commands expose sandbox shells, remote commands, and SFTP through the SSH protocol. For usage flows, see [SSH](/sandboxes/ssh/overview).
...
`msb connect <sandbox>` is a top-level alias for `msb ssh <sandbox>`.
