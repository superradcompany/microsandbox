---
title: SSH commands
"og:title": Microsandbox CLI SSH commands
sidebarTitle: "SSH"
description: Connect to running sandboxes and serve SSH endpoints with the microsandbox CLI.
icon: "lock"
---

See [SSH](/sandboxes/ssh) for shell, remote command, and SFTP workflows.
...
| `-n`, `--name NAME` | Explicit sandbox name, up to 128 UTF-8 bytes; useful when the name collides with `serve`, `authorize`, or `help` |
...
| `-p`, `--port PORT` | Listener port. Defaults to `2222` |
...
Explicit form of [`msb ssh`](#msb-ssh), with the same arguments and options.

```bash
msb ssh connect devbox
```

All SSH commands inherit `ssh.inactivity_timeout_secs` from user configuration when neither timeout flag is present. The built-in default is 10 minutes. A [managed timeout](/configuration#ssh) takes precedence over both flags, including `--no-inactivity-timeout`, for local and cloud sessions.
