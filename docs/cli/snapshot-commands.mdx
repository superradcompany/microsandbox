...
| `--guest-flush` | Guest writeback policy: `auto` (default), `required`, or `skip`; see [guest flushing](/sandboxes/snapshot-consistency#guest-flushing) |
...
Guest flushing is shared by the CLI and SDKs. See [guest flushing](/sandboxes/snapshot-consistency#guest-flushing) for policies and examples.
