...
"og:title": Microsandbox CLI setup and management commands
sidebarTitle: "Management"
description: Inspect the active backend, diagnose runtime setup, and manage the microsandbox CLI installation.
icon: "wrench"
---

For first-time installation, see the [CLI overview](/cli/overview#install).
...
Run downgrade from the newer CLI before replacing it: the older binary does not contain rollback steps for newer migrations. If you already installed an older binary and see `database schema is newer than this msb binary`, follow the [catalog recovery steps](/migrations/v0.7#database-schema-is-newer-than-this-msb-binary). [Snapshot groups can also block downgrade](/migrations/v0.7#snapshot-groups-can-block-downgrade).
