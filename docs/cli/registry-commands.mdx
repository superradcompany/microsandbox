...
"og:title": Microsandbox CLI container registry commands
sidebarTitle: "Registries"
description: Manage container registry credentials for local OCI image pulls with the microsandbox CLI.
icon: "key"
---

These commands update the local credential store; see [registry authentication](/configuration#auth-resolution-order) for credential resolution. Cloud credentials are managed separately.
