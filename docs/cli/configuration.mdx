---
title: Configuration
"og:title": Microsandbox CLI YAML configuration reference
description: Configure sandbox resources, networking, and storage with reusable microsandbox CLI YAML files.
icon: "file-code"
---
...
A non-empty `allow` list implies deny-by-default egress. `strict: true` requires hostname-based allow rules to use an inspectable request authority. Plain HTTP exposes that authority in `Host`; HTTPS only exposes it when TLS interception is enabled and not bypassed. Without that visibility, strict mode denies HTTPS that would otherwise be allowed only by a hostname rule. Top-level `ports` is shorthand for `network.ports`; when both are present, the lists are combined and duplicate host ports are rejected.
