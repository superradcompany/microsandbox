...
`msb context` confirms that subsequent commands will run on cloud and shows what selected the backend without exposing the API key. For profiles, SDK selection, and precedence, see [Backends](/operations/backends).
...
- **Backends.** Cloud is explicit: set `MSB_BACKEND=cloud`, select a cloud profile, or choose it in code. See [Backends](/operations/backends).
