---
title: "Sandbox"
description: "Sandbox API 레퍼런스"
icon: "cube"
---


See [Overview](/ko/sandboxes/overview) for configuration examples and [Lifecycle](/ko/sandboxes/lifecycle) for state management.
...
| init | `str \| dict \|` [`InitConfig`](#initconfig) | Hand off PID 1 to a guest init binary. See [Custom init system](/ko/sandboxes/customize#custom-init-system) and [`InitConfig`](#initconfig) for accepted shapes |
...
| volumes | `dict[str, dict]` | Volume mounts. See [Volumes](/ko/sdk/python/volumes). |
| patches | `list[PatchConfig]` | Rootfs modifications applied before boot |
| ports | `dict[int, int]` | Port mappings (`host_port: guest_port`) |
| network | [`Network`](/ko/sdk/python/networking#network) | Network policy and configuration |
| secrets | `list[`[`SecretEntry`](/ko/sdk/python/secrets#secretentry)`]` | Secret injection |
...
Get a filesystem handle for reading and writing files inside the running sandbox. This is a synchronous property -- use `sb.fs` (no `await`). See [Filesystem](/ko/sdk/python/filesystem) for the full API.

**Returns**

| Type | Description |
|------|-------------|
| [`SandboxFs`](/ko/sdk/python/filesystem) | Filesystem handle |
...
| args_or_options | `list[str] \| ExecOptions \| None` | Command arguments (e.g. `["-c", "print('hello')"]`) or [`ExecOptions`](/ko/sdk/python/execution#execoptions) |

**Returns**

| Type | Description |
|------|-------------|
| [`ExecOutput`](/ko/sdk/python/execution#execoutput) | Collected stdout, stderr, and exit status |
...
| args_or_options | `list[str] \| ExecOptions \| None` | Command arguments or [`ExecOptions`](/ko/sdk/python/execution#execoptions) |

**Returns**

| Type | Description |
|------|-------------|
| [`ExecHandle`](/ko/sdk/python/execution#exechandle) | Streaming handle for receiving events and controlling the process |
...
| Type | Description |
|------|-------------|
| [`ExecHandle`](/ko/sdk/python/execution#exechandle) | Streaming handle |
...
See [Patches](/ko/sandboxes/customize#patches) for conceptual context.
...
| user | `str` | - | Default guest user |
| entrypoint | `list[str]` | - | Override image entrypoint |
| init | `str \| dict \|` [`InitConfig`](#initconfig) | - | Hand off PID 1 to a guest init binary. See [Custom init system](/ko/sandboxes/customize#custom-init-system) |
...
| patches | `list[PatchConfig]` | `[]` | Rootfs modifications applied before boot |
| ports | `dict[int, int]` | `{}` | Port mappings (`host_port: guest_port`) |
| network | [`Network`](/ko/sdk/python/networking#network) | `public_only` | Network policy and configuration |
| secrets | `list[`[`SecretEntry`](/ko/sdk/python/secrets#secretentry)`]` | `[]` | Secret injection |
| detached | `bool` | `False` | If `True`, sandbox survives after your process exits |

### InitConfig

Custom init specification. Pass it (or one of the equivalent shorthand shapes) as the `init=` kwarg to [`Sandbox.create()`](#sandboxcreate) to hand PID 1 inside the guest off to your own init binary after agentd's setup. See [Custom init system](/ko/sandboxes/customize#custom-init-system) for image picks, shutdown semantics, and tradeoffs.
