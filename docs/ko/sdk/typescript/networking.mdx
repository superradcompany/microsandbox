---
title: "네트워킹"
description: "네트워킹 API 레퍼런스"
icon: "network-wired"
---


See [Networking](/ko/networking/overview) for conceptual overview and [TLS Interception](/ko/networking/tls) for TLS proxy details.
...
Allow the `Loopback` group (`127.0.0.0/8`, `::1`). The **guest's own** loopback, not the host. To reach a service on the host's localhost, use [`allowHost()`](#allowhost) instead. See the [loopback-vs-host watch-out](/ko/networking/overview#loopback-vs-host-a-common-trap).
...
Set the action taken when a secret reaches a disallowed host. See [`ViolationAction`](/ko/sdk/typescript/secrets#violationaction).
...
Add a secret. See [`SecretBuilder`](/ko/sdk/typescript/secrets#secretbuilder).
...
| secretViolation | [`ViolationAction`](/ko/sdk/typescript/secrets#violationaction) ` \| null` | Action on disallowed secret use |
