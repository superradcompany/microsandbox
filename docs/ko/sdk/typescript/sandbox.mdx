...
Get a filesystem handle for reading and writing files inside the running sandbox. See [Filesystem](/ko/sdk/typescript/filesystem) for the full API.

**Returns**

| Type | Description |
|------|-------------|
| [`SandboxFs`](/ko/sdk/typescript/filesystem) | Filesystem handle |
...
| Type | Description |
|------|-------------|
| [`ExitStatus`](/ko/sdk/typescript/execution#exitstatus) | Exit code and success flag |
...
| `init(cmd, args?)` | Hand off PID 1 to `cmd` (absolute path or `"auto"`) after agentd setup. See [Custom init system](/ko/sandboxes/customize#custom-init-system) |
...
| `volume(guestPath, m => ...)` | Mount a volume — see [Volumes](/ko/sdk/typescript/volumes) |
| `patch(p => ...)` | Modify the rootfs before boot — see [Snapshots](/ko/sdk/typescript/snapshots) |
...
| `network(n => ...)` | Configure DNS / TLS / policy / secrets — see [Networking](/ko/sdk/typescript/networking) |
| `secret(s => ...)` | Add a secret via [`SecretBuilder`](/ko/sdk/typescript/secrets#secretbuilder) |
