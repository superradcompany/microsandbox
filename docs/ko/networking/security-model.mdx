---
title: "보안 모델"
description: "microsandbox가 네트워크 계층을 어떻게 보호하는지"
icon: "shield-halved"
---
...
- **Host trust store integrity.** When [`trust_host_cas`](/ko/sdk/rust/networking#trust_host_cas) is opted into, an attacker who can plant a CA on the host has that trust inside every sandbox. Off by default for exactly this reason.

## See also

- [DNS](/ko/networking/dns): the controls that ride on the DNS interceptor
- [TLS interception](/ko/networking/tls): plaintext visibility for per-host policy and secret injection
