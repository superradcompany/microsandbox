...
The two features compose. Turn on [`trust_host_cas`](/ko/sdk/rust/networking#trust_host_cas) alongside TLS interception when you need both: interception provides richer inspection on the ports it covers, host-CA trust covers everything else. Leave it off to keep the guest's TLS stack validating strictly against its stock Mozilla bundle.

## See also

- [DNS](/ko/networking/dns): the DNS interceptor is what makes per-host rules possible, and DoT interception reuses this TLS path
- [Security model](/ko/networking/security-model): how TLS interception interacts with secret injection and SSRF defenses
