...
`cpu_placement: auto` is intentionally not a portable macOS default: managed placement is unsupported on macOS because the operating system does not expose an equivalent public hard-affinity API. Use `inherit` on macOS. Keep `thp: madvise` and the block writeback limit on `auto` until a representative workload proves a different policy is better.
...
The background worker releases credit only after range writeback and a same-inode data-and-allocation-metadata barrier complete. Guest `FLUSH` still performs the existing full Imago flush and sync after all earlier block work; the controller does not acknowledge guest durability early. Controller, storage, worker, or full-sync failures remain latched and fail future mutations and reactivations closed.
