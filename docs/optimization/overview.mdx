...
`auto` also enables spawn-time admission against one aggregate dirty-credit pool. The default pool is the lower of 10% of physical memory and the host's `vm.dirty_background_bytes` or `vm.dirty_background_ratio` threshold. On a small host, the automatic per-disk limit shrinks to fit the pool; if the result would be below the VMM's 128 MiB minimum, creation fails with guidance instead of silently dropping containment. Set `block_writeback_pool_mib` to override the aggregate pool. A fixed `{"mib": N}` limit remains unpooled unless the pool field is also set, which keeps focused experiments explicit.
