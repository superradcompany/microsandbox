...
| `runtime.block_writeback_limit` | `auto` | Bounds buffered host dirty data per writable raw disk | Linux, writable raw disks using writeback caching |
...
| Buffered block writeback limit | `runtime.block_writeback_limit` | Global runtime policy only |

The Rust, TypeScript, Python, and Go SDK builders expose the same per-sandbox or per-snapshot choices. The block writeback limit is global because it controls host VMM storage policy rather than one guest-visible sandbox specification.
...
On Linux, `auto` gives each writable raw disk a 512 MiB hard budget for buffered host dirty data. Libkrun reserves exact page-aligned extents before each mutation, starts background writeback in 128 MiB batches, and applies backpressure before the per-disk budget can be exceeded. Rewritten pages remain charged until the newest generation completes, so repeatedly overwriting one region cannot release another region's credit.
...
`off` disables this VMM-owned bound for newly spawned sandboxes. It may improve peak sequential throughput, but it permits a long-running guest to accumulate more host dirty data and may worsen the final flush tail. The policy does not depend on cgroups or guest cooperation. It has no effect on read-only disks, direct-I/O disks, non-raw formats, cache modes without writeback, or non-Linux VMM paths.

On the fixed OVH customer disk suite, both the old internal-barrier controller and the repaired range-completion controller scored 1,668 MB/s sequential write at 512 MiB; the otherwise identical `off` profile scored 9,752 MB/s while host-global Dirty reached approximately 1 GiB. The range-completion repair is the correct containment-versus-durability contract, but it cannot reproduce cache-speed throughput when the hard budget is below the workload's active dirty set. Do not promote this fixed `auto` value as a product default until a budget staircase and aggregate multi-sandbox host-admission policy are measured.
