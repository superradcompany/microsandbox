...
| `runtime.block_writeback_limit` and `runtime.block_writeback_pool_mib` | `off` and unset | Opt-in bound for buffered host dirty data per writable raw disk and admission across active VMs | Linux, writable raw disks using writeback caching |
...
| Buffered block writeback limit and admission pool | `runtime.block_writeback_limit`, `runtime.block_writeback_pool_mib` | Global runtime policy only |

The Rust, TypeScript, Python, and Go SDK builders expose the same per-sandbox or per-snapshot choices. The block writeback limit and pool are global because they control host VMM storage policy rather than one guest-visible sandbox specification.
...
```json
{
  ...
  "runtime": {
    "block_writeback_limit": "off"
  }
}
```

`cpu_placement: auto` is intentionally not a portable macOS default: managed placement is unsupported on macOS because the operating system does not expose an equivalent public hard-affinity API. Use `inherit` on macOS. Keep `thp: madvise` unless a representative workload and density test justify another policy. Buffered-writeback containment remains `off` by default because the measured 512 MiB through 1.5 GiB staircase does not recover sequential throughput; opt into `auto` only when the bounded host-dirty contract is worth that trade-off.
...
The default `off` policy leaves the controller and admission path unused. On Linux, explicit `auto` prefers a 512 MiB hard budget for each writable raw disk. Libkrun reserves exact page-aligned extents before each mutation, starts background writeback in bounded batches, and applies backpressure before the per-disk budget can be exceeded. Rewritten pages remain charged until the newest generation completes, so repeatedly overwriting one region cannot release another region's credit.

The background worker kicks queued ranges before waiting for their page-cache writeback to complete, and it releases exact range credit only after that completion. It does not turn each internal batch into a durability barrier. Guest `FLUSH` still performs the existing full Imago flush and sync after all earlier block work, including the host-filesystem metadata required to retrieve the data; the controller does not acknowledge guest durability early. Controller, storage, worker, range-completion, or full-sync failures remain latched and fail future mutations and reactivations closed.

`auto` also enables spawn-time admission against one aggregate dirty-credit pool. The default pool is the lower of 10% of physical memory and a conservative estimate of the host's dirty-background threshold. An explicit `vm.dirty_background_bytes` is used directly; ratio mode applies `vm.dirty_background_ratio` to current `MemAvailable`, not `MemTotal`, because Linux explicitly excludes unavailable memory from its ratio denominator. On a small or memory-pressured host, the automatic per-disk limit shrinks to fit the pool; if the result would be below the VMM's 128 MiB minimum, creation fails with guidance instead of silently dropping containment. Set `block_writeback_pool_mib` to override the aggregate pool. A fixed `{"mib": N}` limit remains unpooled unless the pool field is also set, which keeps focused experiments explicit.

For example, this admits at most eight one-disk VMs, four two-disk VMs, or another combination whose reservations fit 4 GiB:

```json
{
  "runtime": {
    "block_writeback_limit": { "mib": 512 },
    "block_writeback_pool_mib": 4096
  }
}
```

Each VM reserves `per-disk limit × eligible writable raw disks` in one retryable SQLite transaction before libkrun is built. Concurrent creators cannot both claim the last credit. The reservation is held by an owner-only process lock, released on normal exit, and recovered from a stale lock after a process crash. Missing, malformed, or unverifiable leases stay charged, so damaged coordination state fails closed. This transaction is not on the block-I/O path: libkrun enforces the admitted per-disk budgets locally after boot.
...
flowchart LR
    ...
    G -. "crash: next creator verifies stale lease" .-> C
```
...
`off` disables both the VMM-owned bound and admission for newly spawned sandboxes. It may improve peak sequential throughput, but it permits a long-running guest to accumulate more host dirty data and may worsen the final flush tail. Neither the per-disk controller nor admission depends on cgroups or guest cooperation. The policy has no effect on read-only disks, direct-I/O disks, non-raw formats, cache modes without writeback, or non-Linux VMM paths.

The fixed OVH customer disk suite measured 512 MiB, 1 GiB, 1.25 GiB and 1.5 GiB at approximately 1,668 MB/s sequential write against 8,783 MB/s with the controller off. Raising only the hard ceiling did not alter the early 128–256 MiB writeback-generation schedule, so the 1.5 GiB profile consumed three times the potential host admission credit of 512 MiB without recovering throughput. The dated planning report records the complete staircase and multi-VM validation; keep the controller opt-in until its high-water scheduling is redesigned and remeasured.
