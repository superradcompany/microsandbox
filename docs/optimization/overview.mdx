...
`cpu_placement: auto` is intentionally not a portable macOS default: managed placement is unsupported on macOS because the operating system does not expose an equivalent public hard-affinity API. Use `inherit` on macOS. Keep `thp: madvise` unless a representative workload and density test justify another policy. Buffered-writeback containment remains `off` by default because the finite per-disk window is a workload-and-density policy. The density-first `auto` window is 512 MiB; the unchanged OVH customer disk suite needed a fixed 1.5 GiB window to keep its repeatedly overwritten 1 GiB file at containment-off throughput. Size a fixed window and the aggregate pool from representative hot-set and concurrency measurements rather than assuming either profile is universal.
...
`off` disables both the VMM-owned bound and admission for newly spawned sandboxes. It permits a long-running guest to accumulate more host dirty data and may worsen the final flush tail. It is not inherently faster than a correctly sized pressure-triggered window: the focused OVH customer disk suite reached parity with `off` at a fixed 1.5 GiB limit, while the density-first 512 MiB `auto` window correctly backpressures that suite's 1 GiB hot set. Neither the per-disk controller nor admission depends on cgroups or guest cooperation. The policy has no effect on read-only disks, direct-I/O disks, non-raw formats, cache modes without writeback, or non-Linux VMM paths.
