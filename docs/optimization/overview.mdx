...
| `runtime.block_writeback_limit` and `runtime.block_writeback_pool_mib` | `auto` and derived | Bounds buffered host dirty data per writable raw disk and admission across active VMs | Linux, writable raw disks using writeback caching |
...
The fixed OVH customer disk suite is being used to measure 512 MiB, 1 GiB, 1.25 GiB, and 1.5 GiB against `off`. Keep the controller disabled by default until that staircase and real multi-VM admission validation are complete; the final measured policy and trade-off will replace this note before the branch is proposed for merge.
