...
mod tests {
    ...
    #[test]
    fn shorthand_bare_ipv4_destination_becomes_cidr() {
        let destination = parse_network_destination(None, Some("1.1.1.1")).unwrap();

        match destination {
            Destination::Cidr(cidr) => assert_eq!(cidr.to_string(), "1.1.1.1/32"),
            other => panic!("expected CIDR destination, got {other:?}"),
        }
    }

    #[test]
    fn typed_ipv6_destination_becomes_cidr() {
        let destination =
            parse_network_destination(Some("ip"), Some("2001:4860:4860::8888")).unwrap();
    ...
    #[test]
    fn typed_domain_destination_stays_domain() {
        let destination = parse_network_destination(Some("domain"), Some("GitHub.com.")).unwrap();

        match destination {
            Destination::Domain(name) => assert_eq!(name.as_str(), "github.com"),
            other => panic!("expected domain destination, got {other:?}"),
        }
    }

    #[test]
    fn typed_group_destination_stays_group() {
        let destination = parse_network_destination(Some("group"), Some("public")).unwrap();

        match destination {
            Destination::Group(group) => assert_eq!(group, DestinationGroup::Public),
