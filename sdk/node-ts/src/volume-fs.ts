...
import type {
  ...
  NapiFsMetadata,
  NapiVolumeFs,
  NapiVolumeFsReadStream,
  NapiVolumeFsWriteSink,
} from "./internal/napi.js";
import type { FsEntry, FsEntryKind, FsMetadata } from "./fs-types.js";

export class VolumeFsReadStream
  implements AsyncIterable<Uint8Array>, AsyncDisposable
{
  private readonly inner: NapiVolumeFsReadStream;
  private done = false;

  /** @internal */
  constructor(inner: NapiVolumeFsReadStream) {
    this.inner = inner;
  }
...
export class VolumeFsWriteSink implements AsyncDisposable {
  private readonly inner: NapiVolumeFsWriteSink;
  private closed = false;

  /** @internal */
  constructor(inner: NapiVolumeFsWriteSink) {
    this.inner = inner;
  }
...
export class VolumeFs {
  private readonly inner: NapiVolumeFs;

  /** @internal */
  constructor(inner: NapiVolumeFs) {
    this.inner = inner;
  }
  ...
  async readStream(path: string): Promise<VolumeFsReadStream> {
    const raw = await withMappedErrors(() => this.inner.readStream(path));
    return new VolumeFsReadStream(raw);
  }
  ...
  async writeStream(path: string): Promise<VolumeFsWriteSink> {
    const raw = await withMappedErrors(() => this.inner.writeStream(path));
    return new VolumeFsWriteSink(raw);
  }
