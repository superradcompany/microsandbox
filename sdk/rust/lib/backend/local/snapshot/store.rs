//! Local snapshot artifact storage operations: open, list, and index maintenance.
...
use super::{LocalSnapshotArtifact, SnapshotFormat, SnapshotScope};
...
impl LocalBackend {
    ...
    /// with `.` or `~`; otherwise as a bare name resolved under this
    /// backend's snapshots directory.
    pub(super) async fn open_snapshot_artifact(
        &self,
        path_or_name: &str,
        ...
        let dir = if looks_like_path(path_or_name) {
            ...
            self.snapshots_dir().join(path_or_name)
        };
        ...
        if !manifest_path.exists() && dir.join(V066_DESCRIPTOR_FILENAME).exists() {
            super::migration::reconcile_explicit(self.db().await?, &dir).await?;
        }
        ...
        let artifact = LocalSnapshotArtifact::new(dir.clone(), digest.clone(), manifest);
        ...
        if dir.parent() == Some(snapshots_dir.as_path())
            && let Ok(None) = self.find_snapshot_model_by_digest(&digest).await
            && let Err(e) = self
                .upsert_snapshot_index(artifact.path(), artifact.digest(), artifact.manifest())
                .await
        {
    ...
    pub(super) async fn upsert_snapshot_index(
        &self,
        artifact_path: &Path,
        digest: &str,
        manifest: &Manifest,
    ...
    pub(super) async fn list_snapshot_models(
        &self,
    ) -> MicrosandboxResult<Vec<snapshot_entity::Model>> {
    ...
    pub(super) async fn list_snapshot_artifacts(
        &self,
        dir: &Path,
        ...
        while let Some(entry) = entries.next_entry().await? {
            ...
            if !path.join(DESCRIPTOR_FILENAME).exists()
                && !path.join(V066_DESCRIPTOR_FILENAME).exists()
            {
                continue;
            }
            match self
                .open_snapshot_artifact(path.to_string_lossy().as_ref())
                .await
            {
                Ok(s) => out.push(s),
                Err(e) => {
    ...
    pub(super) async fn remove_snapshot_artifact(
        &self,
        path_or_name: &str,
        force: bool,
    ) -> MicrosandboxResult<()> {
        ...
        let (digest, artifact_path) =
            if path_or_name.starts_with("sha256:") || path_or_name.starts_with("sha512:") {
                ...
                let snapshot = self.open_snapshot_artifact(path_or_name).await?;
                (snapshot.digest().to_string(), snapshot.path().to_path_buf())
            } else {
                ...
                if let Some(row) = row {
                    ...
                    let dir = self.snapshots_dir().join(path_or_name);
                    let snapshot = self
                        .open_snapshot_artifact(dir.to_string_lossy().as_ref())
                        .await?;
                    (snapshot.digest().to_string(), snapshot.path().to_path_buf())
                }
            };
    ...
    pub(super) async fn reindex_snapshot_dir(&self, dir: &Path) -> MicrosandboxResult<usize> {
        let snapshots = self.list_snapshot_artifacts(dir).await?;
        let mut indexed = 0usize;
        for snap in &snapshots {
            if let Err(e) = self
                .upsert_snapshot_index(snap.path(), snap.digest(), snap.manifest())
                .await
            {
    ...
    pub(super) async fn find_snapshot_model(
        &self,
        needle: &str,
    ) -> MicrosandboxResult<snapshot_entity::Model> {
        let db = self.db().await?.read();
        ...
        row.ok_or_else(|| MicrosandboxError::SnapshotNotFound(needle.into()))
    }

    /// Look up a snapshot by digest in the local index.
    pub(super) async fn find_snapshot_model_by_digest(
        &self,
        digest: &str,
