...
func TestResizeTimeoutMillis(t *testing.T) {
	if got := resizeTimeoutMillis(0); got != 1 {
		t.Fatalf("zero timeout must stay bounded, got %d", got)
	}
	...
	if got := resizeTimeoutMillis(-time.Second); got != 1 {
		t.Fatalf("negative timeout must stay bounded, got %d", got)
	}
}
