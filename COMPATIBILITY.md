...
In v0.7.0, the private launcher is `msb machine`; `msb sandbox` remains the public command group and `sbx` its alias. Complete legacy `msb sandbox [internal flags]` invocations are routed through a separate boot-only decoder, preserving old SDK launches without shadowing public subcommands. The strict modern decoder still requires explicit execution intent. Public top-level verbs such as `msb run` remain supported.

Before launching a VM, SDKs query the selected binary's side-effect-free `__launch-protocol` command. Protocol 2 supports the strict boot/restore contract; protocol 1 is the v0.6.17 boot contract. These generations are independent of package versions. For pre-probe binaries, the SDK verifies the legacy launcher through help and identifies the historical wire format using `--version`: v0.6.10–v0.6.15 use flat networking without isolated file mounts, v0.6.16 adds isolated file mounts, and v0.6.17 onward use resolved networking. Legacy decoding preserves flat network policy fields. The accepted v0.6.9 ↔ v0.6.10 exception remains outside this adapter.

Negotiation does not change runtime resolution precedence, require matching package versions, or retry a failed VM launch. Malformed capability responses, crashes, and probe timeouts are errors. Restore, branch, checkpoint disk chains, and other features unavailable in the selected legacy format produce an upgrade-required error rather than silently dropping requested behavior. Home-installed binaries remain preferred over packaged or embedded binaries.
...
Windows disk attachment retains the existing exclusive sidecar filenames and sharing mode. The launcher duplicates non-inheritable sidecar handles into the exact spawned process and sends their values through a bounded startup-only stdin message, required by the hidden `msb machine --disk-locks-stdin` flag. The runtime adopts them before opening guest disks; successful handoff closes the launcher's copies without an unlocked interval. An older runtime rejects the unknown flag, while older launchers retain their existing parent-owned behavior. Failed or cancelled handoffs terminate the child before storage cleanup; confirmed exit also clears any remaining parent copies. Stop/start observes owned-disk sidecar release, independently of retained SDK objects. This changes the Windows private launch contract, not SDK APIs, agent framing, snapshot formats, or disk bytes.
