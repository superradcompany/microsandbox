...
In v0.7.0, the private launcher is `msb machine`; `msb sandbox` is the public command group and `sbx` is its alias. The current CLI also recognizes historical internal `msb sandbox` launches. The SDK selects the tested v0.6.x launch contract for historical executables and encodes the corresponding arguments and JSON. Public top-level verbs such as `msb run` remain supported.
...
The shared cloud decoder accepts both historical and current secret field names. Cloud must adopt the updated types/runtime to support both deployed contracts; changing the SDK alone is insufficient. The cloud contract retains optional global `passthrough_hosts` through serialization and domain conversion, so later secret additions inherit the default. Per-secret blocking overrides still take precedence.

### Compatibility code organization

New shared conversion types live under `packages/microsandbox-types/rust/lib/compatibility/v0_6/{local,cloud}`. Existing SDK catalog and launch codecs, runtime launch decoders, cloud-create decoding, and snapshot projection remain at their established paths. Normal database reads use the migrated representation directly. A separate refactor patch groups the existing modules by version without changing behavior.
