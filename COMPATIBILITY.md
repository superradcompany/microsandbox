...
Published-port accept-queue depth uses the optional `tcp_accept_queue_size` key, omitted when unset so a launch that does not ask for it is byte-identical for every runtime. Omission now means a 1,024-deep queue on each published TCP listener, where earlier runtimes inherited mio's 128; the host kernel clamps both to `somaxconn`. Previous SDKs launching the current runtime get the deeper queue too. That is deliberate and not pinned at the boundary: queue depth is not a resource or policy limit, and the shallow queue was the defect. An explicit value requires a runtime that advertises `tcp_accept_queue_size` through `__launch-protocol`; the SDK probes only when the key is set and returns an upgrade-required error otherwise, because v0.7.x `machine` runtimes decode network JSON leniently and would drop the key. v0.6.x launch contracts reject it before launch and never receive the key. Managed cloud rejects it before HTTP, as the cloud create contract carries no published ports.
