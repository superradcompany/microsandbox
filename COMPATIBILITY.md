...
Shared contracts live under `packages/microsandbox-types/rust/lib/compat`, grouped by the release that introduced the shape: `v0_5_0/local/secrets`, `v0_6_5/cloud/create`, `v0_6_7/cloud/secrets`, and `v0_7_0/local/secrets`. Later releases reuse these contracts. Saved-config migration and downgrade conversion live in `crates/db/lib/compat`, with dispatch in `config.rs` and adapters under `v0_5_0/secrets`, `v0_6_5/config`, and `v0_7_0/secrets`; cloud dispatch lives in the types crate’s `compat/cloud.rs`. Known destination releases use `semver::Version` across CLI downgrade and SDK launch selection; each compatibility layer owns its own release boundaries. Incoming unversioned launch/cloud requests retain detection of formats from previous versions. Cloud requests continue using the existing `/v1` API without a separate body version. Compatibility readers accept request formats from previous and current versions; additional cloud contract versioning is deferred until a concrete incompatible change requires it. Explicit launch-message versioning remains separate follow-up work. Concrete Rust types describe cloud-create requests, secret payloads, and previous injection scopes; catalog remapping preserves unknown saved fields. The SDK owns executable discovery, `LaunchContract`, and preflight in `sdk/rust/lib/runtime/launch_contract.rs`, along with outgoing encoding; `launch_input.rs` contains previous environment-encoding helpers. The runtime selects incoming readers in `crates/runtime/lib/client/compat/launch.rs`. The dispatcher selects the current reader when `execution` is present, `v0_6_10/launch.rs` when `bootstrap` is present, and `v0_5_9/launch.rs` otherwise. The previous readers share launch fields and conversion alongside the dispatcher in `compat/launch.rs`; `network.rs` provides shared network compatibility decoding. Explicit but invalid format fields fail without trying another reader. `client/launch.rs` contains both `LaunchConfig` and the `LaunchCapabilities` probe response. SDK tests exercise its encoder against the runtime decoder; runtime tests also consume independently captured release payloads. Launch and cloud requests deserialize into concrete wire types before conversion; duplicate recognized fields and conflicting previous/current spellings are rejected. Database migrations retain JSON remapping to preserve unrelated saved fields. Previous saved-format decoding and the duplicate-key JSON fixture reader in the SDK are test-only. The runtime-owned launch type remains in the runtime crate to avoid a dependency cycle. Normal database reads and writes use the migrated current representation.

Compatibility format conversions use `to_current` and `to_previous_version` to make their direction explicit. Versioned modules target the previous contract named by their release folder. `encode` and `decode` remain at serialization and parsing entry points that select or accept multiple formats; serde callbacks use `deserialize_*`.

Saved-config downgrade dispatch and database record preparation live in `crates/db/lib/compat/config.rs`; the CLI owns preflight and rollback transactions. Each versioned `to_previous_version` adapter prepares that contract or rejects settings it cannot preserve; the v0.7.0 adapter returns no rewrite for supported records. These version folders describe contracts from previous versions, not sequential migration steps.
