...
Runtime SDK and environment path inputs are captured when a local backend is constructed. Changes take effect on a new backend; path accessors use its resolved settings and filesystem fallbacks without rereading managed policy. Callers constructing a bare `GlobalConfig` must provide runtime path values themselves or obtain the resolved config from a backend.
