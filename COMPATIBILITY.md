...
Linux local branching can place subsequent ephemeral RAM generations in an owner-only tmpfs namespace under `/dev/shm/microsandbox-memory-<uid>/`, separated by canonical backend-cache identity. The first capture uses disk backing: configured guest MiB excludes some architecture-specific mappings, so only a retained capture establishes the exact capacity required for RAM admission. The caller reserves both the original backend handoff lock and the RAM-side lock; the runtime uses RAM only when that exact capacity is known, the backend lacks reflink support, the reservation exists and admission fits host, cgroup and tmpfs capacity. Otherwise it prepares the existing disk-backed generation before freezing. A changed topology discards the old RAM reservation and uses a complete disk-backed capture. Serialized local handoffs are unchanged. RAM files are immutable and independently pinned; reclamation skips pending handoffs, active writers and mapped children, even after the original home is deleted. This is reclaimable, potentially swappable local backing, not durable snapshot storage or a promise of zero-copy generations.
