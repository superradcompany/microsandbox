...
use super::{
    MetricsCollection, MetricsErrorPolicy, MetricsExportBatch, MetricsExporter,
...
pub(super) struct ExporterWorker {
...
impl ExporterWorker {
    ...
    pub(super) async fn run(mut self) -> MicrosandboxResult<()> {
    ...
    /// retries it.
    async fn run_export(&mut self) -> MicrosandboxResult<()> {
    ...
    /// Build a `MicrosandboxError` for an operation that exceeded its timeout.
    fn timeout_error(operation: &str, timeout: Duration) -> MicrosandboxError {
        MicrosandboxError::Custom(format!("{operation} timed out after {timeout:?}"))
    }
}
...
mod tests {
    ...
    use super::*;
    use crate::metrics::mocks::collection;
    use tokio::sync::mpsc;
