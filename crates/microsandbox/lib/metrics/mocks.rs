...
use crate::{MicrosandboxError, MicrosandboxResult, sandbox::SandboxMetrics};

use super::{MetricsCollection, MetricsExportBatch, MetricsExporter, SandboxMetricSnapshot};
...
#[derive(Default)]
pub(super) struct RecordingExporter {
    ...
    pub(super) fail_exports: AtomicBool,

    /// Number of [`MetricsExporter::shutdown`] calls observed.
    pub(super) shutdown_count: AtomicUsize,
}
...
impl RecordingExporter {
    ...
    pub(super) fn batches(&self) -> Vec<MetricsExportBatch> {
        self.batches
            .lock()
    ...
    fn export(&self, batch: Arc<MetricsExportBatch>) -> BoxFuture<'static, MicrosandboxResult<()>> {
        ...
        Box::pin(async move {
            if collector.fail_exports.load(Ordering::Acquire) {
                return Err(MicrosandboxError::Custom("export failed".into()));
            }
            collector
                .batches
                .lock()
...
pub(super) fn collection(seq: i32) -> MetricsCollection {
    MetricsCollection {
