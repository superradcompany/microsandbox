...
#[cfg(test)]
use super::MetricsCollection;
use super::{
    MetricsExporter, MetricsExporterConfig, builder::MetricsCollectorBuilder, reader::CollectFn,
    worker::ExporterWorker,
};
...
pub struct RunningCollector {
    ...
    join: JoinHandle<MicrosandboxResult<()>>,
}

/// Validated configuration handed to [`MetricsCollector::from_config`]
/// by the builder.
pub(super) struct CollectorConfig {
    pub(super) collect_interval: Duration,
    pub(super) collect_fn: CollectFn,
    pub(super) collectors: Vec<(Arc<dyn MetricsExporter>, MetricsExporterConfig)>,
}
...
impl MetricsCollector {
    /// Create a new metrics collector builder.
    ...
    pub(super) fn from_config(config: CollectorConfig) -> Self {
        Self {
    ...
    pub async fn start(self) -> MicrosandboxResult<RunningCollector> {
    ...
    async fn run(self, mut cmd_rx: mpsc::Receiver<CollectorCmd>) -> MicrosandboxResult<()> {
        let Self {
            collect_interval,
            collect_fn,
            collectors,
        ...
        let mut tasks: JoinSet<MicrosandboxResult<()>> = JoinSet::new();
        ...
        let mut first_error: Option<MicrosandboxError> = None;
        while let Some(join_result) = tasks.join_next().await {
            match join_result {
                ...
                Err(err) if err.is_panic() => {
                    if first_error.is_none() {
                        first_error = Some(MicrosandboxError::Custom(
...
impl RunningCollector {
    ...
    pub async fn shutdown(self) -> MicrosandboxResult<()> {
        let _ = self.cmd_tx.send(CollectorCmd::Shutdown).await;
        match self.join.await {
            Ok(result) => result,
            Err(err) if err.is_panic() => Err(MicrosandboxError::Custom(
    ...
    #[cfg(test)]
    pub(super) async fn inject_for_test(&self, collection: MetricsCollection) {
        let _ = self
            .cmd_tx
