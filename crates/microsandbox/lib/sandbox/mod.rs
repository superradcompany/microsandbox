...
async fn insert_sandbox_record(
    ...
    db.transaction(|txn| {
        ...
        async move {
            ...
            // Atomic volume registration — see `auto_volumes` doc on
            // `SandboxConfig`. A failure here rolls the entire
            // transaction back, so neither the sandbox row nor any
            // volume row is observable.
            for intent in &auto_volumes {
