...
fn make_artifact(parent: &Path, name: &str, upper_bytes: &[u8]) -> (std::path::PathBuf, String) {
    ...
    let upper_sha256 = format!("sha256:{}", hex::encode(hasher.finalize()));
...
#[tokio::test]
async fn open_verifies_a_valid_artifact() {
...
#[tokio::test]
async fn open_rejects_tampered_upper() {
    let tmp = TempDir::new().unwrap();
...
async fn open_rejects_unknown_schema() {
    ...
    std::fs::write(
        ...
        br#"{"schema":42,"format":"raw","fstype":"ext4","image":{"ref":"x","manifest_digest":"sha256:01"},"parent":null,"created_at":"2026-05-01T12:00:00Z","labels":{},"upper":{"file":"upper.ext4","size_bytes":4,"sha256":"sha256:02"},"source_sandbox":null}"#,
    )
    .unwrap();
