//! A launch document must not choose a policy by repeating a JSON key.
...
impl<'de> Visitor<'de> for UniqueVisitor {
    ...
    fn expecting(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter.write_str("launch JSON with unique object keys")
    }
    ...
    fn visit_f64<E: de::Error>(self, value: f64) -> Result<Value, E> {
        Number::from_f64(value)
            .map(Value::Number)
            .ok_or_else(|| E::custom("nonfinite launch number"))
    }
    ...
    fn visit_map<A: MapAccess<'de>>(self, mut object: A) -> Result<Value, A::Error> {
        ...
        while let Some(key) = object.next_key::<String>()? {
            if fields.contains_key(&key) {
                ...
                return Err(de::Error::custom("duplicate launch JSON field"));
            }
