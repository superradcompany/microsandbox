...
impl DnsBuilder {
    ...
    /// cache.
    ///
    /// TODO(phase-2): retrofit to lazy-parse + accumulate-at-`.build()`,
    /// matching the model the new `NetworkPolicy::builder()` will use.
    /// See `PLAN-network-policy.md` §Phase-2 follow-ups.
