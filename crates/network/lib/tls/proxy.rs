...
pub(crate) struct TlsProxyContext {
    ...
    /// SOCKS5 proxy to dial `connect_dst` through, when `upstream_stream` is
    /// `None`. See [`crate::proxy::connect_upstream`].
    pub(crate) transparent_proxy: Option<SocketAddr>,
...
async fn bypass_relay(
    ...
    let mut server = match upstream_stream {
        Some(s) => s,
        None => connect_upstream(dst, &proxy_connect, &shared, transparent_proxy).await?,
    };
...
pub(crate) async fn intercept_relay(
    ...
    let server_stream = match upstream_stream {
        Some(s) => s,
        None => connect_upstream(connect_dst, &proxy_connect, &shared, transparent_proxy).await?,
    };
