    ...
    loop {
        ...
        while let Some(frame) = device.stage_next_frame() {
            match classify_frame(frame) {
                FrameAction::TcpSyn { dst, .. } => {
                    // Policy check before socket creation.
                    if network_policy
                        .evaluate_egress(dst, Protocol::Tcp)
                        .is_allow()
                        && !conn_tracker.has_socket_for(&dst)
                    {
                        conn_tracker.create_tcp_socket(dst, &mut sockets);
                    }
        ...
        // ── Phase 2: Process egress + maintenance ────────────────────────
        loop {
        ...
        // ── Phase 3: Service established connections ─────────────────────
        ...
        // ── Phase 2b: Flush egress after Phase 3 ────────────────────────
        // Phase 3 may have written data to smoltcp sockets (e.g. proxy
        // responses, DNS replies). A second egress pass ensures this data
        // reaches the guest immediately instead of waiting for the next
        // iteration.
        loop {
            let result = iface.poll_egress(now, &mut device, &mut sockets);
            if matches!(result, smoltcp::iface::PollResult::None) {
...
fn smoltcp_now() -> Instant {
    // smoltcp Instants are milliseconds from an arbitrary epoch. Using
    // system uptime avoids clock adjustment issues.
        ...
        IpProtocol::Udp => {
            ...
            if udp.dst_port() == 53 {
                ...
                FrameAction::UdpRelay {
                    src: SocketAddr::new(ipv4.src_addr().into(), udp.src_port()),
                    dst: SocketAddr::new(ipv4.dst_addr().into(), udp.dst_port()),
                }
            }
        }
...
fn classify_ipv6(payload: &[u8]) -> FrameAction {
    ...
    match ipv6.next_header() {
        ...
        IpProtocol::Udp => {
            let Ok(udp) = UdpPacket::new_checked(ipv6.payload()) else {
                return FrameAction::Passthrough;
            };
            if udp.dst_port() == 53 {
                FrameAction::Dns
                ...
                FrameAction::UdpRelay {
                    src: SocketAddr::new(ipv6.src_addr().into(), udp.src_port()),
                    dst: SocketAddr::new(ipv6.dst_addr().into(), udp.dst_port()),
                }
            }
        }
        _ => FrameAction::Passthrough, // ICMPv6, etc.
    }
}
