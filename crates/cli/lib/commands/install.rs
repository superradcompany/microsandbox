...
use super::common::{
    SandboxConfigSources, SandboxOpts, apply_sandbox_opts_after_config, validate_mount_dir_spec,
    validate_mount_disk_spec, validate_mount_file_spec, validate_mount_named_spec,
...
pub struct InstallArgs {
    ...
    /// Image to install (e.g. python, ubuntu). May be omitted when an explicit config supplies it.
...
pub async fn run(args: InstallArgs) -> anyhow::Result<()> {
    ...
    let resolved = sandbox_config::resolve(&args.config)?;
    let image = resolved.image(args.image.as_deref(), None)?;
    let registry_auth = resolved.registry_auth()?;
    let display_image = image.display();
    let alias_name = args
        .name
        .as_deref()
        .unwrap_or_else(|| derive_name(&display_image));
    ...
    // Resolve and quote all config paths before a forced install can replace an existing alias.
    let script = build_script(args.image.as_deref(), &display_image, alias_name, &args)?;
    ...
    if !no_pull && let Some(reference) = image.oci_reference() {
        super::image::pull_if_missing_with_auth(
            reference,
            false,
            super::pull::PullMaterialization::Layered,
            registry_auth,
        )
        .await?;
    }
...
fn build_script(
    ...
    #[cfg(windows)]
    {
        build_cmd_script(image, display_image, alias_name, args)
    }
    #[cfg(not(windows))]
    {
        if args.tmp {
            build_tmp_script(image, display_image, args)
        } else {
            build_persisted_script(image, display_image, alias_name, args)
        }
    }
}
...
fn build_tmp_script(
    ...
    append_config_options(&mut parts, &args.config, shell_quote)?;
    ...
    script.push_str(&format!("# image: {}\n", sanitize_comment(display_image)));
...
fn build_cmd_script(
    ...
    append_config_options(&mut parts, &args.config, cmd_quote)?;
...
/// Convert install's intentionally smaller CLI surface into the shared option model for validation.
fn install_sandbox_opts(args: &InstallArgs) -> SandboxOpts {
    SandboxOpts {
        cpus: args.cpus,
...
/// Record explicit config inputs as absolute paths so an installed alias is cwd-independent.
fn append_config_options(
...
mod tests {
    ...
    fn args(tmp: bool) -> InstallArgs {
        InstallArgs {
            config: SandboxConfigSources::default(),
    ...
    #[test]
    fn explicit_config_can_supply_the_installed_image() {
    ...
    #[test]
    fn install_still_requires_an_image_source() {
        let error = TestCli::try_parse_from(["msb"]).unwrap_err();
    ...
    fn windows_build_script_invokes_adjacent_msb_exe() {
        let args = args(true);
        let script = build_script(Some("alpine"), "alpine", "hello", &args).unwrap();
        assert!(is_generated_alias(&script));
