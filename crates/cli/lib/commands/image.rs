...
pub async fn run_prune(args: ImagePruneArgs) -> anyhow::Result<()> {
    ...
    prune_report_kv("Image refs", &report.image_refs_removed.to_string());
    prune_report_kv("Manifests", &report.manifests_removed.to_string());
    prune_report_kv("Layers", &report.layers_removed.to_string());

    if report.fsmeta_removed > 0 {
        prune_report_kv("Fsmeta", &report.fsmeta_removed.to_string());
    }

    if report.vmdk_removed > 0 {
        prune_report_kv("VMDK", &report.vmdk_removed.to_string());
    }

    if let Some(bytes) = report.bytes_reclaimed {
        prune_report_kv("Reclaimed", &format_bytes_u64(bytes));
    }
...
/// Print an unstyled image prune report row.
fn prune_report_kv(key: &str, value: &str) {
    eprintln!("  {key:<14}{value}", key = format!("{key}:"));
}
