...
pub(super) fn parse_and_validate(file: &mut File) -> Result<ParsedImage, Ext4Error> {
...
pub(super) fn read_block_at(file: &mut File, block: u64) -> Result<Vec<u8>, Ext4Error> {
...
pub(super) fn write_block_at(file: &mut File, block: u64, data: &[u8]) -> Result<(), Ext4Error> {
    file.seek(SeekFrom::Start(block * EXT4_BLOCK_SIZE as u64))?;
    file.write_all(data)?;
    Ok(())
}
