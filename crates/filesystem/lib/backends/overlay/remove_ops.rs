...
use super::layer;
use super::origin;
use super::types::{NodeState, RedirectState};
use super::whiteout;
...
pub(crate) fn do_rename(
    ...
    if let Some(ref node) = source_node {
        if source_is_dir {
            ...
            let has_lower = whiteout::has_lower_entry(fs, olddir, oldname.to_bytes())?;
    ...
    if let Ok(ref de) = dest_entry {
        ...
        if dest_type == libc::S_IFDIR as u32 {
            remove_upper_dir_artifacts(new_parent_fd, newname)?;
            // Remove the empty dest dir from upper so renameat doesn't fail with EEXIST.
            unsafe { libc::unlinkat(new_parent_fd, newname.as_ptr(), libc::AT_REMOVEDIR) };
        }
    }
    ...
    if let Some(ref node) = source_node {
        if source_is_dir {
            ...
            if dest_has_lower {
                ...
                node.opaque.store(true, Ordering::Relaxed);
            }
        }
    }

    // Update dentry cache.
    update_dentry_cache(fs, olddir, oldname_id, newdir, newname, &source_node);
...
fn rename_lower_directory(
    ...
    // Copy override xattr from lower source if available.
    ...
    if let Ok(Some(ovr)) = stat_override::get_override(lower_fd) {
        let _ = stat_override::set_override_at(new_parent_fd, newname, ovr.uid, ovr.gid, ovr.mode, ovr.rdev);
    }

    // Open the newly created upper directory to write the redirect xattr.
    let dir_fd = open_dir_by_name(new_parent_fd, newname)?;
    ...
    *node.redirect.write().unwrap() = Some(RedirectState {
    ...
    // Update dentry cache.
    update_dentry_cache(fs, olddir, oldname_id, newdir, newname, &Some(std::sync::Arc::clone(&{
        let nodes = fs.nodes.read().unwrap();
        nodes.get(&node.inode).cloned().unwrap()
    })));
...
fn update_dentry_cache(
    ...
    newdir: u64,
    newname: &CStr,
    source_node: &Option<std::sync::Arc<super::types::OverlayNode>>,
) {
    ...
    if let Some(dentry) = dentries.remove(&(olddir, oldname_id)) {
        ...
        if let Some(node) = source_node {
            node.primary_parent.store(newdir, Ordering::Relaxed);
        ...
        dentries.insert(
            ...
            super::types::Dentry {
                ...
                name: newname_id,
                node: dentry.node,
                flags: super::types::DentryFlags::EMPTY,
            },
        );
    }
}
